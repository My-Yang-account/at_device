...
#include "ftplib_config.h"

#if defined(__unix__) || defined(__VMS) || defined(RTTHREAD)
#include <unistd.h>
#endif
#if defined(_WIN32)
#include <windows.h>
...
#include <errno.h>
#include <ctype.h>
#if defined(__unix__) || defined(RTTHREAD)
#include <sys/time.h>
#include <sys/types.h>
...
/*
 * strdup - return a ftplib_malloc'ed copy of a string
 */
char *strdup(const char *src)
{
    int l = strlen(src) + 1;
    char *dst = ftplib_malloc(l);
    if (dst)
    strcpy(dst,src);
    return dst;
}
...
static int FtpXfer(const char *localfile, const char *path, netbuf *nControl, int typ, int mode)
{
    ...
    dbuf = ftplib_malloc(FTPLIB_BUFSIZ);
    if (typ == FTPLIB_FILE_WRITE)
    {
...
GLOBALDEF void FtpQuit(netbuf *nControl)
{
    ...
    ftplib_free(nControl->buf);
    ftplib_free(nControl);
}
