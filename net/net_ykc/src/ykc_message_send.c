...
static void net_ykc_message_send_thread_entry(void *parameter)
{
    ...
    while(1)
    {
        s_ykc_socket_info.program_state = step;
        ...
        if((handle->net_fault) &NET_FAULT_PHYSICAL_LAYER){
            ...
            s_ykc_socket_info.socket_state = YKC_SOCKET_STATE_PHY;
        ...
        if((handle->net_fault) &NET_FAULT_SIM_CARD){
            ...
            s_ykc_socket_info.socket_state = YKC_SOCKET_STATE_SIM;
        ...
        if((handle->net_fault) &NET_FAULT_DATA_LINK_LAYER){
            ...
            s_ykc_socket_info.socket_state = YKC_SOCKET_STATE_DATA_LINK;
        ...
        if((handle->net_fault) &NET_FAULT_MODULE_INIT){
            ...
            s_ykc_socket_info.socket_state = YKC_SOCKET_STATE_MODULE_INIT;
        ...
        /** 进行域名解析 */
        if(s_ykc_socket_info.domain_is_prase == 0x00){
            char ip_str[0x10];   /** 点分十进制式IP，最大长度15，预留一位 */
            if(ykc_socket_domain_parse(0x00, host, strlen(host), ip_str, sizeof(ip_str)) >= 0x00){
                net_operation_set_target_socket_domain(ip_str, strlen(ip_str));
                net_operation_set_target_socket_port(port);
                s_ykc_socket_info.domain_is_prase = 0x01;

                LOG_D("ykc domain prase success[%s:%d]", ip_str, port);
            }
        }
        /***************************************************** [登录认证] **********************************************************/
        /***************************************************** [登录认证] **********************************************************/
        if(s_ykc_socket_info.socket_state != YKC_SOCKET_STATE_LOGIN_SUCCESS){
            rt_kprintf("state(%d, %d, %d)\n", s_ykc_socket_info.socket_state, step, (rt_tick_get() - delay));
            switch(step){
            case NET_YKC_NET_STATE_OPEN_SOCKET:
                s_ykc_socket_info.socket_state = YKC_SOCKET_STATE_OPEN;
                ...
                if(((rt_tick_get() - delay) > NET_YKC_LOGIN_OPERATION_INTERVAL) || is_power_on){
                    ...
                    if(result >= 0){
                        ...
                        /** 状态变化，提前改变状态 */
                        s_ykc_socket_info.program_state = step;
                        s_ykc_socket_info.socket_state = YKC_SOCKET_STATE_LOGIN_WAIT;
            ...
            case NET_YKC_NET_STATE_LOGIN:
            {
                ...
                while(rentry < NET_YKC_WAIT_LOGIN_RENTRY){
                    if(ykc_net_event_receive(NET_YKC_EVENT_HANDLE_SERVER, NET_YKC_EVENT_TYPE_RESPONSE, 0x00,
                        ...
                        s_ykc_socket_info.socket_state = YKC_SOCKET_STATE_LOGIN_SUCCESS;
        ...
        if(s_ykc_socket_info.socket_state != YKC_SOCKET_STATE_LOGIN_SUCCESS){   /* 未登录上服务器前不进行网络数据交互事件处理 */
            for(uint8_t gunno = 0; gunno < NET_SYSTEM_GUN_NUMBER; gunno++){
        ...
        for(uint8_t gunno = 0; gunno < NET_SYSTEM_GUN_NUMBER; gunno++){
            ...
            if(_event){
                ...
                if(ykc_net_event_receive(NET_YKC_EVENT_HANDLE_SERVER, NET_YKC_EVENT_TYPE_REQUEST, gunno,
                    ...
                    if(pro_result != 0x00){
                        ykc_chargepile_update_result_report(0x03);   /** 默认失败原因是超时 */
                    }
                }
                /***** [运营平台远程控制并充启机请求] *****/
                if(ykc_net_event_receive(NET_YKC_EVENT_HANDLE_SERVER, NET_YKC_EVENT_TYPE_REQUEST, gunno,
...
int32_t ykc_message_send_init(void)
{
    ...
    net_thread_init_hook(&s_ykc_message_send_thread, &entry, sizeof(entry), NET_THREAD_RUNNING_OPTION_ENTRY_MAX);

    memset(name, 0x00, NET_THREAD_MONITOR_NAME_MAX);
    memcpy(name, "yk_pms", strlen("yk_pms"));
    net_thread_init_hook(&s_ykc_message_send_thread, name, strlen((char*)name), NET_THREAD_RUNNING_OPTION_NAME);
    ...
    net_thread_init_hook(&s_ykc_server_message_pro_thread, &entry, sizeof(entry), NET_THREAD_RUNNING_OPTION_ENTRY_MAX);

    memset(name, 0x00, NET_THREAD_MONITOR_NAME_MAX);
    memcpy(name, "yk_smp", strlen("yk_smp"));
    net_thread_init_hook(&s_ykc_server_message_pro_thread, name, strlen((char*)name), NET_THREAD_RUNNING_OPTION_NAME);
