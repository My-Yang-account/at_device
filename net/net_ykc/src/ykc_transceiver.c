...
int32_t ykc_transceiver_init(void)
{
    ...
    net_thread_init_hook(&s_ykc_message_recv_thread, &entry, sizeof(entry), NET_THREAD_RUNNING_OPTION_ENTRY_MAX);

    memset(name, 0x00, NET_THREAD_MONITOR_NAME_MAX);
    memcpy(name, "yk_drec", strlen("yk_drec"));
    net_thread_init_hook(&s_ykc_message_recv_thread, name, strlen((char*)name), NET_THREAD_RUNNING_OPTION_NAME);
