...
static uint8_t ykc_readline_data(int fd)
{
    ...
    while(1)
    {
        if(net_get_ota_info()->state < NET_OTA_STATE_LOGIN_WAIT){
            if((socket->state >= YKC_SOCKET_STATE_LOGIN_WAIT) && (socket->fd >= 0x00)){
                if(socket->state == YKC_SOCKET_STATE_LOGIN_SUCCESS){
//                    if(hdj == 0){
//                        if(++hk > 90){
//                            hdj = 1;
//                            lld = 1;
...
//                    }
                ...
                if((length = ykc_readline_data(socket->fd))/* || lld*/){
//                    if(lld){
//                        lld = 0;
////                        length=  sizeof(ggk);
////                        memcpy(s_ykc_message_recv_buff, ggk, sizeof(ggk));
//                        length=  sizeof(power_data);
//                        memcpy(s_ykc_message_recv_buff, power_data, sizeof(power_data));
//                        rt_kprintf("zhfFH(%d)\n",((Net_YkcPro_Head_t*)s_ykc_message_recv_buff)->type);
//                    }
