...
/*****************************************************************
 * 函数名                   ykc_monitor_callback_response_request_server_info
 * 功能                       处理运营平台的请求服务器信息响应
 *           data       数据
 *           length     数据长度
 * 返回                        无
 ****************************************************************/
static void ykc_monitor_callback_response_request_server_info(uint8_t* data, uint16_t length)
{
#ifdef NET_YKC_MONITOR_USING_EXTEND_PROTOCOL
    if(data == NULL){
        LOG_E("ykc monitor input data is null when call ykc_monitor_callback_response_request_server_info");
        return;
    }
    if(length < sizeof(Net_YkcMonitorPro_Preq_RequestServerInfo_t)){
        LOG_E("ykc monitor input data length out of range when call ykc_monitor_callback_response_request_server_info|%d, %d", \
                length, sizeof(Net_YkcMonitorPro_Preq_RequestServerInfo_t));
        return;
    }

    Net_YkcMonitorPro_Preq_RequestServerInfo_t *request = (Net_YkcMonitorPro_Preq_RequestServerInfo_t*)data;
    if(ykc_monitor_pile_number_invalid(request->body.pile_number, request->head.type)){
        LOG_W("ykc monitor request server info pile number error(%s)", request->body.pile_number);
        return;
    }
    if(request->body.info_type == 0x00){
//        ykc_monitor_net_event_send(NET_YKC_MONITOR_USER_EVENT_HANDLE_SERVER, NET_YKC_MONITOR_EVENT_TYPE_REQUEST, 0x00, NET_YKC_MONITOR_USER_SREQ_EVENT_QUERY_SET_CONFIG_INFO);
    }
#endif /* NET_YKC_MONITOR_USING_EXTEND_PROTOCOL */
}
