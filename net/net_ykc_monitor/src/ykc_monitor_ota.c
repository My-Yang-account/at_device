...
static uint8_t s_ykc_monitor_ota_file_ver[5];
...
//                    while(s_ykc_monitor_ota_info->flag.permit_ota == 0x00){
            ...
            case NET_OTA_STATE_OPEN_LINK:
                if(s_ykc_monitor_ota_info->flag.start_ota){
                    ...
                    memset(s_ykc_monitor_ota_file_ver, '\0', sizeof(s_ykc_monitor_ota_file_ver));

                    sscanf(((char*)s_handle->get_system_data(NET_SYSTEM_DATA_NAME_HARDWARE_VERSION, NULL, NET_SYSTEM_DATA_OPTION_PLAT_YKC_MONITOR)), "%u", &ver_data);
                    if(ver_data >= 7103){
                        memcpy(s_ykc_monitor_ota_file_ver, "-V10", strlen("-V10"));
                    }else{
                        memcpy(s_ykc_monitor_ota_file_ver, "-V01", strlen("-V01"));
                    }

                    LOG_D("ykc monitor ftp_file_size(%d)[%x, %x, %d][%s], OTA FTP link success\n", pack_len, s_ykc_monitor_spiflash_addr,
                            NET_OTA_DATA_ADDR, s_ykc_monitor_ota_storage_info.pack_len, s_ykc_monitor_ota_file_ver);
            ...
            case NET_OTA_STATE_SUCCESS:
                if(s_ykc_monitor_ota_info->flag.start_ota){
                    ...
                    rt_kprintf("ffffffffffffffffffffff\n");
...
static int32_t ykc_monitor_parse_ota_data(const uint8_t *data, uint32_t len)
{
    ...
    if (s_ykc_monitor_spiflash_addr + len >= NET_OTA_DATA_ADDR + s_ykc_monitor_ota_storage_info.pack_len) {
        ...
        if(s_ykc_monitor_ota_flag.file_correct == true){
            if(memcmp((s_ykc_monitor_ota_file_flag + NET_YKC_MONITOR_OTA_FILE_FLAG_LEN), s_ykc_monitor_ota_file_ver, NET_YKC_MONITOR_OTA_FILE_VERSION_FLAG_LEN) != 0){
