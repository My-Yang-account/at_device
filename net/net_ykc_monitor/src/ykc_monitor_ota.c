...
static int32_t ykc_monitor_parse_ota_data(const uint8_t *data, uint32_t len)
{
    ...
    if (s_ykc_monitor_spiflash_addr + len >= NET_OTA_DATA_ADDR + s_ykc_monitor_ota_storage_info.pack_len) {
        ...
        if(s_ykc_monitor_ota_flag.file_correct == true){
            char *ver = ((char*)s_handle->get_system_data(NET_SYSTEM_DATA_NAME_SOFTWARE_MODEL, NULL, 0x00, NET_SYSTEM_DATA_OPTION_PLAT_YKC_MONITOR));
            char ver_bin[NET_YKC_MONITOR_OTA_FILE_VERSION_FLAG_LEN];

            memset(ver_bin, 0x00, NET_YKC_MONITOR_OTA_FILE_VERSION_FLAG_LEN);
            memcpy(ver_bin, (s_ykc_monitor_ota_file_flag + NET_YKC_MONITOR_OTA_FILE_FLAG_LEN + 0x01), (NET_YKC_MONITOR_OTA_FILE_VERSION_FLAG_LEN - 0x01));
            if(strstr(ver, ver_bin) == 0){
                s_ykc_monitor_ota_flag.file_correct = false;   /* 文件版本错误 */
                LOG_W("ykc monitor ota file version error, is not allow update(%s, %s)\n", ver, ver_bin);
            }
        }
...
int32_t ykc_monitor_ota_init(void)
{
    ...
    net_thread_init_hook(&s_ykc_monitor_ota_thread, &entry, sizeof(entry), NET_THREAD_RUNNING_OPTION_ENTRY_MAX);

    memset(name, 0x00, NET_THREAD_MONITOR_NAME_MAX);
    memcpy(name, "ym_ota", strlen("ym_ota"));
    net_thread_init_hook(&s_ykc_monitor_ota_thread, name, strlen((char*)name), NET_THREAD_RUNNING_OPTION_NAME);
