...
#define NET_YKC_MONITOR_TPLAT_SOCKET_INFO_INTERVAL                   (30 *60000)  /* 目标平台socket信息上报间隔 */
#define NET_YKC_MONITOR_BILLING_RULE_INFO_INTERVAL                   (30 *60000)  /* 目标平台计费信息上报间隔 */
#define NET_YKC_MONITOR_TIMESYNC_INTERVAL                            (8 *3600 *1000)  /* 向平台请求对时间隔 */
...
#define NET_YKC_MONITOR_WAIT_DEV_INFO_RESPONSE_TIME                  5000        /* 等待设备信息响应时间(ms)，用于重发报文 */
#define NET_YKC_MONITOR_WAIT_INFO_PARA_CONFIRM_TIME                  5000        /* 等待对桩信息、参数修改的确认结果时间(ms) */
#define NET_YKC_MONITOR_WAIT_TIMESYNC_RESPONSE_TIME                  10000       /* 等待时间同步响应时间(ms)，用于重发报文 */
...
typedef struct{
    uint8_t tplat_log : 1;                                                       /* 功能开关：上报目标平台日志 */
}ykc_monitor_fswitch;

typedef struct{
    struct{
        uint8_t recved_info_modify : 1;                                          /* 辅助信息：已接收到了信息修改报文 */
        uint8_t dev_info_reported : 1;                                           /* 辅助信息：设备信息已上报 */
        uint8_t is_time_sync : 1;                                                /* 辅助信息：已进行时间同步 */
        uint8_t is_requesting_info : 1;                                          /* 辅助信息：正在向服务器请求信息 */
    }flag;
    uint32_t info_modify_tick;                                                   /* 信息修改时基 */
    uint32_t dev_info_tick;                                                      /* 设备信息上报时基 */

    uint8_t request_info_type;                                                   /* 向服务器请求信息类型 */
}ykc_monitor_assistant_info;
...
NET_DEF_SRAM2 uint8_t s_ykc_monitor_current_transaction_number[NET_SYSTEM_GUN_NUMBER][NET_YKC_MONITOR_SERIAL_NUMBER_LENGTH_DEFAULT];
NET_DEF_SRAM2 static uint8_t s_ykc_monitor_same_transaction_report_count[NET_SYSTEM_GUN_NUMBER];
NET_DEF_SRAM2 static uint8_t s_ykc_monitor_transaction_verify[NET_SYSTEM_GUN_NUMBER];
NET_DEF_SRAM2 static struct ykc_monitor_wait_response s_ykc_monitor_wait_response;
NET_DEF_SRAM2 static ykc_monitor_socket_info_t s_ykc_monitor_socket_info;
NET_DEF_SRAM2 static uint16_t s_ykc_monitor_message_serial_number[NET_SYSTEM_GUN_NUMBER];                      /* 报文序列号 */
NET_DEF_SRAM2 static uint32_t s_ykc_monitor_message_send_state[NET_SYSTEM_GUN_NUMBER];                        /* 报文发送状态 */
NET_DEF_SRAM2 static uint32_t s_ykc_monitor_chargepile_event[NET_YKC_MONITOR_EVENT_TYPE_SIZE][NET_SYSTEM_GUN_NUMBER];
NET_DEF_SRAM2 static uint32_t s_ykc_monitor_server_event[NET_YKC_MONITOR_EVENT_TYPE_SIZE][NET_SYSTEM_GUN_NUMBER];

#ifdef NET_YKC_MONITOR_AS_MONITOR
#ifdef NET_YKC_MONITOR_USING_EXTEND_PROTOCOL
NET_DEF_SRAM2 static uint32_t s_ykc_monitor_tplat_socket_tick = 0x00;
NET_DEF_SRAM2 static uint32_t s_ykc_monitor_billing_rule_tick = 0x00;
NET_DEF_SRAM2 static uint32_t s_ykc_monitor_timesync_tick = 0x00;
#endif /* #ifdef NET_YKC_MONITOR_USING_EXTEND_PROTOCOL */
NET_DEF_SRAM2 static uint32_t s_ykc_monitor_current_log_id = 0x00;
NET_DEF_SRAM2 static uint32_t s_ykc_monitor_log_tick = 0x00;
NET_DEF_SRAM2 static uint8_t s_ykc_monitor_log_step = NET_YKC_MONITOR_LOG_STEP_NULL;
NET_DEF_SRAM2 static struct ykc_monitor_loghead s_ykc_monitor_loghead;

NET_DEF_SRAM2 static ykc_monitor_fswitch s_ykc_monitor_fswitch;
NET_DEF_SRAM2 static ykc_monitor_assistant_info s_ykc_monitor_assistant_info;

NET_DEF_SRAM2 static uint32_t s_ykc_monitor_user_chargepile_event[NET_YKC_MONITOR_EVENT_TYPE_SIZE][NET_SYSTEM_GUN_NUMBER];
NET_DEF_SRAM2 static uint32_t s_ykc_monitor_user_server_event[NET_YKC_MONITOR_EVENT_TYPE_SIZE][NET_SYSTEM_GUN_NUMBER];
NET_DEF_SRAM2 static uint32_t s_ykc_monitor_external_chargepile_event[NET_SYSTEM_GUN_NUMBER];
#endif /* NET_YKC_MONITOR_AS_MONITOR */

NET_DEF_SRAM2 static struct rt_thread s_ykc_monitor_message_send_thread;
NET_DEF_SRAM0 static uint8_t s_ykc_monitor_message_send_thread_stack[NET_YKC_MONITOR_MESSAGE_SEND_THREAD_STACK_SIZE];
NET_DEF_SRAM2 static struct rt_thread s_ykc_monitor_server_message_pro_thread;
NET_DEF_SRAM0 static uint8_t s_ykc_monitor_server_message_pro_thread_stack[NET_YKC_MONITOR_SERVER_MESSAGE_PRO_THREAD_STACK_SIZE];
NET_DEF_SRAM2 static ykc_monitor_response_message_buf_t s_ykc_monitor_response_buff;
NET_DEF_SRAM2 static struct rt_semaphore s_ykc_monitor_response_buff_sem;

/** 登录签到 */
NET_DEF_SRAM2 Net_YkcMonitorPro_PReq_LogIn_t g_ykc_monitor_preq_login;
/** 上报心跳 */
NET_DEF_SRAM2 Net_YkcMonitorPro_PReq_HeartBeat_t g_ykc_monitor_preq_heartbeat[NET_SYSTEM_GUN_NUMBER];   // OK
/** 计费模型验证 */
NET_DEF_SRAM2 Net_YkcMonitorPro_PReq_BillingModel_Verify_t g_ykc_monitor_preq_billing_model_verify;   // OK
/** 计费模型请求 */
NET_DEF_SRAM2 Net_YkcMonitorPro_PReq_BillingModel_Request_t g_ykc_monitor_preq_billing_model_request;  // OK
/** 上传实时监测数据 */
NET_DEF_SRAM2 Net_YkcMonitorPro_PRes_Query_PReq_Report_RealTimeData_t g_ykc_monitor_preq_report_realtime_data[NET_SYSTEM_GUN_NUMBER];  // OK
/** 充电握手 */
NET_DEF_SRAM2 Net_YkcMonitorPro_PReq_ShakeHand_t g_ykc_monitor_preq_shake_hand[NET_SYSTEM_GUN_NUMBER];   // OK
/** 参数配置 */
NET_DEF_SRAM2 Net_YkcMonitorPro_PReq_ParameterConfig_t g_ykc_monitor_preq_parameter_config[NET_SYSTEM_GUN_NUMBER];   // OK
/** 充电结束 */
NET_DEF_SRAM2 Net_YkcMonitorPro_PReq_ChargeFinish_t g_ykc_monitor_preq_charge_finish[NET_SYSTEM_GUN_NUMBER];   // OK
/** 错误报文 */
NET_DEF_SRAM2 Net_YkcMonitorPro_PReq_ErrorMessage_t g_ykc_monitor_preq_error_message[NET_SYSTEM_GUN_NUMBER];   // OK
/** 充电过程中 BMS 终止 */
NET_DEF_SRAM2 Net_YkcMonitorPro_PReq_BmsEnd_t g_ykc_monitor_preq_bms_end[NET_SYSTEM_GUN_NUMBER];   // OK
/** 充电过程中充电机终止 */
NET_DEF_SRAM2 Net_YkcMonitorPro_PReq_ChargerEnd_t g_ykc_monitor_preq_charger_end[NET_SYSTEM_GUN_NUMBER];   // OK
/** 充电过程 BMS 需求与充电机输出 */
NET_DEF_SRAM2 Net_YkcMonitorPro_PReq_BmsCommand_ChargerOut_t g_ykc_monitor_preq_bmscommand_chargerout[NET_SYSTEM_GUN_NUMBER];   // OK
/** 充电过程 BMS 信息 */
NET_DEF_SRAM2 Net_YkcMonitorPro_PReq_BmsInfo_t g_ykc_monitor_preq_bms_info[NET_SYSTEM_GUN_NUMBER];   // OK
/** 充电桩主动申请启动充电 */
NET_DEF_SRAM2 Net_YkcMonitorPro_PReq_ApplyCharge_Active_t g_ykc_monitor_preq_apply_charge_active[NET_SYSTEM_GUN_NUMBER];  // OK
/** 交易记录 */
NET_DEF_SRAM2 Net_YkcMonitorPro_PReq_TransactionRecords_t g_ykc_monitor_preq_transaction_records[NET_SYSTEM_GUN_NUMBER];
/** 地锁数据上送 */
NET_DEF_SRAM2 Net_YkcMonitorPro_PReq_GroundLock_Info_t g_ykc_monitor_preq_ground_lock_info[NET_SYSTEM_GUN_NUMBER];  // OK
/** 充电桩主动申请并充充电 */
NET_DEF_SRAM2 Net_YkcMonitorPro_PReq_ApplyMergeCharge_Active_t g_ykc_monitor_preq_apply_merge_charge_active[NET_SYSTEM_GUN_NUMBER];  // OK
/** 升级结果上送 */
NET_DEF_SRAM2 Net_YkcMonitorPro_PRes_RemoteUpdate_t g_ykc_monitor_pres_remote_update;  // OK
...
static void net_ykc_monitor_message_send_thread_entry(void *parameter)
{
    ...
    while(1)
    {
        ...
        if(s_ykc_monitor_socket_info.domain_is_prase == 0x00){
            uint8_t valid_len;
            net_plat_socket_info_t *socket_info = net_operation_get_target_socket_info(0x01);
...
#ifdef NET_YKC_MONITOR_AS_MONITOR
#ifdef NET_YKC_MONITOR_USING_EXTEND_PROTOCOL
        if(s_ykc_monitor_tplat_socket_tick > rt_tick_get()){
            s_ykc_monitor_tplat_socket_tick = rt_tick_get();
        }
        if((rt_tick_get() - s_ykc_monitor_tplat_socket_tick) > NET_YKC_MONITOR_TPLAT_SOCKET_INFO_INTERVAL){
            s_ykc_monitor_tplat_socket_tick = rt_tick_get();
            net_operation_tplat_info_trigger(0x01);
        }

        if(s_ykc_monitor_billing_rule_tick > rt_tick_get()){
            s_ykc_monitor_billing_rule_tick = rt_tick_get();
        }
        if((rt_tick_get() - s_ykc_monitor_billing_rule_tick) > NET_YKC_MONITOR_BILLING_RULE_INFO_INTERVAL){
            s_ykc_monitor_billing_rule_tick = rt_tick_get();
            net_operation_updated_billing_trigger();
        }

        if(s_ykc_monitor_assistant_info.flag.is_time_sync){
            if((rt_tick_get() - s_ykc_monitor_timesync_tick) > NET_YKC_MONITOR_TIMESYNC_INTERVAL){
                s_ykc_monitor_timesync_tick = rt_tick_get();
                s_ykc_monitor_assistant_info.flag.is_time_sync = 0x00;
...
#endif /* #ifdef NET_YKC_MONITOR_USING_EXTEND_PROTOCOL */
        if(s_ykc_monitor_assistant_info.flag.dev_info_reported == 0x00){
            if(s_ykc_monitor_assistant_info.dev_info_tick > rt_tick_get()){
                s_ykc_monitor_assistant_info.dev_info_tick = rt_tick_get();
            }
            if((rt_tick_get() - s_ykc_monitor_assistant_info.dev_info_tick) > NET_YKC_MONITOR_WAIT_DEV_INFO_RESPONSE_TIME){
                s_ykc_monitor_assistant_info.dev_info_tick = rt_tick_get();
                ykc_monitor_net_event_send(NET_YKC_MONITOR_USER_EVENT_HANDLE_CHARGEPILE, NET_YKC_MONITOR_EVENT_TYPE_REQUEST, 0x00, NET_YKC_MONITOR_USER_PREQ_EVENT_REPORT_DEV_INFO_ASYNCHRONOUSLY);
            }
        }

        if(s_ykc_monitor_assistant_info.flag.recved_info_modify){
            if(s_ykc_monitor_assistant_info.info_modify_tick > rt_tick_get()){
                s_ykc_monitor_assistant_info.info_modify_tick = rt_tick_get();
            }
            if((rt_tick_get() - s_ykc_monitor_assistant_info.info_modify_tick) > NET_YKC_MONITOR_WAIT_INFO_PARA_CONFIRM_TIME){
                s_ykc_monitor_assistant_info.info_modify_tick = rt_tick_get();
                g_ykc_monitor_sreq_pres_info_para_modify_confirm.ongoing = 0x00;
                g_ykc_monitor_sres_info_para_confirm_result.ongoing = 0x00;
                s_ykc_monitor_assistant_info.flag.recved_info_modify = 0x00;          /** 超过一定时间未接收到确认报文则丢弃此次修改请求 */
            }
        }else{
            s_ykc_monitor_assistant_info.info_modify_tick = rt_tick_get();
        }
...
//                ykc_monitor_set_message_send_state(gunno, NET_YKC_MONITOR_SEND_STATE_ONGOING, NET_YKC_MONITOR_USER_PREQ_EVENT_STARTING_INFO);
                ...
                ykc_monitor_message_send_port(NETYKC_MONITOR_PREQCMD_REPORT_STARTING_INFO, s_ykc_monitor_socket_info.fd, s_ykc_monitor_response_buff.general_transmit_buff,
                        s_ykc_monitor_response_buff.length, NULL);
...
//                ykc_monitor_set_message_send_state(gunno, NET_YKC_MONITOR_SEND_STATE_COMPLETE, NET_YKC_MONITOR_USER_PREQ_EVENT_CHARGE_FINISH);
            ...
            /***** [修改的桩信息、参数确认] *****/
            if(ykc_monitor_net_event_receive(NET_YKC_MONITOR_USER_EVENT_HANDLE_CHARGEPILE, NET_YKC_MONITOR_EVENT_TYPE_REQUEST, gunno,
                    (NET_YKC_MONITOR_EVENT_OPTION_OR |NET_YKC_MONITOR_EVENT_OPTION_CLEAR), NET_YKC_MONITOR_USER_PREQ_EVENT_INFOPARA_CONFIRM, NULL) > 0){
                Net_YkcMonitorPro_Sreq_Pres_InfoPara_ModifyConfirm_t *info_para = (Net_YkcMonitorPro_Sreq_Pres_InfoPara_ModifyConfirm_t*)(s_ykc_monitor_response_buff.general_transmit_buff);
//                ykc_monitor_set_message_send_state(gunno, NET_YKC_MONITOR_SEND_STATE_ONGOING, NET_YKC_MONITOR_USER_PREQ_EVENT_INFOPARA_CONFIRM);
                info_para->head.sequence = g_ykc_monitor_sreq_pres_info_para_modify_confirm.head.sequence;
                ykc_monitor_message_send_port(NETYKC_MONITOR_PRESCMD_INFOPARA_CONFIRM, s_ykc_monitor_socket_info.fd, s_ykc_monitor_response_buff.general_transmit_buff,
                        s_ykc_monitor_response_buff.length, NULL);
//                ykc_monitor_set_message_wait_response_state(gunno, NET_YKC_MONITOR_USER_PREQ_EVENT_INFOPARA_CONFIRM);
//                ykc_monitor_set_message_send_state(gunno, NET_YKC_MONITOR_SEND_STATE_COMPLETE, NET_YKC_MONITOR_USER_PREQ_EVENT_INFOPARA_CONFIRM);
                ykc_monitor_response_buff_release_sem();
                rt_thread_mdelay(250);
            }
        }
        ...
        for(uint8_t gunno = 0; gunno < NET_SYSTEM_GUN_NUMBER; gunno++){
            ...
            /***** [服务器功能控制开关响应] *****/
            if(ykc_monitor_net_event_receive(NET_YKC_MONITOR_USER_EVENT_HANDLE_CHARGEPILE, NET_YKC_MONITOR_EVENT_TYPE_RESPONSE, gunno,
                    (NET_YKC_MONITOR_EVENT_OPTION_OR |NET_YKC_MONITOR_EVENT_OPTION_CLEAR), NET_YKC_MONITOR_USER_PRES_EVENT_FUNCTION_SWITCH, NULL) > 0){
                Net_YkcMonitorPro_Pres_FunctionSwitch_t *fswitch = (Net_YkcMonitorPro_Pres_FunctionSwitch_t*)(s_ykc_monitor_response_buff.general_transmit_buff);

                fswitch->head.sequence = g_ykc_monitor_sreq_function_switch.head.sequence;
                ykc_monitor_message_send_port(NETYKC_MONITOR_PRESCMD_FUNCTION_SWITCH, s_ykc_monitor_socket_info.fd, s_ykc_monitor_response_buff.general_transmit_buff,
                        s_ykc_monitor_response_buff.length, NULL);
                ykc_monitor_response_buff_release_sem();
                rt_thread_mdelay(250);
            }
...
//                ykc_monitor_set_message_wait_response_state(gunno, NET_YKC_MONITOR_USER_PREQ_EVENT_REPORT_MODULE_SETUPINFO);
            ...
            /***** [上报目标socket信息请求] *****/
            if(ykc_monitor_net_event_receive(NET_YKC_MONITOR_USER_EVENT_HANDLE_CHARGEPILE, NET_YKC_MONITOR_EVENT_TYPE_REQUEST, gunno,
                    (NET_YKC_MONITOR_EVENT_OPTION_OR |NET_YKC_MONITOR_EVENT_OPTION_CLEAR), NET_YKC_MONITOR_USER_PREQ_EVENT_REPORT_TSOCKET_INFO, NULL) > 0){
#ifdef NET_YKC_MONITOR_USING_EXTEND_PROTOCOL
                Net_YkcMonitorPro_Preq_Pres_TsocketInfo_t *tplat_socket = (Net_YkcMonitorPro_Preq_Pres_TsocketInfo_t*)(s_ykc_monitor_response_buff.general_transmit_buff);

                tplat_socket->head.sequence = s_ykc_monitor_message_serial_number[gunno]++;
                ykc_monitor_message_send_port(NETYKC_MONITOR_PRESCMD_QUERY_TSOCKET_INFO, s_ykc_monitor_socket_info.fd, s_ykc_monitor_response_buff.general_transmit_buff,
                        s_ykc_monitor_response_buff.length, NULL);
//                ykc_monitor_set_message_wait_response_state(gunno, NET_YKC_MONITOR_USER_PREQ_EVENT_REPORT_TSOCKET_INFO);
            ...
            /***** [上报目标平台计费信息请求] *****/
            if(ykc_monitor_net_event_receive(NET_YKC_MONITOR_USER_EVENT_HANDLE_CHARGEPILE, NET_YKC_MONITOR_EVENT_TYPE_REQUEST, gunno,
                    (NET_YKC_MONITOR_EVENT_OPTION_OR |NET_YKC_MONITOR_EVENT_OPTION_CLEAR), NET_YKC_MONITOR_USER_PREQ_EVENT_REPORT_BILLING_INFO, NULL) > 0){
#ifdef NET_YKC_MONITOR_USING_EXTEND_PROTOCOL
                Net_YkcMonitorPro_Preq_Pres_BillingRule_t *billing = (Net_YkcMonitorPro_Preq_Pres_BillingRule_t*)(s_ykc_monitor_response_buff.general_transmit_buff);
...
//                ykc_monitor_set_message_wait_response_state(gunno, NET_YKC_MONITOR_USER_PREQ_EVENT_REPORT_TSOCKET_INFO);
            ...
            /***** [上报模块故障信息请求] *****/
            if(ykc_monitor_net_event_receive(NET_YKC_MONITOR_USER_EVENT_HANDLE_CHARGEPILE, NET_YKC_MONITOR_EVENT_TYPE_REQUEST, gunno,
                    (NET_YKC_MONITOR_EVENT_OPTION_OR |NET_YKC_MONITOR_EVENT_OPTION_CLEAR), NET_YKC_MONITOR_USER_PREQ_EVENT_REPORT_MFAULT_INFO, NULL) > 0){
#ifdef NET_YKC_MONITOR_USING_EXTEND_PROTOCOL
                Net_YkcMonitorPro_Preq_Sres_ModuleFaultInfo_t *mfault = (Net_YkcMonitorPro_Preq_Sres_ModuleFaultInfo_t*)(s_ykc_monitor_response_buff.general_transmit_buff);
                mfault->head.sequence = s_ykc_monitor_message_serial_number[gunno]++;
                ykc_monitor_message_send_port(NETYKC_MONITOR_PREQ_SRESCMD_MFAULT_INFO, s_ykc_monitor_socket_info.fd, s_ykc_monitor_response_buff.general_transmit_buff,
                        s_ykc_monitor_response_buff.length, NULL);
//                ykc_monitor_set_message_wait_response_state(gunno, NET_YKC_MONITOR_USER_PREQ_EVENT_REPORT_TSOCKET_INFO);
                ykc_monitor_response_buff_release_sem();
                rt_thread_mdelay(250);
            ...
            /***** [向服务器请求信息] *****/
            if(ykc_monitor_net_event_receive(NET_YKC_MONITOR_USER_EVENT_HANDLE_CHARGEPILE, NET_YKC_MONITOR_EVENT_TYPE_REQUEST, gunno,
                    (NET_YKC_MONITOR_EVENT_OPTION_OR |NET_YKC_MONITOR_EVENT_OPTION_CLEAR), NET_YKC_MONITOR_USER_PREQ_EVENT_REPORT_REQUEST_SERVER, NULL) > 0){
#ifdef NET_YKC_MONITOR_USING_EXTEND_PROTOCOL
                Net_YkcMonitorPro_Preq_RequestServerInfo_t *req_server = (Net_YkcMonitorPro_Preq_RequestServerInfo_t*)(s_ykc_monitor_response_buff.general_transmit_buff);
                req_server->head.sequence = s_ykc_monitor_message_serial_number[gunno]++;
                req_server->body.info_type = s_ykc_monitor_assistant_info.request_info_type;
                ykc_monitor_message_send_port(NETYKC_MONITOR_PREQ_SRESCMD_REQUEST_SERVER_INFO, s_ykc_monitor_socket_info.fd, s_ykc_monitor_response_buff.general_transmit_buff,
                        s_ykc_monitor_response_buff.length, NULL);
//                ykc_monitor_set_message_wait_response_state(gunno, NET_YKC_MONITOR_USER_PREQ_EVENT_REPORT_TSOCKET_INFO);
                ykc_monitor_response_buff_release_sem();
                rt_thread_mdelay(250);
            ...
            /***** [上报导引状态信息] *****/
            if(ykc_monitor_net_event_receive(NET_YKC_MONITOR_USER_EVENT_HANDLE_CHARGEPILE, NET_YKC_MONITOR_EVENT_TYPE_REQUEST, gunno,
                    (NET_YKC_MONITOR_EVENT_OPTION_OR |NET_YKC_MONITOR_EVENT_OPTION_CLEAR), NET_YKC_MONITOR_USER_PREQ_EVENT_REPORT_GUIDANCE_CHANGED, NULL) > 0){
#ifdef NET_YKC_MONITOR_USING_EXTEND_PROTOCOL
...
//                ykc_monitor_set_message_wait_response_state(gunno, NET_YKC_MONITOR_USER_PREQ_EVENT_REPORT_TSOCKET_INFO);
                ykc_monitor_response_buff_release_sem();
                ykc_monitor_clear_guidance_changed_sending(gunno);
                rt_thread_mdelay(250);
#endif /* NET_YKC_MONITOR_USING_EXTEND_PROTOCOL */
            }
            /***** [上报器件控制状态信息] *****/
            if(ykc_monitor_net_event_receive(NET_YKC_MONITOR_USER_EVENT_HANDLE_CHARGEPILE, NET_YKC_MONITOR_EVENT_TYPE_REQUEST, gunno,
                    (NET_YKC_MONITOR_EVENT_OPTION_OR |NET_YKC_MONITOR_EVENT_OPTION_CLEAR), NET_YKC_MONITOR_USER_PREQ_EVENT_REPORT_DEVICE_CTRL_CHANGED, NULL) > 0){
#ifdef NET_YKC_MONITOR_USING_EXTEND_PROTOCOL
                Net_YkcMonitorPro_PreqReport_SreqQuery_RealtimeInfo_t *dev_ctrl = (Net_YkcMonitorPro_PreqReport_SreqQuery_RealtimeInfo_t*)(s_ykc_monitor_response_buff.general_transmit_buff);
                dev_ctrl->head.sequence = s_ykc_monitor_message_serial_number[gunno]++;
                ykc_monitor_message_send_port(NETYKC_MONITOR_PREQ_SREQCMD_RUNNING_REALTIME_INFO, s_ykc_monitor_socket_info.fd, s_ykc_monitor_response_buff.general_transmit_buff,
                        s_ykc_monitor_response_buff.length, NULL);
//                ykc_monitor_set_message_wait_response_state(gunno, NET_YKC_MONITOR_USER_PREQ_EVENT_REPORT_TSOCKET_INFO);
                ykc_monitor_response_buff_release_sem();
...
#ifdef NET_YKC_MONITOR_USING_EXTEND_PROTOCOL
                Net_YkcMonitorPro_PreqReport_SreqQuery_RealtimeInfo_t *dev_fb = (Net_YkcMonitorPro_PreqReport_SreqQuery_RealtimeInfo_t*)(s_ykc_monitor_response_buff.general_transmit_buff);
                dev_fb->head.sequence = s_ykc_monitor_message_serial_number[gunno]++;
                ykc_monitor_message_send_port(NETYKC_MONITOR_PREQ_SREQCMD_RUNNING_REALTIME_INFO, s_ykc_monitor_socket_info.fd, s_ykc_monitor_response_buff.general_transmit_buff,
                        s_ykc_monitor_response_buff.length, NULL);
//                ykc_monitor_set_message_wait_response_state(gunno, NET_YKC_MONITOR_USER_PREQ_EVENT_REPORT_TSOCKET_INFO);
                ykc_monitor_response_buff_release_sem();
...
//                ykc_monitor_set_message_wait_response_state(gunno, NET_YKC_MONITOR_USER_PREQ_EVENT_REPORT_TSOCKET_INFO);
            ...
            /***** [上报BMS报文请求] *****/
            if(ykc_monitor_net_event_receive(NET_YKC_MONITOR_USER_EVENT_HANDLE_CHARGEPILE, NET_YKC_MONITOR_EVENT_TYPE_REQUEST, gunno,
                    (NET_YKC_MONITOR_EVENT_OPTION_OR |NET_YKC_MONITOR_EVENT_OPTION_CLEAR), NET_YKC_MONITOR_USER_PREQ_EVENT_REPORT_BMS_MESSAGE, NULL) > 0){
#ifdef NET_YKC_MONITOR_USING_EXTEND_PROTOCOL
                Net_YkcMonitorPro_PreqReport_SreqQuery_RealtimeInfo_t *bms_msg = (Net_YkcMonitorPro_PreqReport_SreqQuery_RealtimeInfo_t*)(s_ykc_monitor_response_buff.general_transmit_buff);
                bms_msg->head.sequence = s_ykc_monitor_message_serial_number[gunno]++;
                ykc_monitor_message_send_port(NETYKC_MONITOR_PREQ_SREQCMD_RUNNING_REALTIME_INFO, s_ykc_monitor_socket_info.fd, s_ykc_monitor_response_buff.general_transmit_buff,
                        s_ykc_monitor_response_buff.length, NULL);
//                ykc_monitor_set_message_wait_response_state(gunno, NET_YKC_MONITOR_USER_PREQ_EVENT_REPORT_TSOCKET_INFO);
                ykc_monitor_response_buff_release_sem();
            ...
            /***** [上报液冷故障信息请求] *****/
            if(ykc_monitor_net_event_receive(NET_YKC_MONITOR_USER_EVENT_HANDLE_CHARGEPILE, NET_YKC_MONITOR_EVENT_TYPE_REQUEST, gunno,
                    (NET_YKC_MONITOR_EVENT_OPTION_OR |NET_YKC_MONITOR_EVENT_OPTION_CLEAR), NET_YKC_MONITOR_USER_PREQ_EVENT_REPORT_LIQUID_FAULT, NULL) > 0){
#ifdef NET_YKC_MONITOR_USING_EXTEND_PROTOCOL
                Net_YkcMonitorPro_PreqReport_SreqQuery_RealtimeInfo_t *liquid_f = (Net_YkcMonitorPro_PreqReport_SreqQuery_RealtimeInfo_t*)(s_ykc_monitor_response_buff.general_transmit_buff);
                liquid_f->head.sequence = s_ykc_monitor_message_serial_number[gunno]++;
                ykc_monitor_message_send_port(NETYKC_MONITOR_PREQ_SREQCMD_RUNNING_REALTIME_INFO, s_ykc_monitor_socket_info.fd, s_ykc_monitor_response_buff.general_transmit_buff,
                        s_ykc_monitor_response_buff.length, NULL);
//                ykc_monitor_set_message_wait_response_state(gunno, NET_YKC_MONITOR_USER_PREQ_EVENT_REPORT_TSOCKET_INFO);
                ykc_monitor_response_buff_release_sem();
            ...
            /***** [上报模块分配日志信息] *****/
            if(ykc_monitor_net_event_receive(NET_YKC_MONITOR_USER_EVENT_HANDLE_CHARGEPILE, NET_YKC_MONITOR_EVENT_TYPE_REQUEST, gunno,
                    (NET_YKC_MONITOR_EVENT_OPTION_OR |NET_YKC_MONITOR_EVENT_OPTION_CLEAR), NET_YKC_MONITOR_USER_PREQ_EVENT_MODULE_ALLOCATE_LOG, NULL) > 0){
#ifdef NET_YKC_MONITOR_USING_EXTEND_PROTOCOL
                Net_YkcMonitorPro_PreqReportInfo_t *mallocate = (Net_YkcMonitorPro_PreqReportInfo_t*)(s_ykc_monitor_response_buff.general_transmit_buff);
                mallocate->head.sequence = s_ykc_monitor_message_serial_number[gunno]++;
                ykc_monitor_message_send_port(NETYKC_MONITOR_PREQCMD_REPORT_INFO, s_ykc_monitor_socket_info.fd, s_ykc_monitor_response_buff.general_transmit_buff,
                        s_ykc_monitor_response_buff.length, NULL);
//                ykc_monitor_set_message_wait_response_state(gunno, NET_YKC_MONITOR_USER_PREQ_EVENT_REPORT_TSOCKET_INFO);
                ykc_monitor_response_buff_release_sem();
            ...
            if(_event){
                ...
                if(ykc_monitor_net_event_receive(NET_YKC_MONITOR_EVENT_HANDLE_SERVER, NET_YKC_MONITOR_EVENT_TYPE_REQUEST, gunno,
                    ...
                    if(result >= 0x00){
                        ...
                        if(s_ykc_monitor_assistant_info.request_info_type == NET_YKC_MONITOR_REQ_SERVER_TIMESYNC){
                            s_ykc_monitor_assistant_info.flag.is_requesting_info = 0x00;
                        }
...
/***************************************************** [服务器监控数据请求] **********************************************************/
            ...
            if(_event){
                ...
                if(ykc_monitor_net_event_receive(NET_YKC_MONITOR_USER_EVENT_HANDLE_SERVER, NET_YKC_MONITOR_EVENT_TYPE_REQUEST, gunno,
                        (NET_YKC_MONITOR_EVENT_OPTION_OR |NET_YKC_MONITOR_EVENT_OPTION_CLEAR), NET_YKC_MONITOR_USER_SREQ_EVENT_INFOPARA_MODIFY, NULL) > 0){
                    response = ykc_monitor_get_response_buff(RT_WAITING_FOREVER);
                    result = ykc_monitor_response_padding_info_para_confirm(response->general_transmit_buff, NET_YKC_MONITOR_GENERA_RESPONSE_BUFF_LENGTH, &(response->length));
                    if(result >= 0x00){
                        s_ykc_monitor_assistant_info.flag.recved_info_modify = 0x01;
                        ykc_monitor_net_event_send(NET_YKC_MONITOR_USER_EVENT_HANDLE_CHARGEPILE, NET_YKC_MONITOR_EVENT_TYPE_REQUEST, gunno, NET_YKC_MONITOR_USER_PREQ_EVENT_INFOPARA_CONFIRM);
                    }else{
                ...
                if(ykc_monitor_net_event_receive(NET_YKC_MONITOR_USER_EVENT_HANDLE_SERVER, NET_YKC_MONITOR_EVENT_TYPE_REQUEST, gunno,
                    ...
                    if(result >= 0x00){
                        ((Net_YkcMonitorPro_Sres_InfoPara_ConfirmResult_t *)response->general_transmit_buff)->body.type = NETYKC_MONITOR_SREQCMD_MODIFY_PILE_INFO;
...
/***************************************************** [服务器监控数据响应] **********************************************************/
            ...
            if(_event){
                ...
                if(ykc_monitor_net_event_receive(NET_YKC_MONITOR_USER_EVENT_HANDLE_SERVER, NET_YKC_MONITOR_EVENT_TYPE_RESPONSE, gunno,
                    ...
                    s_ykc_monitor_assistant_info.flag.dev_info_reported = 0x01;
                }
                /***** [修改的桩信息、参数确认结响应] *****/
                if(ykc_monitor_net_event_receive(NET_YKC_MONITOR_USER_EVENT_HANDLE_SERVER, NET_YKC_MONITOR_EVENT_TYPE_RESPONSE, gunno,
                        (NET_YKC_MONITOR_EVENT_OPTION_OR |NET_YKC_MONITOR_EVENT_OPTION_CLEAR), NET_YKC_MONITOR_USER_SRES_EVENT_INFOPARA_CONFIRM_RES, NULL) > 0){
                    if(g_ykc_monitor_sres_info_para_confirm_result.body.result == 0x00){
                        if(s_ykc_monitor_assistant_info.flag.recved_info_modify == 0x01){
                            if(g_ykc_monitor_sres_info_para_confirm_result.body.type == NETYKC_MONITOR_PRESCMD_INFOPARA_CONFIRM){
                                ykc_monitor_message_pro_info_para_modify(&g_ykc_monitor_sreq_pres_info_para_modify_confirm, sizeof(g_ykc_monitor_sreq_pres_info_para_modify_confirm));
                            }
                        }
                    }
                    s_ykc_monitor_assistant_info.flag.recved_info_modify = 0x00;
        ...
        for(uint8_t gunno = 0; gunno < NET_SYSTEM_GUN_NUMBER; gunno++){
            ...
            /***** [功能开关控制请求] *****/
            if(ykc_monitor_net_event_receive(NET_YKC_MONITOR_USER_EVENT_HANDLE_SERVER, NET_YKC_MONITOR_EVENT_TYPE_REQUEST, gunno,
                    (NET_YKC_MONITOR_EVENT_OPTION_OR |NET_YKC_MONITOR_EVENT_OPTION_CLEAR), NET_YKC_MONITOR_USER_SREQ_EVENT_FUNCTION_SWITCH, NULL) > 0){
                uint8_t res = 0x00;
                result = ykc_monitor_message_pro_function_switch(&g_ykc_monitor_sreq_function_switch, sizeof(g_ykc_monitor_sreq_function_switch));
                if(result >= 0x00){
                    res = 0x01;
                }
                /** 锁模块需要异步上报 */
        ...
        for(uint8_t gunno = 0; gunno < NET_SYSTEM_GUN_NUMBER; gunno++){
            ...
            /***** [上报目标平台socket信息请求] *****/
            if(ykc_monitor_net_event_receive(NET_YKC_MONITOR_EXTERNAL_EHANDLE_CHARGEPILE, NET_YKC_MONITOR_EVENT_TYPE_REQUEST, gunno,
                    (NET_YKC_MONITOR_EVENT_OPTION_OR |NET_YKC_MONITOR_EVENT_OPTION_CLEAR), NET_YKC_MONITOR_EXTERNAL_PREQ_EVENT_TSOCKET_INFO, NULL) > 0){
#ifdef NET_YKC_MONITOR_USING_EXTEND_PROTOCOL
                response = ykc_monitor_get_response_buff(RT_WAITING_FOREVER);
                result = ykc_monitor_message_padding_tsocket_info(response->general_transmit_buff, NET_YKC_MONITOR_GENERA_RESPONSE_BUFF_LENGTH, &(response->length));
                if(result >= 0x00){
                    ykc_monitor_net_event_send(NET_YKC_MONITOR_USER_EVENT_HANDLE_CHARGEPILE, NET_YKC_MONITOR_EVENT_TYPE_REQUEST, gunno, NET_YKC_MONITOR_USER_PREQ_EVENT_REPORT_TSOCKET_INFO);
                }else{
                    ykc_monitor_response_buff_release_sem();
                }
#endif /* NET_YKC_MONITOR_USING_EXTEND_PROTOCOL */
            ...
            /***** [上报目标平台计费信息请求] *****/
            if(ykc_monitor_net_event_receive(NET_YKC_MONITOR_EXTERNAL_EHANDLE_CHARGEPILE, NET_YKC_MONITOR_EVENT_TYPE_REQUEST, gunno,
                    (NET_YKC_MONITOR_EVENT_OPTION_OR |NET_YKC_MONITOR_EVENT_OPTION_CLEAR), NET_YKC_MONITOR_EXTERNAL_PREQ_EVENT_BILLING_INFO, NULL) > 0){
#ifdef NET_YKC_MONITOR_USING_EXTEND_PROTOCOL
                response = ykc_monitor_get_response_buff(RT_WAITING_FOREVER);
                result = ykc_monitor_message_padding_billing_rule(response->general_transmit_buff, NET_YKC_MONITOR_GENERA_RESPONSE_BUFF_LENGTH, &(response->length));
                if(result >= 0x00){
                    ykc_monitor_net_event_send(NET_YKC_MONITOR_USER_EVENT_HANDLE_CHARGEPILE, NET_YKC_MONITOR_EVENT_TYPE_REQUEST, gunno, NET_YKC_MONITOR_USER_PREQ_EVENT_REPORT_BILLING_INFO);
                }else{
                    ykc_monitor_response_buff_release_sem();
                }
#endif /* NET_YKC_MONITOR_USING_EXTEND_PROTOCOL */
            ...
            /***** [上报模块故障信息请求] *****/
            if(ykc_monitor_net_event_receive(NET_YKC_MONITOR_EXTERNAL_EHANDLE_CHARGEPILE, NET_YKC_MONITOR_EVENT_TYPE_REQUEST, gunno,
                    (NET_YKC_MONITOR_EVENT_OPTION_OR |NET_YKC_MONITOR_EVENT_OPTION_CLEAR), NET_YKC_MONITOR_EXTERNAL_PREQ_EVENT_MFAULT_INFO, NULL) > 0){
#ifdef NET_YKC_MONITOR_USING_EXTEND_PROTOCOL
                response = ykc_monitor_get_response_buff(RT_WAITING_FOREVER);
                result = ykc_monitor_message_padding_module_fault_info(response->general_transmit_buff, NET_YKC_MONITOR_GENERA_RESPONSE_BUFF_LENGTH, &(response->length));
                if(result >= 0x00){
                    ykc_monitor_net_event_send(NET_YKC_MONITOR_USER_EVENT_HANDLE_CHARGEPILE, NET_YKC_MONITOR_EVENT_TYPE_REQUEST, 0x00, NET_YKC_MONITOR_USER_PREQ_EVENT_REPORT_MFAULT_INFO);
                }else{
                    ykc_monitor_response_buff_release_sem();
                }
#endif /* NET_YKC_MONITOR_USING_EXTEND_PROTOCOL */
            ...
            /***** [向服务器请求信息] *****/
            if(ykc_monitor_net_event_receive(NET_YKC_MONITOR_EXTERNAL_EHANDLE_CHARGEPILE, NET_YKC_MONITOR_EVENT_TYPE_REQUEST, gunno,
                    (NET_YKC_MONITOR_EVENT_OPTION_OR |NET_YKC_MONITOR_EVENT_OPTION_CLEAR), NET_YKC_MONITOR_EXTERNAL_PREQ_EVENT_REQUEST_SERVER, NULL) > 0){
#ifdef NET_YKC_MONITOR_USING_EXTEND_PROTOCOL
                response = ykc_monitor_get_response_buff(RT_WAITING_FOREVER);
                result = ykc_monitor_message_padding_request_server_info(response->general_transmit_buff, NET_YKC_MONITOR_GENERA_RESPONSE_BUFF_LENGTH, &(response->length));
                if(result >= 0x00){
                    ykc_monitor_net_event_send(NET_YKC_MONITOR_USER_EVENT_HANDLE_CHARGEPILE, NET_YKC_MONITOR_EVENT_TYPE_REQUEST, 0x00, NET_YKC_MONITOR_USER_PREQ_EVENT_REPORT_REQUEST_SERVER);
                }else{
                    ykc_monitor_response_buff_release_sem();
                }
#endif /* NET_YKC_MONITOR_USING_EXTEND_PROTOCOL */
            ...
            /** 导引状态信息填充 */
            if(ykc_monitor_net_event_receive(NET_YKC_MONITOR_EXTERNAL_EHANDLE_CHARGEPILE, NET_YKC_MONITOR_EVENT_TYPE_REQUEST, gunno,
                    (NET_YKC_MONITOR_EVENT_OPTION_OR |NET_YKC_MONITOR_EVENT_OPTION_CLEAR), NET_YKC_MONITOR_EXTERNAL_PREQ_EVENT_GUIDANCE_CHANGED, NULL) > 0){
                response = ykc_monitor_get_response_buff(RT_WAITING_FOREVER);
                result = ykc_monitor_guidance_changed_info_padding(gunno, response->general_transmit_buff, NET_YKC_MONITOR_GENERA_RESPONSE_BUFF_LENGTH, &(response->length));
                if(result >= 0x00){
                    ykc_monitor_net_event_send(NET_YKC_MONITOR_USER_EVENT_HANDLE_CHARGEPILE, NET_YKC_MONITOR_EVENT_TYPE_REQUEST, gunno, NET_YKC_MONITOR_USER_PREQ_EVENT_REPORT_GUIDANCE_CHANGED);
                }else{
                    ykc_monitor_clear_guidance_changed_sending(gunno);
                    ykc_monitor_response_buff_release_sem();
                }
            ...
            /** 器件控制状态信息填充 */
            if(ykc_monitor_net_event_receive(NET_YKC_MONITOR_EXTERNAL_EHANDLE_CHARGEPILE, NET_YKC_MONITOR_EVENT_TYPE_REQUEST, gunno,
                    (NET_YKC_MONITOR_EVENT_OPTION_OR |NET_YKC_MONITOR_EVENT_OPTION_CLEAR), NET_YKC_MONITOR_EXTERNAL_PREQ_EVENT_DEVICE_CTRL_CHANGED, NULL) > 0){
                response = ykc_monitor_get_response_buff(RT_WAITING_FOREVER);
                result = ykc_monitor_dev_control_changed_info_padding(gunno, response->general_transmit_buff, NET_YKC_MONITOR_GENERA_RESPONSE_BUFF_LENGTH, &(response->length));
                if(result >= 0x00){
            ...
            /***** [上报BMS报文] *****/
            if(ykc_monitor_net_event_receive(NET_YKC_MONITOR_EXTERNAL_EHANDLE_CHARGEPILE, NET_YKC_MONITOR_EVENT_TYPE_REQUEST, gunno,
                    (NET_YKC_MONITOR_EVENT_OPTION_OR |NET_YKC_MONITOR_EVENT_OPTION_CLEAR), NET_YKC_MONITOR_EXTERNAL_PREQ_EVENT_BMS_MESSAGE, NULL) > 0){
#ifdef NET_YKC_MONITOR_USING_EXTEND_PROTOCOL
                response = ykc_monitor_get_response_buff(RT_WAITING_FOREVER);
                result = ykc_monitor_message_padding_request_bms_message(gunno, response->general_transmit_buff, NET_YKC_MONITOR_GENERA_RESPONSE_BUFF_LENGTH, &(response->length));
                if(result >= 0x00){
                    ykc_monitor_net_event_send(NET_YKC_MONITOR_USER_EVENT_HANDLE_CHARGEPILE, NET_YKC_MONITOR_EVENT_TYPE_REQUEST, 0x00, NET_YKC_MONITOR_USER_PREQ_EVENT_REPORT_BMS_MESSAGE);
                }else{
                    ykc_monitor_response_buff_release_sem();
                }
#endif /* NET_YKC_MONITOR_USING_EXTEND_PROTOCOL */
            ...
            /***** [上报液冷故障信息] *****/
            if(ykc_monitor_net_event_receive(NET_YKC_MONITOR_EXTERNAL_EHANDLE_CHARGEPILE, NET_YKC_MONITOR_EVENT_TYPE_REQUEST, gunno,
                    (NET_YKC_MONITOR_EVENT_OPTION_OR |NET_YKC_MONITOR_EVENT_OPTION_CLEAR), NET_YKC_MONITOR_EXTERNAL_PREQ_EVENT_LIQUID_FAULT, NULL) > 0){
#ifdef NET_YKC_MONITOR_USING_EXTEND_PROTOCOL
                response = ykc_monitor_get_response_buff(RT_WAITING_FOREVER);
                result = ykc_monitor_message_padding_liquid_fault_info(response->general_transmit_buff, NET_YKC_MONITOR_GENERA_RESPONSE_BUFF_LENGTH, &(response->length));
                if(result >= 0x00){
                    ykc_monitor_net_event_send(NET_YKC_MONITOR_USER_EVENT_HANDLE_CHARGEPILE, NET_YKC_MONITOR_EVENT_TYPE_REQUEST, gunno, NET_YKC_MONITOR_USER_PREQ_EVENT_REPORT_LIQUID_FAULT);
                }else{
                    ykc_monitor_response_buff_release_sem();
                }
#endif /* NET_YKC_MONITOR_USING_EXTEND_PROTOCOL */
        ...
        if(ykc_monitor_net_event_receive(NET_YKC_MONITOR_EXTERNAL_EHANDLE_CHARGEPILE, NET_YKC_MONITOR_EVENT_TYPE_REQUEST, 0x00,
                (NET_YKC_MONITOR_EVENT_OPTION_OR |NET_YKC_MONITOR_EVENT_OPTION_CLEAR), NET_YKC_MONITOR_EXTERNAL_PREQ_EVENT_MODULE_ALLOCATE, NULL) > 0){
            response = ykc_monitor_get_response_buff(RT_WAITING_FOREVER);
            result = ykc_monitor_mallocate_info_padding(response->general_transmit_buff, NET_YKC_MONITOR_GENERA_RESPONSE_BUFF_LENGTH, &(response->length));
            if(result >= 0x00){
...
int32_t ykc_monitor_message_send_init(void)
{
    ...
    s_ykc_monitor_assistant_info.dev_info_tick = 0x00;
    s_ykc_monitor_assistant_info.flag.dev_info_reported = 0x00;
#endif /* NET_YKC_MONITOR_AS_MONITOR */
    ...
    for(uint8_t gunno = 0x00; gunno < NET_SYSTEM_GUN_NUMBER; gunno++){
        ...
        s_ykc_monitor_message_serial_number[gunno] = 0x00;
        s_ykc_monitor_message_send_state[gunno] = 0x00;

        for(uint8_t event = 0x00; event < NET_YKC_MONITOR_EVENT_TYPE_SIZE; event++){
            s_ykc_monitor_chargepile_event[event][gunno] = 0x00;
            s_ykc_monitor_server_event[event][gunno] = 0x00;
        }
#ifdef NET_YKC_MONITOR_AS_MONITOR
        for(uint8_t event = 0x00; event < NET_YKC_MONITOR_EVENT_TYPE_SIZE; event++){
            s_ykc_monitor_user_chargepile_event[event][gunno] = 0x00;
            s_ykc_monitor_user_server_event[event][gunno] = 0x00;
        }
        s_ykc_monitor_external_chargepile_event[gunno] = 0x00;
#endif /* NET_YKC_MONITOR_AS_MONITOR */
        memset(&s_ykc_monitor_current_transaction_number[gunno], 0x00, sizeof(s_ykc_monitor_current_transaction_number[gunno]));
#endif /* NET_DESIGNATE_REGION */
    }

#ifdef NET_DESIGNATE_REGION
    memset(g_ykc_monitor_preq_heartbeat, 0x00, sizeof(g_ykc_monitor_preq_heartbeat));
    memset(g_ykc_monitor_preq_report_realtime_data, 0x00, sizeof(g_ykc_monitor_preq_report_realtime_data));
    memset(g_ykc_monitor_preq_shake_hand, 0x00, sizeof(g_ykc_monitor_preq_shake_hand));
    memset(g_ykc_monitor_preq_parameter_config, 0x00, sizeof(g_ykc_monitor_preq_parameter_config));
    memset(g_ykc_monitor_preq_charge_finish, 0x00, sizeof(g_ykc_monitor_preq_charge_finish));
    memset(g_ykc_monitor_preq_error_message, 0x00, sizeof(g_ykc_monitor_preq_error_message));
    memset(g_ykc_monitor_preq_bms_end, 0x00, sizeof(g_ykc_monitor_preq_bms_end));
    memset(g_ykc_monitor_preq_charger_end, 0x00, sizeof(g_ykc_monitor_preq_charger_end));
    memset(g_ykc_monitor_preq_bmscommand_chargerout, 0x00, sizeof(g_ykc_monitor_preq_bmscommand_chargerout));
    memset(g_ykc_monitor_preq_bms_info, 0x00, sizeof(g_ykc_monitor_preq_bms_info));
    memset(g_ykc_monitor_preq_apply_charge_active, 0x00, sizeof(g_ykc_monitor_preq_apply_charge_active));
    memset(g_ykc_monitor_preq_transaction_records, 0x00, sizeof(g_ykc_monitor_preq_transaction_records));
    memset(g_ykc_monitor_preq_ground_lock_info, 0x00, sizeof(g_ykc_monitor_preq_ground_lock_info));
    memset(g_ykc_monitor_preq_apply_merge_charge_active, 0x00, sizeof(g_ykc_monitor_preq_apply_merge_charge_active));

    memset(&g_ykc_monitor_preq_login, 0x00, sizeof(g_ykc_monitor_preq_login));
    memset(&g_ykc_monitor_preq_billing_model_verify, 0x00, sizeof(g_ykc_monitor_preq_billing_model_verify));
    memset(&g_ykc_monitor_preq_billing_model_request, 0x00, sizeof(g_ykc_monitor_preq_billing_model_request));
    memset(&g_ykc_monitor_pres_remote_update, 0x00, sizeof(g_ykc_monitor_pres_remote_update));


    memset(&s_ykc_monitor_wait_response, 0x00, sizeof(s_ykc_monitor_wait_response));
    memset(&s_ykc_monitor_socket_info, 0x00, sizeof(s_ykc_monitor_socket_info));
    memset(&s_ykc_monitor_response_buff, 0x00, sizeof(s_ykc_monitor_response_buff));

#ifdef NET_YKC_MONITOR_AS_MONITOR
#ifdef NET_YKC_MONITOR_USING_EXTEND_PROTOCOL
    s_ykc_monitor_tplat_socket_tick = 0x00;
    s_ykc_monitor_billing_rule_tick = 0x00;
#endif /* #ifdef NET_YKC_MONITOR_USING_EXTEND_PROTOCOL */
    s_ykc_monitor_current_log_id = 0x00;
    s_ykc_monitor_log_tick = 0x00;
    s_ykc_monitor_log_step = NET_YKC_MONITOR_LOG_STEP_NULL;
    memset(&s_ykc_monitor_loghead, 0x00, sizeof(s_ykc_monitor_loghead));
    memset(&s_ykc_monitor_fswitch, 0x00, sizeof(s_ykc_monitor_fswitch));
    memset(&s_ykc_monitor_assistant_info, 0x00, sizeof(s_ykc_monitor_assistant_info));
#endif /* NET_YKC_MONITOR_AS_MONITOR */
#endif /* NET_DESIGNATE_REGION */
