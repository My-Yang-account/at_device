...
#define NET_YKC_MONITOR_QRCODE_LENGTH_DEFAULT                          0x96        /* 默认二维码长度 */
...
enum ykc_monitor_cmd{
    ...
    NETYKC_MONITOR_SREQCMD_QRCODE_CONFIG = 0x5A,                     /* 指令：服务器设置二维码 */
    NETYKC_MONITOR_PRESCMD_QRCODE_CONFIG = 0x59,                     /* 指令：桩响应设置二维码 */
...
typedef struct{
    ...
    struct{
        ...
        uint8_t card_number[NET_YKC_MONITOR_CARD_NUMBER_COUNT_MAX][NET_YKC_MONITOR_CARD_NUMBER_LENGTH_MAX];
    }body;
...
typedef struct{
    ...
    struct{
        ...
        uint8_t card_number[NET_YKC_MONITOR_CARD_NUMBER_COUNT_MAX + 0x02][NET_YKC_MONITOR_CARD_NUMBER_LENGTH_MAX];
    }body;
...
typedef struct{
    ...
    struct{
        uint8_t pile_number[NET_YKC_MONITOR_CHARGEPILE_LENGTH_DEFAULT];  /* 桩号*/
        uint8_t gunno;                           /* 枪号 */
        uint8_t qrcode[NET_YKC_MONITOR_QRCODE_LENGTH_DEFAULT];   /* 二维码 */
        uint16_t result;
    }body;
}Net_YkcMonitorPro_SReq_Qrcode_Config_t;
...
typedef struct{
    ...
    uint16_t check_sum;                          /* 校验码 */
}Net_YkcMonitorPro_PRes_Qrcode_Config_t;
