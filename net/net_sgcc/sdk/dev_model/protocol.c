...
static int evs_service_get_Maintain_handler(const char *request, char **response, int *response_len)
{
    ...
    if (callback)
    {
        ((int (*)(const evs_service_feedback_maintain_query *feedcak))callback)(&service_feedback_maintain_status);
    }
...
//    data->gunStatus = 10;
//    data->gunNo = 0x02;
//    data->eLockStatus = 11;          // 4 充电枪电子锁状态
//    data->DCK1Status = 11;           // 5 直流输出接触器K1状态
//    data->DCK2Status = 11;           // 6 直流输出接触器K2状态
//    data->DCPlusFuseStatus = 11;     // 7 DC+熔断器状态
//    data->DCMinusFuseStatus = 11;    // 8 DC-熔断器状态
//
//    data->conTemp1 = (250 + 500);            // 9 充电接口DC+温度
//    data->conTemp2 = (250 + 500);            // 10 充电接口DC-温度
//    data->dcVol = 0x00;                 // 11 输出电压
//    data->dcCur = 0x00;                 // 12 输出电流
...
static int send_event_ver_info(evs_event_ver_info *data)
{
    ...
    cJSON_AddStringToObject(root, "sdkVer", "SDK_v1.1.7");
...
static int send_event_logquery_result(evs_event_logQuery_Result *data)
{
    ...
    if (data->retType == 10)
    {
        if (data->askType == 10) //交易记录
        {
            ...
            cJSON_AddStringToObject(tradeRoot, "eleModelId", data->dataArea.tradeInfo.eleModelId);
            cJSON_AddStringToObject(tradeRoot, "serModelId", data->dataArea.tradeInfo.serModelId);

            sprintf(temp, "%d", data->dataArea.tradeInfo.sumStart);
            cJSON_AddStringToObject(tradeRoot, "sumStart", temp);
            sprintf(temp, "%d", data->dataArea.tradeInfo.sumEnd);
            cJSON_AddStringToObject(tradeRoot, "sumEnd", temp);

            cJSON_AddNumberToObject(tradeRoot, "sharpElect", data->dataArea.tradeInfo.sharpElect);
            cJSON_AddNumberToObject(tradeRoot, "peakElect", data->dataArea.tradeInfo.peakElect);
            cJSON_AddNumberToObject(tradeRoot, "flatElect", data->dataArea.tradeInfo.flatElect);
            cJSON_AddNumberToObject(tradeRoot, "valleyElect", data->dataArea.tradeInfo.valleyElect);
            ...
            cJSON_AddNumberToObject(tradeRoot, "sharpPowerCost", data->dataArea.tradeInfo.sharpPowerCost);
            cJSON_AddNumberToObject(tradeRoot, "peakPowerCost", data->dataArea.tradeInfo.peakPowerCost);
            cJSON_AddNumberToObject(tradeRoot, "flatPowerCost", data->dataArea.tradeInfo.flatPowerCost);
            cJSON_AddNumberToObject(tradeRoot, "valleyPowerCost", data->dataArea.tradeInfo.valleyPowerCost);

            cJSON_AddNumberToObject(tradeRoot, "sharpServCost", data->dataArea.tradeInfo.sharpServCost);
            cJSON_AddNumberToObject(tradeRoot, "peakServCost", data->dataArea.tradeInfo.peakServCost);
            cJSON_AddNumberToObject(tradeRoot, "flatServCost", data->dataArea.tradeInfo.flatServCost);
            cJSON_AddNumberToObject(tradeRoot, "valleyServCost", data->dataArea.tradeInfo.valleyServCost);
        ...
        else if (data->askType == 11) //电表底值
        {
            ...
            cJSON_AddItemToObject(meterRoot, "acOutMeterIty", body = cJSON_CreateObject());
            ...
            sprintf(temp, "%d", data->dataArea.meterData.sumMeter);
            cJSON_AddStringToObject(body, "sumMeter", temp);
