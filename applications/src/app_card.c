...
#if (defined(APP_USING_NO_BMS) || defined(APP_USING_LV_MODULE_BMS))
...
#endif /* (defined(APP_USING_NO_BMS) || defined(APP_USING_LV_MODULE_BMS)) */
...
static int32_t app_card_info_process(void* handle, uint8_t info_type, uint8_t *parameter, uint8_t plen)
{
    ...
    if((*(sys_read_config_item_content(CONFIG_ITEM_SUPORT_OFFLINE_CARD, 0))) == APP_THA_ENUM_FALSE){
        return APP_CARD_OPERATE_RET_NULL;
    }
    s_rfidr = (rfid_reader*)handle;
