...
#ifdef APP_INCLUDE_V2G
                    if(rtransaction.gun_running_mode == APP_GUN_RUNNING_MODE_V2G){
                        _total_elect += mw_get_meter_reserve_total_wh(count);
                    }else{
...
#ifdef APP_INCLUDE_V2G
                if(rtransaction.gun_running_mode == APP_GUN_RUNNING_MODE_V2G){
                    _total_elect = mw_get_meter_reserve_total_wh(gunno);
                }else{
...
/***************************************************************
 * 函数名          ofsm_start_info_padding_reservation
 * 功能               预约启动  信息填充
 * 参数              gunno   枪号
 * 返回
 **************************************************************/
static void ofsm_start_info_padding_reservation(uint8_t gunno)
{
    if(gunno >= APP_SYSTEM_GUNNO_SIZE){   /* 枪号不对 */
        return;
    }
    ...
    s_ofsm_info[gunno].base.flag.vin_authorization_success = APP_THA_ENUM_FALSE;
    s_ofsm_info[gunno].base.flag.vin_is_authorized = APP_THA_ENUM_FALSE;
    s_ofsm_info[gunno].base.start_type = APP_CHARGE_START_WAY_RESERVATION;
    s_ofsm_info[gunno].base.account_ballance_before = 0x00;
    s_ofsm_info[gunno].base.account_ballance_after = 0x00;
    s_ofsm_info[gunno].base.charge_strategy = APP_CHARGE_STRATEGY_FULL;
    s_ofsm_info[gunno].base.charge_strategy_para = 0x00;
        ...
        if(s_ofsm_info[gunno].base.charge_way == APP_CHARGE_WAY_PARACHARGE_LOCAL){
            if(gunno == s_ofsm_info[gunno].base.main_gunno){
                ...
                if(s_ofsm_info[gunno].base.gun_running_mode == APP_GUN_RUNNING_MODE_V2G){
                    s_ofsm_info[gunno].base.start_elect = (mw_get_meter_reserve_total_wh(gunno) + mw_get_meter_reserve_total_wh(deputy_gunno));
                }else{
...
#ifdef APP_INCLUDE_V2G
            if(s_ofsm_info[gunno].base.gun_running_mode == APP_GUN_RUNNING_MODE_V2G){
                s_ofsm_info[gunno].base.start_elect = mw_get_meter_reserve_total_wh(gunno);
            }else{
...
#ifdef APP_INCLUDE_V2G
    if(s_ofsm_info[gunno].base.gun_running_mode == APP_GUN_RUNNING_MODE_V2G){
        s_ofsm_info[gunno].base.current_elect = mw_get_meter_reserve_total_wh(gunno);
    }else{
...
static void ofsm_idleing_fun(uint8_t gunno)
{
    ...
    case CC1_0V:
        ...
        if(s_ofsm_info[gunno].base.cc1_state == CC1_6V){
...
static void ofsm_readying_fun(uint8_t gunno)
{
    ...
    if(s_ofsm_info[gunno].base.charge_way == APP_CHARGE_WAY_PARACHARGE_LOCAL){
        if((gunno != s_ofsm_info[gunno].base.main_gunno) && (s_ofsm_info[gunno].state != s_ofsm_info[s_ofsm_info[gunno].base.main_gunno].state)){
            if((s_ofsm_info[s_ofsm_info[gunno].base.main_gunno].state < APP_OFSM_STATE_SIZE) && (s_ofsm_info[s_ofsm_info[gunno].base.main_gunno].state != APP_OFSM_STATE_RESERVATION)){
...
static void ofsm_reservation_fun(uint8_t gunno)
{
    ...
    if (++s_debug_count[gunno] > (1000  + 500 *gunno) / 100) {
        s_debug_count[gunno] = 0;
        LOG_I("gunno(%d) reservation state (%dV | S%d) time[base:%d, remain:%d, %d]...", gunno, mw_get_cc1_value(mw_get_cc1(gunno)), charge_state, \
...
static void ofsm_starting_fun(uint8_t gunno)
{
...
#ifdef APP_INCLUDE_V2G
                if(s_ofsm_info[gunno].base.gun_running_mode == APP_GUN_RUNNING_MODE_V2G){
                    s_ofsm_info[gunno].base.charge_elect_last = (mw_get_meter_reserve_total_wh(gunno) + mw_get_meter_reserve_total_wh(deputy_gunno));
                }else{
...
#ifdef APP_INCLUDE_V2G
                if(s_ofsm_info[gunno].base.gun_running_mode == APP_GUN_RUNNING_MODE_V2G){
                    s_ofsm_info[gunno].base.charge_elect_last = mw_get_meter_reserve_total_wh(gunno);
                }else{
    ...
    if((system_fault != APP_SYS_FAULT_NO_ERROR) && (system_fault != APP_SYS_FAULT_CARD_READER) && (system_fault != APP_SYS_FAULT_DOOR)){
        ...
        is_stop_charge_authorization = true;
        LOG_D("gunno(%d) charge finish deal to no BMS fault stop(%d)\n", gunno, s_thaisen_transaction[gunno].stop_reason);
    }
    /** 启动失败 */
    if(is_stop_charge_authorization){
        s_ofsm_fun[gunno] = s_ofsm_fun_list[gunno][APP_OFSM_STATE_STOPING];
        s_ofsm_info[gunno].state = APP_OFSM_STATE_STOPING;
...
static void ofsm_charging_fun(uint8_t gunno)
{
    ...
    if(s_ofsm_info[gunno].base.current_check_time >= APP_CURRENT_DETECT_PERIOD){
        ...
        if(s_ofsm_info[gunno].base.gun_running_mode == APP_GUN_RUNNING_MODE_V2G){
            if(_ammeter_current < 0x00)
                _ammeter_current = 0x00 - _ammeter_current;
        }else{
            if(_ammeter_current < 0x00)
        ...
        if((_module_current_diff <= APP_CURRENT_STEADY_DIFF) && (_ammeter_current_diff <= APP_CURRENT_STEADY_DIFF)){
            ...
            if((s_ofsm_info[gunno].base.meter_curr_steady_count > APP_CURRENT_STEADY_COUNT) && (s_ofsm_info[gunno].base.module_curr_steady_count > APP_CURRENT_STEADY_COUNT)){
                ...
                if(_module_current_diff > APP_CURRENT_COMPARE_DIFF){
                    ...
                    if((s_ofsm_info[gunno].base.charge_way == APP_CHARGE_WAY_PARACHARGE_LOCAL) || (s_ofsm_info[gunno].base.charge_way == APP_CHARGE_WAY_PARACHARGE_CLOUD)){
                        if(gunno != s_ofsm_info[gunno].base.main_gunno){
                            is_deputy_abnormal = APP_THA_ENUM_TRUE;
                        }
                    }
                    /** 这是副枪异常停止 */
                    if(is_deputy_abnormal == APP_THA_ENUM_TRUE){
                        /** 副枪有故障导致整机停止 */
                        if(s_ofsm_info[gunno].base.flag.is_deputygun_stop == APP_THA_ENUM_FALSE){
                            s_thaisen_transaction[gunno].order_info.is_charging = APP_THA_ENUM_FALSE;
                            s_thaisen_transaction[gunno].order_info.waiting_charge = APP_THA_ENUM_FALSE;
                        ...
                        stop_way = APP_SYSTEM_STOP_WAY_CURRENT_ABNORMAL;
                        s_thaisen_transaction[gunno].stop_reason = stop_way;
                        s_ofsm_info[gunno].base.reason_code = stop_way;

                        LOG_D("gunno(%d) charge finish deal to stop way(%d)\n", gunno, stop_way);
    ...
    case APP_BATTERY_CHARGE_STAGE_1:
    {
        ...
        if(s_ofsm_info[gunno].base.battery_type == APP_BATTERY_TYPE_64V125AH){
            ...
            if(((rt_tick_get() - s_ofsm_info[gunno].base.parameter_steady_tick) > APP_NO_BMS_STAGE_CROSSING_CONTINUOUS_TIME_MAX) || s_ofsm_info[gunno].base.charge_time >= APP_NO_BMS_STAGE_1_CHARGE_TIME_MAX_64V125AH){
                s_ofsm_info[gunno].base.parameter_steady_tick = rt_tick_get();
            ...
            if(((rt_tick_get() - s_ofsm_info[gunno].base.parameter_steady_tick) > APP_NO_BMS_STAGE_CROSSING_CONTINUOUS_TIME_MAX) || s_ofsm_info[gunno].base.charge_time >= APP_NO_BMS_STAGE_1_CHARGE_TIME_MAX_51_2V125AH){
                s_ofsm_info[gunno].base.parameter_steady_tick = rt_tick_get();
                s_ofsm_info[gunno].base.bat_charge_stage = APP_BATTERY_CHARGE_STAGE_2;
            }
        }
    ...
    case APP_BATTERY_CHARGE_STAGE_2:
    {
        ...
        if((rt_tick_get() - s_ofsm_info[gunno].base.parameter_steady_tick) > APP_NO_BMS_FULL_CONTINUOUS_TIME_MAX){
            /** 满充 */
            thaisen_close_charge_module(gunno);
            s_thaisen_transaction[gunno].stop_reason = APP_SYSTEM_STOP_WAY_CHARGE_FULL;
            s_ofsm_info[gunno].base.reason_code = s_thaisen_transaction[gunno].stop_reason;
            s_ofsm_info[gunno].base.flag.is_fault_stop = APP_THA_ENUM_FALSE;

            is_stop_charge_authorization = true;
            LOG_D("gunno(%d) charge finish deal to no BMS charge full\n", gunno);
        }
    }
        break;
    default:
    ...
    if((s_ofsm_info[gunno].base.charge_way == APP_CHARGE_WAY_PARACHARGE_LOCAL) && (gunno == s_ofsm_info[gunno].base.main_gunno)){
        ...
        if(s_ofsm_info[gunno].elect_calculate_tick > current_tick){
            if((current_tick + 0xFFFFFFFF - s_ofsm_info[gunno].elect_calculate_tick) > 1000){  /** 一秒计算一次 */
                uint32_t _elect = 0x00;
#ifdef APP_INCLUDE_V2G
                if(s_ofsm_info[gunno].base.gun_running_mode == APP_GUN_RUNNING_MODE_V2G){
                    _elect = mw_get_meter_reserve_total_wh(gunno) + mw_get_meter_reserve_total_wh(deputy_gunno);
                }else{
                    _elect = mw_get_meter_total_wh(gunno) + mw_get_meter_total_wh(deputy_gunno);
                }
#else
                _elect = mw_get_meter_total_wh(gunno) + mw_get_meter_total_wh(deputy_gunno);
#endif /* APP_INCLUDE_V2G */
...
#ifdef APP_INCLUDE_V2G
                if(s_ofsm_info[gunno].base.gun_running_mode == APP_GUN_RUNNING_MODE_V2G){
                    _elect = mw_get_meter_reserve_total_wh(gunno);
                }else{
                    _elect = mw_get_meter_total_wh(gunno);
                }
#else
                _elect = mw_get_meter_total_wh(gunno);
#endif /* APP_INCLUDE_V2G */
...
#ifdef APP_INCLUDE_V2G
            if(s_ofsm_info[gunno].base.gun_running_mode == APP_GUN_RUNNING_MODE_V2G){
                s_ofsm_info[gunno].base.melect_last = mw_get_meter_reserve_total_wh(gunno);
            }else{
    ...
    if(s_ofsm_info[gunno].base.melect_check_time > APP_SIMULATE_ELECT_CALCULATE_PERIOD){
        ...
        case APP_MELECT_DETECT_CURR_RANGE_1:
        {
            uint32_t _meter_value = 0x00;

            s_ofsm_info[gunno].base.melect_err_count = 0x00;
#ifdef APP_INCLUDE_V2G
            if(s_ofsm_info[gunno].base.gun_running_mode == APP_GUN_RUNNING_MODE_V2G){
                _meter_value = mw_get_meter_reserve_total_wh(gunno);
            }else{
                _meter_value = mw_get_meter_total_wh(gunno);
            }
#else
            _meter_value = mw_get_meter_total_wh(gunno);
#endif /* APP_INCLUDE_V2G */
            if((_meter_value - s_ofsm_info[gunno].base.melect_last) < APP_SIMULATE_ELECT_COMPARE_VALUE_1){
        ...
        case APP_MELECT_DETECT_CURR_RANGE_2:
        {
            ...
            _meter_value = mw_get_meter_total_wh(gunno);
#endif /* APP_INCLUDE_V2G */
            if((_meter_value - s_ofsm_info[gunno].base.melect_last) < APP_SIMULATE_ELECT_COMPARE_VALUE_2){
    ...
    if(s_ofsm_info[gunno].base.flag.is_ammeter_elect_error == APP_THA_ENUM_TRUE){
        uint8_t _meter_resume = APP_THA_ENUM_FALSE;
...
#ifdef APP_INCLUDE_V2G
            if(s_ofsm_info[gunno].base.gun_running_mode == APP_GUN_RUNNING_MODE_V2G){
                if((mw_get_meter_reserve_total_wh(gunno) != 0x00) && (mw_get_meter_reserve_total_wh(deputy_gunno) != 0x00)){
                    _meter_resume = APP_THA_ENUM_TRUE;
                }
            }else{
...
#endif /* APP_INCLUDE_V2G */
            if(_meter_resume == APP_THA_ENUM_TRUE){
...
#ifdef APP_INCLUDE_V2G
            if(s_ofsm_info[gunno].base.gun_running_mode == APP_GUN_RUNNING_MODE_V2G){
                if(mw_get_meter_reserve_total_wh(gunno) != 0x00){
                    _meter_resume = APP_THA_ENUM_TRUE;
                }
            }else{
...
#endif /* APP_INCLUDE_V2G */
                if(reach_target == APP_THA_ENUM_TRUE){
    ...
    if(((s_ofsm_info[gunno].base.charge_way == APP_CHARGE_WAY_PARACHARGE_CLOUD) && (s_ofsm_info[gunno].base.main_gunno == gunno)) ||
        ...
        symbol_value = s_ofsm_info[gunno].base.current_a;
#ifdef APP_INCLUDE_V2G
        if(s_ofsm_info[gunno].base.gun_running_mode == APP_GUN_RUNNING_MODE_V2G){
...
#endif /* APP_INCLUDE_V2G */
        if(symbol_value < TINY_CURRENT_ABNOAMAL_VALUE){
...
//                s_request_screen_time_step = 2;
        ...
        case APP_OFSM_STATE_CHARGING:
        {
            ...
            symbol_value = s_ofsm_info[thread_gunno].base.current_a /10;
#ifdef APP_INCLUDE_V2G
            if(s_ofsm_info[thread_gunno].base.gun_running_mode == APP_GUN_RUNNING_MODE_V2G){
                if(symbol_value < 0x00)
                    symbol_value = 0x00 - symbol_value;
            ...
            if((*(sys_read_config_item_content(CONFIG_ITEM_INEN_TEMPPRO, 0))) != 1){
                ...
                case TCHECK_RESULT_OVERTEMP_1:
                    ...
                    if((s_gun_charging_curr[thread_gunno] == 0) && (s_chargepile_output_steady[thread_gunno] == true)){
                        int32_t _ammeter_current = (mw_get_meter_ia(thread_gunno) /1000);
#ifdef APP_INCLUDE_V2G
                        if(s_ofsm_info[thread_gunno].base.gun_running_mode == APP_GUN_RUNNING_MODE_V2G){
                            if(_ammeter_current < 0x00)
                                _ammeter_current = 0x00 - _ammeter_current;
        ...
        /** FAST 协议启用 */
        if((*(uint8_t*)(sys_read_config_item_content(CONFIG_ITEM_SUPORT_FAST_PROTOCOL, 0x00))) == CONFIG_ENABLE_ENUM){
        ...
        /** 宇通协议启用 */
        if((*(uint8_t*)(sys_read_config_item_content(CONFIG_ITEM_SUPORT_YT_PROTOCOL, 0x00))) == CONFIG_ENABLE_ENUM){
        ...
        /** 湾区协议启用 */
        if((*(uint8_t*)(sys_read_config_item_content(CONFIG_ITEM_SUPORT_BAY_PROTOCOL, 0x00))) == CONFIG_ENABLE_ENUM){
        ...
        /** BMS多帧启用 */
        if((*(uint8_t*)(sys_read_config_item_content(CONFIG_ITEM_SUPORT_BMS_SEVERAL_FRAME, 0x00))) == CONFIG_ENABLE_ENUM){
            mw_charglib_set_function_enable(thread_gunno, APP_FUNCTION_BMS_SEVERAL_FRAME, APP_THA_ENUM_TRUE);
        }else{
            mw_charglib_set_function_enable(thread_gunno, APP_FUNCTION_BMS_SEVERAL_FRAME, APP_THA_ENUM_FALSE);
        }

#ifdef APP_INCLUDE_V2G
        if(s_ofsm_info[thread_gunno].base.gun_running_mode == APP_GUN_RUNNING_MODE_V2G){
            s_ofsm_info[thread_gunno].base.ammeter_elect = mw_get_meter_reserve_total_wh(thread_gunno);
        }else{
