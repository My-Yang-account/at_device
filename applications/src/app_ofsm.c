...
static void transaction_record_query_report(uint8_t gunno)
{
    ...
    if(mw_storage_record_get_userdata_record_num(USER_DATA_TYPE_STORAGE, gunno) > 0x00){
        ...
        while(check_rentry < 3){
            ...
            if(rtransaction.stop_reason == APP_ORIGIN_USER_STOPWAY_POWEROFF){
                if(mw_get_meter_total_wh(gunno) >= rtransaction.ammeter_stop){
                    uint32_t loss_elect = 0;
                    loss_elect = mw_get_meter_total_wh(gunno) - rtransaction.ammeter_stop;
                    rtransaction.ammeter_stop = mw_get_meter_total_wh(gunno);
    ...
    if(app_nsal_query_transaction_verify_state(gunno, APP_TARGET_PLATFORM_ID) == APP_THA_ENUM_TRUE){
        rt_kprintf("0000000000000000000aaaaaaaad(%d, %d)\n", gunno, s_current_order_index[TARGET_PLATFORM_INDEX][gunno]);
    ...
    if(app_nsal_query_transaction_verify_state(gunno, APP_MONITOR_PLATFORM_ID) == APP_THA_ENUM_TRUE){
        rt_kprintf("monitor 0000000000000000000aaaaaaaad(%d, %d)\n", gunno, s_current_order_index[MONITOR_PLATFORM_INDEX][gunno]);
...
#define ADJUST_PERIOD       30000 /RUNNING_PERIOD
    ...
    uint8_t gun_idle[APP_SYSTEM_GUNNO_SIZE], gunno = 0x00, set_gunno = 0x00;
    ...
    for(gunno = 0x00; gunno < APP_SYSTEM_GUNNO_SIZE; gunno++){
        ...
        if(app_nsal_is_set_power(gunno)){
            app_nsal_clear_set_power(gunno);
            if(app_nsal_get_setup_power() > s_ofsm_info[gunno].base.system_power_max){
                server_adjust_power = false;
                app_nsal_report_set_power_result(gunno, 0x00, 0x00);
                LOG_E("power adjust fail deal to power of range|%d, %d\n", app_nsal_get_setup_power(), s_ofsm_info[gunno].base.system_power_max);
            }else{
                set_gunno = gunno;
                server_adjust_power = true;
                s_issue_power_adjust = true;
                s_system_power_output = app_nsal_get_setup_power() *10;
                LOG_I("gunno(%d) server adjust system power|%d, %d", gunno, app_nsal_get_setup_power(), s_ofsm_info[gunno].base.system_power_max);
            }
        }
    }
    ...
    if((++s_power_adjust_delay > ADJUST_PERIOD) || server_adjust_power || s_power_on == 0 ){
        ...
        if(server_adjust_power){
            if(set_gunno < APP_SYSTEM_GUNNO_SIZE){
                uint32_t issue_power = app_nsal_get_setup_power();
...
static void ofsm_readying_fun(uint8_t gunno)
{
    ...
    case CC1_4V:
        if(app_nsal_is_remote_start(gunno)){
            ...
            clear_swipe_card_state(gunno);
            if(thaisen_get_hci_page_pos() == gunno){
        ...
        if(is_charging_authorization == true){
            ...
            s_thaisen_transaction[gunno].stop_reason = mw_system_stop_way_convert(APP_SYSTEM_STOP_WAY_POWER_OFF);
    ...
    if(s_ofsm_info[gunno].base.state.current != APP_OFSM_STATE_READYING){
...
static void ofsm_starting_fun(uint8_t gunno)
{
    ...
    case CC1_6V:
    {
        ...
        if(stop_way != APP_SYSTEM_STOP_WAY_PASSIVE){
            ...
            s_thaisen_transaction[gunno].stop_reason = mw_system_stop_way_convert(APP_SYSTEM_STOP_WAY_PULL_GUN);
    ...
    case APP_CHARGE_STATE_CHARGING:
    {
        ...
        if(s_ofsm_info[gunno].base.flag.vin_authorization_success == APP_THA_ENUM_FALSE){
            ...
            s_thaisen_transaction[gunno].stop_reason = mw_system_stop_way_convert(APP_SYSTEM_STOP_WAY_AUTHEN_FAIL);
            s_ofsm_info[gunno].base.reason_code = s_thaisen_transaction[gunno].stop_reason;
...
static void ofsm_charging_fun(uint8_t gunno)
{
    ...
    if(app_nsal_is_remote_stop(gunno)) {
        s_thaisen_transaction[gunno].stop_reason = mw_system_stop_way_convert(APP_SYSTEM_STOP_WAY_APP_STOP);
        s_ofsm_info[gunno].base.reason_code = s_thaisen_transaction[gunno].stop_reason;
        s_ofsm_info[gunno].base.flag.is_fault_stop = APP_THA_ENUM_FALSE;
        ...
        if((thaisen_get_hci_page_pos() == gunno) &&
            ...
            if(get_card_info_type() == CARD_INFO_TYPE_CARD_NUMBER){
                ...
                if(s_ofsm_info[gunno].base.start_type == APP_CHARGE_START_WAY_ONLINE_CARD){
                    s_thaisen_transaction[gunno].stop_reason = mw_system_stop_way_convert(APP_SYSTEM_STOP_WAY_ONLINECARD_STOP);
                    s_ofsm_info[gunno].base.reason_code = s_thaisen_transaction[gunno].stop_reason;
                    LOG_D("gunno(%d) charge finish deal to online card stop\n", gunno);
                }else{
                    s_thaisen_transaction[gunno].stop_reason = mw_system_stop_way_convert(APP_SYSTEM_STOP_WAY_OFFLINECARD_STOP);
                    s_ofsm_info[gunno].base.reason_code = s_thaisen_transaction[gunno].stop_reason;
        ...
        s_thaisen_transaction[gunno].stop_reason = mw_system_stop_way_convert(APP_SYSTEM_STOP_WAY_SCREEN_STOP);
        s_ofsm_info[gunno].base.reason_code = s_thaisen_transaction[gunno].stop_reason;
        s_ofsm_info[gunno].base.flag.is_fault_stop = APP_THA_ENUM_FALSE;
        ...
        if(s_ofsm_info[gunno].base.charge_strategy == APP_CHARGE_STRATEGY_TIME){
            if((s_ofsm_info[gunno].base.current_time - s_ofsm_info[gunno].base.start_time) >= s_ofsm_info[gunno].base.charge_strategy_para){
                ...
                is_stop_charge_authorization = true;
                LOG_D("gunno(%d) charge finish deal to reach target time(%d)\n", gunno, s_ofsm_info[gunno].base.charge_strategy_para);
            }
            ...
            if(s_ofsm_info[gunno].base.elect_a >= s_ofsm_info[gunno].base.charge_strategy_para){
                s_thaisen_transaction[gunno].stop_reason = mw_system_stop_way_convert(APP_SYSTEM_STOP_WAY_REACH_ELECT);
                s_ofsm_info[gunno].base.reason_code = s_thaisen_transaction[gunno].stop_reason;
                s_ofsm_info[gunno].base.flag.is_fault_stop = APP_THA_ENUM_FALSE;
            ...
            if(s_ofsm_info[gunno].base.fees_total >= s_ofsm_info[gunno].base.charge_strategy_para){
                ...
                is_stop_charge_authorization = true;
                LOG_D("gunno(%d) charge finish deal to reach target money\n", gunno, s_ofsm_info[gunno].base.charge_strategy_para);
            }
            ...
            if((bms_info->BCS.SOC >= s_ofsm_info[gunno].base.charge_strategy_para) &&
                ...
                s_thaisen_transaction[gunno].stop_reason = mw_system_stop_way_convert(APP_SYSTEM_STOP_WAY_SOC_LIMIT);
                s_ofsm_info[gunno].base.reason_code = s_thaisen_transaction[gunno].stop_reason;
                s_ofsm_info[gunno].base.flag.is_fault_stop = APP_THA_ENUM_FALSE;

                is_stop_charge_authorization = true;
                LOG_D("gunno(%d) charge finish deal to reach target SOC\n", gunno, s_ofsm_info[gunno].base.charge_strategy_para);
            }
        }
    }
    ...
    if(s_ofsm_info[gunno].base.current_a < TINY_CURRENT_ABNOAMAL_VALUE){
        ...
        if((rt_tick_get() - s_tiny_current_count[gunno]) > TINY_CURRENT_ABNOAMAL_TIMEOUT){
            ...
            if(is_stop_charge_authorization == false){
                s_thaisen_transaction[gunno].stop_reason = mw_system_stop_way_convert(APP_SYSTEM_STOP_WAY_CURRENT_ABNORMAL);
                s_ofsm_info[gunno].base.reason_code = s_thaisen_transaction[gunno].stop_reason;
                s_ofsm_info[gunno].base.flag.is_fault_stop = APP_THA_ENUM_TRUE;
    ...
    if(s_ofsm_info[gunno].base.state.current != APP_OFSM_STATE_CHARGING){
...
static void ofsm_finishing_fun(uint8_t gunno)
{
    ...
    if(s_ofsm_info[gunno].base.state.current != APP_OFSM_STATE_FINISHING){
...
static void ofsm_faulting_fun(uint8_t gunno)
{
    ...
    if(system_fault == APP_SYS_FAULT_NO_ERROR){
        ...
        if(s_ofsm_info[gunno].base.state.current != APP_OFSM_STATE_FAULTING){
