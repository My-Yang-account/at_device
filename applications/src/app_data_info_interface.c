...
struct fault_info *thaisen_app_get_fault_info(uint8_t gunno)
{
    ...
    s_fault_info.charge_fault = mw_system_fault_convert(app_get_highest_priority_charge_fault(gunno), APP_FCONVERT_LIB_TO_APP);
    s_fault_info.system_fault = mw_system_fault_convert(app_get_highest_priority_system_fault(gunno), APP_FCONVERT_LIB_TO_APP);
...
{
    app_cmd_debug_result_info(gunno, language, cmd, para, plen, buf, ilen);
}
