...
#define SCREEN_AUX_A_SELCT_REG_NEW              0x1007          /* 新屏幕A枪辅源选择控件地址 */
#define SCREEN_AUX_A_SELCT_KEY_NEW              0x000D          /* 新屏幕A枪辅源选择控件键值 */
#define SCREEN_AUX_B_SELCT_REG_NEW              0x1007          /* 新屏幕B枪辅源选择控件地址 */
#define SCREEN_AUX_B_SELCT_KEY_NEW              0x000E          /* 新屏幕B枪辅源选择控件键值 */

#define SCREEN_AUX_A_SELCT_REG_OLD              0x1007          /* 老屏幕A枪辅源选择控件地址 */
#define SCREEN_AUX_A_SELCT_KEY_OLD              0x000D          /* 老屏幕A枪辅源选择控件键值 */
...
#else /* SCREEN_USING_OFFLINE_BILLING */
#ifdef SCREEN_USING_V2G
#define SCREEN_OFFLINE_BILLING_PAGE_NUM   1   /** 离线计费页面总数 */
#else
...
enum LCD_DISPLAY_PAGE_TYPE{
	...
	LCD_PAGE_CMD_DEBUG  = 16,	    //指令调试
	LCD_PAGE_OTHER_CONFIG  = 17,	//其它配置
    ...
    LCD_PAGE_DISCHARGE_MODE = 25,   //放电模式
	...
	LCD_PAGE_MENU_MODE_SELECT = 42, //模式选择信息
...
//	u8 ChargePasswd[6];					//充电密码
    ...
    u8 sup_SupProtocolGB_T;                 //国标协议(27930)支持
    u8 sup_SupSeveralFrame;                 //BMS多帧支持
#ifdef SCREEN_USING_V2G
    u8 sup_V2G;                             //V2G支持
#endif /* SCREEN_USING_V2G */
	u8 fan_type;							//风扇类型
    ...
    u8 Icon_SupModeSelect;                  //模式选择支持icon
    ...
    u8 CurrentMode[LCD_GUN_NUM + 1][THAISEN_CHARGE_MODE_SIZE];          //模式选择：当前模式
#ifdef SCREEN_USING_V2G
    u32 V2G_MSLimitMoney[LCD_GUN_NUM + 1];                       //V2G模式选择：限制金额(单位：0.01元)
    u32 V2G_MSLimitElect[LCD_GUN_NUM + 1];                       //V2G模式选择：限制电量(单位：0.001度)
    u32 V2G_MSLimitTiming[LCD_GUN_NUM + 1];                      //V2G模式选择：定时(单位：1min)
    u8 V2G_CurrentMode[LCD_GUN_NUM + 1][THAISEN_V2G_MODE_SIZE + 1];  //V2G模式选择：当前模式
    u32 V2G_CurrentModePara[LCD_GUN_NUM + 1];                    //V2G当前模式参数
    u8 DisChargeWay[LCD_GUN_NUM + 1];                            //放电方式
#endif /* SCREEN_USING_V2G */
    u32 CurrentModePara[LCD_GUN_NUM + 1];                        //当前模式参数：
    u8 CurrentModeInfo[LCD_GUN_NUM][32];                         //当前模式显示信息：
    u8 Modewarning;                                              //模式信息告警
    /*********************** cmd debug ***************************/
    uint8_t DebugCmd[32];                                        //调试指令
    uint8_t DebugCmdPara[32];                                    //调试指令参数
...
#ifdef SCREEN_USING_V2G
enum DISCHARGE_WAY{
    SS_DISCHARGE_WAY_NULL = 0,               /** 放电方式：空 */
    SS_DISCHARGE_WAY_ON_CONTRAVARIANT = 1,   /** 放电方式：并网 */
    SS_DISCHARGE_WAY_OFF_CONTRAVARIANT = 2,  /** 放电方式：离网 */
    SS_DISCHARGE_WAY_SIZE,                   /** 放电方式 */
};
#endif /* SCREEN_USING_V2G */
...
enum thaisen_charge_mode SerialScreen_Screen_GetCurrentChargeMode(u8 port)
{
    if(LcdData.setData.sup_mode_select == TRUE){
        if(port >= LCD_GUN_NUM)
            return THAISEN_CHARGE_MODE_FULL;
#ifdef SCREEN_USING_V2G
        if(LcdData.runData.GunRunMode[port] != THAISEN_GUN_RUNING_MODE_CHARGE)
            return THAISEN_CHARGE_MODE_FULL;
#endif /* SCREEN_USING_V2G */
        if(LcdData.setData.CurrentMode[port][THAISEN_CHARGE_MODE_LIMIT_MONEY]){
            return THAISEN_CHARGE_MODE_LIMIT_MONEY;
        }else if(LcdData.setData.CurrentMode[port][THAISEN_CHARGE_MODE_LIMIT_ELECT]){
            return THAISEN_CHARGE_MODE_LIMIT_ELECT;
        }else if(LcdData.setData.CurrentMode[port][THAISEN_CHARGE_MODE_LIMIT_TIMING]){
            return THAISEN_CHARGE_MODE_LIMIT_TIMING;
        }else if(LcdData.setData.CurrentMode[port][THAISEN_CHARGE_MODE_LIMIT_RESERVATION]){
            return THAISEN_CHARGE_MODE_LIMIT_RESERVATION;
        }else{
            return THAISEN_CHARGE_MODE_FULL;
        }
    }
    return   THAISEN_CHARGE_MODE_SIZE;
}
...
#ifdef SCREEN_USING_V2G
enum thaisen_v2g_mode SerialScreen_Screen_GetCurrentV2GMode(u8 port)
{
    if((LcdData.setData.sup_mode_select == TRUE) && (LcdData.setData.sup_V2G == TRUE)){
        if(port >= LCD_GUN_NUM)
            return THAISEN_V2G_MODE_SIZE;
        if(LcdData.runData.GunRunMode[port] != THAISEN_GUN_RUNING_MODE_V2G)
            return THAISEN_V2G_MODE_SIZE;

        if(LcdData.setData.V2G_CurrentMode[port][THAISEN_V2G_MODE_LIMIT_MONEY]){
            return THAISEN_V2G_MODE_LIMIT_MONEY;
        }else if(LcdData.setData.V2G_CurrentMode[port][THAISEN_V2G_MODE_LIMIT_ELECT]){
            return THAISEN_V2G_MODE_LIMIT_ELECT;
        }else if(LcdData.setData.V2G_CurrentMode[port][THAISEN_V2G_MODE_LIMIT_TIMING]){
            return THAISEN_V2G_MODE_LIMIT_TIMING;
        }else if(LcdData.setData.V2G_CurrentMode[port][THAISEN_V2G_MODE_AUTO]){
            return THAISEN_V2G_MODE_AUTO;
        }else{
            return THAISEN_V2G_MODE_SIZE;
        }
    }
...
u32 SerialScreen_Screen_GetV2GParameter(u8 port)
{
    if((LcdData.setData.sup_mode_select == TRUE) && (LcdData.setData.sup_V2G == TRUE)){
        if(port >= LCD_GUN_NUM)
            return 0;
...
#endif /* SCREEN_USING_V2G */
    {
        mode = SerialScreen_Screen_GetCurrentChargeMode(port);
        switch(mode){
        case THAISEN_CHARGE_MODE_FULL:
            return THAISEN_MODE_CHARGE_FULL;
        case THAISEN_CHARGE_MODE_LIMIT_MONEY:
            return THAISEN_MODE_CHARGE_LIMIT_MONEY;
        case THAISEN_CHARGE_MODE_LIMIT_ELECT:
            return THAISEN_MODE_CHARGE_LIMIT_ELECT;
        case THAISEN_CHARGE_MODE_LIMIT_TIMING:
            return THAISEN_MODE_CHARGE_LIMIT_TIMING;
        case THAISEN_CHARGE_MODE_LIMIT_RESERVATION:
            return THAISEN_MODE_CHARGE_LIMIT_RESERVATION;
        default:
            break;
        }
    }
#ifdef SCREEN_USING_V2G
    else if(LcdData.runData.GunRunMode[port] == THAISEN_GUN_RUNING_MODE_V2G){
        mode = SerialScreen_Screen_GetCurrentV2GMode(port);
        switch(mode){
        case THAISEN_V2G_MODE_LIMIT_MONEY:
            return THAISEN_MODE_V2G_LIMIT_MONEY;
        case THAISEN_V2G_MODE_LIMIT_ELECT:
            return THAISEN_MODE_V2G_LIMIT_ELECT;
        case THAISEN_V2G_MODE_LIMIT_TIMING:
            return THAISEN_MODE_V2G_LIMIT_TIMING;
        case THAISEN_V2G_MODE_AUTO:
            return THAISEN_MODE_V2G_AUTO;
        default:
            break;
        }
    }
...
static void SerialScreen_Screen_ResetModeInfoDef(u8 port)
{
    if(port >= LCD_GUN_NUM)
        return ;
    if((LcdData.setData.CurrentMode[port][THAISEN_CHARGE_MODE_FULL] != TRUE) && (LcdData.setData.CurrentMode[port][THAISEN_CHARGE_MODE_LIMIT_RESERVATION] != TRUE)){
        u8 mode = 0;
        u32 parameter = 0;
        ...
        LcdData.setData.CurrentMode[port][THAISEN_CHARGE_MODE_FULL] = TRUE;

        mode = THAISEN_CHARGE_MODE_FULL;
...
#ifdef SCREEN_USING_V2G
    if(port >= LCD_GUN_NUM)
        return THAISEN_GUN_DISCHARGE_WAY_NULL;
    if(LcdData.setData.sup_V2G != TRUE)
        return THAISEN_GUN_DISCHARGE_WAY_NULL;
    return LcdData.setData.DisChargeWay[port];
#else
    return 0;
#endif /* SCREEN_USING_V2G */
}
...
//    if(LcdData.setData.V2G_CurrentMode[port][THAISEN_V2G_MODE_LIMIT_RESERVATION] != TRUE)
    {
        u8 mode = CP_V2G_MODE_NULL;
        u32 parameter = 0;
        /** 复位V2G相关信息 */
        LcdData.setData.V2G_MSLimitMoney[port] = 0;
        LcdData.setData.V2G_MSLimitElect[port] = 0;
        LcdData.setData.V2G_MSLimitTiming[port] = 0;
    //    LcdData.setData.V2G_MSLimitReservationHour[port] = 0;
    //    LcdData.setData.V2G_MSLimitReservationMin[port] = 0;
        LcdData.setData.V2G_CurrentModePara[port] = 0;
        memset(LcdData.setData.V2G_CurrentMode[port], 0, sizeof(LcdData.setData.V2G_CurrentMode[port]));
        LcdData.setData.V2G_CurrentMode[port][THAISEN_V2G_MODE_SIZE] = TRUE;

        parameter = LcdData.setData.V2G_CurrentModePara[port];
        if(port == LCD_GUN_1){
            UI_SYNC_SINGLE_CFG_DATA(CONFIG_ITEM_CURRENT_V2G_MODE_A, &mode, sizeof(mode));
            UI_SYNC_SINGLE_CFG_DATA(CONFIG_ITEM_V2G_MODE_PARAMETER_A, (u8*)&parameter, sizeof(parameter));
        }else{
            UI_SYNC_SINGLE_CFG_DATA(CONFIG_ITEM_CURRENT_V2G_MODE_B, &mode, sizeof(mode));
            UI_SYNC_SINGLE_CFG_DATA(CONFIG_ITEM_V2G_MODE_PARAMETER_B, (u8*)&parameter, sizeof(parameter));
        }
    }
    LcdData.setData.DisChargeWay[port] = SS_DISCHARGE_WAY_OFF_CONTRAVARIANT;
    LcdData.setData.DisChargeWay[LCD_GUN_NUM] = SS_DISCHARGE_WAY_OFF_CONTRAVARIANT;
#endif /* SCREEN_USING_V2G */
}
...
#define SSCREEN_NORMAL_MODE_FORBID_STATE     2         /* 模式选择-正常模式 禁止修改状态错误码*/
    ...
    case APP_OFSM_STATE_RESERVATION:
        /* 弹出提示：预约中不可修改策略 */
        if((LcdData.setData.CurrentMode[LCD_GUN_NUM][THAISEN_CHARGE_MODE_LIMIT_RESERVATION] == TRUE) &&  (config->mode == THAISEN_MODE_LIMIT_RESERVATION)){
            if(config->mode_parameter != (LcdData.setData.MSLimitReservationHour[LCD_GUN_NUM] *3600 + LcdData.setData.MSLimitReservationMin[LCD_GUN_NUM] *60)){
                return (SSCREEN_NORMAL_MODE_FORBID_STATE + THAISEN_CONFIG_FAIL_OFFSET);
            }
        }
        break;
    default:
    ...
    if(SerialScreen_Screen_GetCurrentV2GMode(port) == THAISEN_V2G_MODE_LIMIT_RESERVATION){
        return (SSCREEN_NORMAL_MODE_INVALID_PARA + SSCREEN_NORMAL_MODE_FORBID_STATE);
    }
#endif
#endif /* SCREEN_USING_V2G */
    switch(config->mode){
...
static s32 SerialScreen_ConfigExecute_FixedCmdDebug(u8 port, void *data, void *sub_data, void *sub_sub_data)  //OK
{
    ...
    LcdData.setData.Icon_SupProtocolGB_T = config->info.protocol_gb_t;
    LcdData.setData.Icon_SupSeveralFrame = config->info.bms_several_frame;
...
{
    u32 ret = 0;
    u8 DebugCmdParaLen = sizeof(LcdData.setData.DebugCmdPara), needIssue = 0, modify_cc1[LCD_GUN_NUM];
    ...
    for(u8 i = 0; i < config->cmd_num; i++){
        ...
        else if((memcmp(config_segment[i].cmd, "CC12_U1", strlen("CC12_U1")) == 0) && (strlen((char*)config_segment[i].cmd) == strlen("CC12_U1"))){
            if(strlen((char*)config_segment[i].parameter) == 0x00){
                /** 参数非法，配置失败 */
                return (i + THAISEN_CONFIG_FAIL_OFFSET);
            }
            modify_cc1[LCD_GUN_1] = 1;
            needIssue = 1;
            memcpy(LcdData.setData.DebugCmd, "CC12_U1", strlen("CC12_U1"));
        }
        /** 枪1 CC1 12V 下限(0.001V  L1 as lower limit gun1) */
        else if((memcmp(config_segment[i].cmd, "CC12_L1", strlen("CC12_L1")) == 0) && (strlen((char*)config_segment[i].cmd) == strlen("CC12_L1"))){
            if(strlen((char*)config_segment[i].parameter) == 0x00){
                /** 参数非法，配置失败 */
                return (i + THAISEN_CONFIG_FAIL_OFFSET);
            }
            modify_cc1[LCD_GUN_1] = 1;
            needIssue = 1;
            memcpy(LcdData.setData.DebugCmd, "CC12_L1", strlen("CC12_L1"));
        }
        /** 枪1 CC1 6V 上限(0.001V) */
        else if((memcmp(config_segment[i].cmd, "CC6_U1", strlen("CC6_U1")) == 0) && (strlen((char*)config_segment[i].cmd) == strlen("CC6_U1"))){
            if(strlen((char*)config_segment[i].parameter) == 0x00){
                /** 参数非法，配置失败 */
                return (i + THAISEN_CONFIG_FAIL_OFFSET);
            }
            modify_cc1[LCD_GUN_1] = 1;
            needIssue = 1;
            memcpy(LcdData.setData.DebugCmd, "CC6_U1", strlen("CC6_U1"));
        }
        /** 枪1 CC1 6V 下限(0.001V) */
        else if((memcmp(config_segment[i].cmd, "CC6_L1", strlen("CC6_L1")) == 0) && (strlen((char*)config_segment[i].cmd) == strlen("CC6_L1"))){
            if(strlen((char*)config_segment[i].parameter) == 0x00){
                /** 参数非法，配置失败 */
                return (i + THAISEN_CONFIG_FAIL_OFFSET);
            }
            modify_cc1[LCD_GUN_1] = 1;
            needIssue = 1;
            memcpy(LcdData.setData.DebugCmd, "CC6_L1", strlen("CC6_L1"));
        }
        /** 枪1 CC1 4V 上限(0.001V) */
        else if((memcmp(config_segment[i].cmd, "CC4_U1", strlen("CC4_U1")) == 0) && (strlen((char*)config_segment[i].cmd) == strlen("CC4_U1"))){
            if(strlen((char*)config_segment[i].parameter) == 0x00){
                /** 参数非法，配置失败 */
                return (i + THAISEN_CONFIG_FAIL_OFFSET);
            }
            modify_cc1[LCD_GUN_1] = 1;
            needIssue = 1;
            memcpy(LcdData.setData.DebugCmd, "CC4_U1", strlen("CC4_U1"));
        }
        /** 枪1 CC1 4V 下限(0.001V) */
        else if((memcmp(config_segment[i].cmd, "CC4_L1", strlen("CC4_L1")) == 0) && (strlen((char*)config_segment[i].cmd) == strlen("CC4_L1"))){
            if(strlen((char*)config_segment[i].parameter) == 0x00){
                /** 参数非法，配置失败 */
                return (i + THAISEN_CONFIG_FAIL_OFFSET);
            }
            modify_cc1[LCD_GUN_1] = 1;
            needIssue = 1;
            memcpy(LcdData.setData.DebugCmd, "CC4_L1", strlen("CC4_L1"));
        }

        /** 枪2 CC1 12V 上限(0.001V  U2 as uplimit gun2) */
        else if((memcmp(config_segment[i].cmd, "CC12_U2", strlen("CC12_U2")) == 0) && (strlen((char*)config_segment[i].cmd) == strlen("CC12_U2"))){
            if(strlen((char*)config_segment[i].parameter) == 0x00){
                /** 参数非法，配置失败 */
                return (i + THAISEN_CONFIG_FAIL_OFFSET);
            }
            modify_cc1[LCD_GUN_2] = 1;
            needIssue = 1;
            memcpy(LcdData.setData.DebugCmd, "CC12_U2", strlen("CC12_U2"));
        }
        /** 枪2 CC1 12V 下限(0.001V  L2 as lower limit gun2) */
        else if((memcmp(config_segment[i].cmd, "CC12_L2", strlen("CC12_L2")) == 0) && (strlen((char*)config_segment[i].cmd) == strlen("CC12_L2"))){
            if(strlen((char*)config_segment[i].parameter) == 0x00){
                /** 参数非法，配置失败 */
                return (i + THAISEN_CONFIG_FAIL_OFFSET);
            }
            modify_cc1[LCD_GUN_2] = 1;
            needIssue = 1;
            memcpy(LcdData.setData.DebugCmd, "CC12_L2", strlen("CC12_L2"));
        }
        /** 枪2 CC1 6V 上限(0.001V) */
        else if((memcmp(config_segment[i].cmd, "CC6_U2", strlen("CC6_U2")) == 0) && (strlen((char*)config_segment[i].cmd) == strlen("CC6_U2"))){
            if(strlen((char*)config_segment[i].parameter) == 0x00){
                /** 参数非法，配置失败 */
                return (i + THAISEN_CONFIG_FAIL_OFFSET);
            }
            modify_cc1[LCD_GUN_2] = 1;
            needIssue = 1;
            memcpy(LcdData.setData.DebugCmd, "CC6_U2", strlen("CC6_U2"));
        }
        /** 枪2 CC1 6V 下限(0.001V) */
        else if((memcmp(config_segment[i].cmd, "CC6_L2", strlen("CC6_L2")) == 0) && (strlen((char*)config_segment[i].cmd) == strlen("CC6_L2"))){
            if(strlen((char*)config_segment[i].parameter) == 0x00){
                /** 参数非法，配置失败 */
                return (i + THAISEN_CONFIG_FAIL_OFFSET);
            }
            modify_cc1[LCD_GUN_2] = 1;
            needIssue = 1;
            memcpy(LcdData.setData.DebugCmd, "CC6_L2", strlen("CC6_L2"));
        }
        /** 枪2 CC1 4V 上限(0.001V) */
        else if((memcmp(config_segment[i].cmd, "CC4_U2", strlen("CC4_U2")) == 0) && (strlen((char*)config_segment[i].cmd) == strlen("CC4_U2"))){
            if(strlen((char*)config_segment[i].parameter) == 0x00){
                /** 参数非法，配置失败 */
                return (i + THAISEN_CONFIG_FAIL_OFFSET);
            }
            modify_cc1[LCD_GUN_2] = 1;
            needIssue = 1;
            memcpy(LcdData.setData.DebugCmd, "CC4_U2", strlen("CC4_U2"));
        }
        /** 枪2 CC1 4V 下限(0.001V) */
        else if((memcmp(config_segment[i].cmd, "CC4_L2", strlen("CC4_L2")) == 0) && (strlen((char*)config_segment[i].cmd) == strlen("CC4_L2"))){
            if(strlen((char*)config_segment[i].parameter) == 0x00){
                /** 参数非法，配置失败 */
                return (i + THAISEN_CONFIG_FAIL_OFFSET);
            }
    ...
    for(u8 i = 0; i < LCD_GUN_NUM; i++){
        if(modify_cc1[i]){
            /** CC1 范围设置无效 */
            if(sys_cc1_range_valid(LcdData.setData.DebugCmdPara_CC12V_MaxTemp[i], LcdData.setData.DebugCmdPara_CC12V_MinTemp[i], \
                    LcdData.setData.DebugCmdPara_CC6V_MaxTemp[i], LcdData.setData.DebugCmdPara_CC6V_MinTemp[i], \
                    LcdData.setData.DebugCmdPara_CC4V_MaxTemp[i], LcdData.setData.DebugCmdPara_CC4V_MinTemp[i]) == 0){
                /** CC1 数据无效，配置失败 */
                return (i + THAISEN_CONFIG_FAIL_OFFSET);
            }
        }
...
static s32 SerialScreen_ConfigExecute_DynamicCmdDebugRead(u8 port, void *data, void *sub_data, void *sub_sub_data)  //OK
{
    ...
    for(u8 i = 0; i < config->cmd_num; i++){
        ...
        /** 枪1 CC1 12V 上限(0.001V  U1 as uplimit gun1) */
            ...
            sprintf((char*)config_segment[i].parameter, "%d.%d%d%d%c", (parameter /1000), ((parameter /100) %10), ((parameter /10) %10), (parameter %10), 'V');
            config_segment[i].parameter_len = strlen((char*)config_segment[i].parameter);
        }
        /** 枪1 CC1 12V 下限(0.001V  L1 as lower limit gun1) */
...
static void SerialScreen_RealTime_InfoGet(void)
{
    ...
    if(++LcdAssistantData.RefrenshPeriod > 1000 /100){   //线程运行时基10ms， 每1s更新一次数据
        ...
        if(LcdData.CurrentPage != LCD_PAGE_ADMIN_PASWD){
            ...
            compare_len = sizeof(LcdData.setData.UserPasswd);
...
{
    if(LcdData.setData.Icon_SupModeSelect != TRUE)
        LcdData.setData.Icon_SupModeSelect = TRUE;
    else
        LcdData.setData.Icon_SupModeSelect = FALSE;
}

#ifdef SCREEN_USING_V2G
void SerialScreen_IsSupportV2GSet(void)
{
    if(LcdData.setData.Icon_SupV2G != TRUE)
...
{
    if(LcdData.setData.Icon_SupProtocolGB_T != TRUE)
        LcdData.setData.Icon_SupProtocolGB_T = TRUE;
    else
        LcdData.setData.Icon_SupProtocolGB_T = FALSE;
}

void SerialScreen_IsSupportBMSSeveralFrameSet(void)
{
    if(LcdData.setData.Icon_SupSeveralFrame != TRUE)
        LcdData.setData.Icon_SupSeveralFrame = TRUE;
    else
        LcdData.setData.Icon_SupSeveralFrame = FALSE;
}
...
#define SSCREEN_PROTOCOL_GB_T_POSITION                             5   /* 指令调试控制信息配置失败原因：国标协议(27930)协议配置  */
#define SSCREEN_BMS_SEVERAL_FRAME_POSITION                         6   /* 指令调试控制信息配置失败原因：BMS多帧配置  */
    ...
    if(LcdData.setData.Icon_SupProtocolGB_T > TRUE){
        LcdData.setData.Icon_SupProtocolGB_T = FALSE;
        result |= (1 <<SSCREEN_PROTOCOL_GB_T_POSITION);
    }
    if(LcdData.setData.Icon_SupSeveralFrame > TRUE){
        LcdData.setData.Icon_SupSeveralFrame = TRUE;
        result |= (1 <<SSCREEN_BMS_SEVERAL_FRAME_POSITION);
    }
...
#undef SSCREEN_PROTOCOL_GB_T_POSITION
#undef SSCREEN_BMS_SEVERAL_FRAME_POSITION
}
...
void SerialScreen_CmdDebugInfoSet(void)
{
    ...
    for(u8 i = 0; i < LCD_GUN_NUM; i++){
        if((LcdData.setData.DebugCmdPara_CC12V_Max[i] != LcdData.setData.DebugCmdPara_CC12V_MaxTemp[i]) || \
                ...
                (LcdData.setData.DebugCmdPara_CC6V_Max[i] != LcdData.setData.DebugCmdPara_CC6V_MaxTemp[i]) || \
                (LcdData.setData.DebugCmdPara_CC6V_Min[i] != LcdData.setData.DebugCmdPara_CC6V_MinTemp[i]) || \
                (LcdData.setData.DebugCmdPara_CC4V_Max[i] != LcdData.setData.DebugCmdPara_CC4V_MaxTemp[i]) || \
                (LcdData.setData.DebugCmdPara_CC4V_Min[i] != LcdData.setData.DebugCmdPara_CC4V_MinTemp[i])){
            /** CC1 范围设置有效 */
            if(sys_cc1_range_valid(LcdData.setData.DebugCmdPara_CC12V_MaxTemp[i], LcdData.setData.DebugCmdPara_CC12V_MinTemp[i], \
                    LcdData.setData.DebugCmdPara_CC6V_MaxTemp[i], LcdData.setData.DebugCmdPara_CC6V_MinTemp[i], \
                    LcdData.setData.DebugCmdPara_CC4V_MaxTemp[i], LcdData.setData.DebugCmdPara_CC4V_MinTemp[i])){

                LcdData.setData.DebugCmdPara_CC12V_Max[i] = LcdData.setData.DebugCmdPara_CC12V_MaxTemp[i];
                LcdData.setData.DebugCmdPara_CC12V_Min[i] = LcdData.setData.DebugCmdPara_CC12V_MinTemp[i];
                ...
                if(i == LCD_GUN_1){
                    UI_SYNC_SINGLE_CFG_DATA(CONFIG_ITEM_GUN1_CC14V_MAX, &LcdData.setData.DebugCmdPara_CC4V_Max[i], sizeof(LcdData.setData.DebugCmdPara_CC4V_Max[i]));
                    UI_SYNC_SINGLE_CFG_DATA(CONFIG_ITEM_GUN1_CC14V_MIN, &LcdData.setData.DebugCmdPara_CC4V_Min[i], sizeof(LcdData.setData.DebugCmdPara_CC4V_Min[i]));

                    UI_SYNC_SINGLE_CFG_DATA(CONFIG_ITEM_GUN1_CC16V_MAX, &LcdData.setData.DebugCmdPara_CC6V_Max[i], sizeof(LcdData.setData.DebugCmdPara_CC6V_Max[i]));
                    UI_SYNC_SINGLE_CFG_DATA(CONFIG_ITEM_GUN1_CC16V_MIN, &LcdData.setData.DebugCmdPara_CC6V_Min[i], sizeof(LcdData.setData.DebugCmdPara_CC6V_Min[i]));

                    UI_SYNC_SINGLE_CFG_DATA(CONFIG_ITEM_GUN1_CC112V_MAX, &LcdData.setData.DebugCmdPara_CC12V_Max[i], sizeof(LcdData.setData.DebugCmdPara_CC12V_Max[i]));
                    UI_SYNC_SINGLE_CFG_DATA(CONFIG_ITEM_GUN1_CC112V_MIN, &LcdData.setData.DebugCmdPara_CC12V_Min[i], sizeof(LcdData.setData.DebugCmdPara_CC12V_Min[i]));
                }else{
                    UI_SYNC_SINGLE_CFG_DATA(CONFIG_ITEM_GUN2_CC14V_MAX, &LcdData.setData.DebugCmdPara_CC4V_Max[i], sizeof(LcdData.setData.DebugCmdPara_CC4V_Max[i]));
                    UI_SYNC_SINGLE_CFG_DATA(CONFIG_ITEM_GUN2_CC14V_MIN, &LcdData.setData.DebugCmdPara_CC4V_Min[i], sizeof(LcdData.setData.DebugCmdPara_CC4V_Min[i]));

                    UI_SYNC_SINGLE_CFG_DATA(CONFIG_ITEM_GUN2_CC16V_MAX, &LcdData.setData.DebugCmdPara_CC6V_Max[i], sizeof(LcdData.setData.DebugCmdPara_CC6V_Max[i]));
                    UI_SYNC_SINGLE_CFG_DATA(CONFIG_ITEM_GUN2_CC16V_MIN, &LcdData.setData.DebugCmdPara_CC6V_Min[i], sizeof(LcdData.setData.DebugCmdPara_CC6V_Min[i]));

                    UI_SYNC_SINGLE_CFG_DATA(CONFIG_ITEM_GUN2_CC112V_MAX, &LcdData.setData.DebugCmdPara_CC12V_Max[i], sizeof(LcdData.setData.DebugCmdPara_CC12V_Max[i]));
                    UI_SYNC_SINGLE_CFG_DATA(CONFIG_ITEM_GUN2_CC112V_MIN, &LcdData.setData.DebugCmdPara_CC12V_Min[i], sizeof(LcdData.setData.DebugCmdPara_CC12V_Min[i]));
                }

                is_changed = 1;
            }else{
                LcdData.setData.DebugCmdPara_CC12V_MaxTemp[i] = LcdData.setData.DebugCmdPara_CC12V_Max[i];
                LcdData.setData.DebugCmdPara_CC12V_MinTemp[i] = LcdData.setData.DebugCmdPara_CC12V_Min[i];

                LcdData.setData.DebugCmdPara_CC6V_MaxTemp[i] = LcdData.setData.DebugCmdPara_CC6V_Max[i];
                LcdData.setData.DebugCmdPara_CC6V_MinTemp[i] = LcdData.setData.DebugCmdPara_CC6V_Min[i];

                LcdData.setData.DebugCmdPara_CC4V_MaxTemp[i] = LcdData.setData.DebugCmdPara_CC4V_Max[i];
    ...
    if(LcdData.setData.DebugCmdPara_Gun1_CurrOffset != LcdData.setData.DebugCmdPara_Gun1_CurrOffsetTemp){
        ...
        if(LcdData.setData.DebugCmdPara_Gun1_CurrOffsetTemp >= CP_CURRENT_OFFSET_SEPARATE){
            ...
            thaisenModuleSetMSetupCurrOffset(LCD_GUN_1, LcdData.setData.DebugCmdPara_Gun1_CurrOffsetTemp);
            LcdData.setData.DebugCmdPara_Gun1_CurrOffsetTemp += CP_CURRENT_OFFSET_SEPARATE;
        }
        /** 负偏移值 */
        else{
    ...
    if(LcdData.setData.DebugCmdPara_Gun2_CurrOffset != LcdData.setData.DebugCmdPara_Gun2_CurrOffsetTemp){
        ...
        if(LcdData.setData.DebugCmdPara_Gun2_CurrOffsetTemp >= CP_CURRENT_OFFSET_SEPARATE){
            ...
            thaisenModuleSetMSetupCurrOffset(LCD_GUN_2, LcdData.setData.DebugCmdPara_Gun2_CurrOffsetTemp);
            LcdData.setData.DebugCmdPara_Gun2_CurrOffsetTemp += CP_CURRENT_OFFSET_SEPARATE;
        }
        /** 负偏移值 */
        else{
    ...
    if(LcdData.setData.Icon_SupProtocolGB_T != LcdData.setData.sup_SupProtocolGB_T){
        is_changed = 1;
    }
    if(LcdData.setData.Icon_SupSeveralFrame != LcdData.setData.sup_SupSeveralFrame){
        is_changed = 1;
    }
    ...
    if(is_changed){
        ...
        if(LcdData.setData.Icon_SupProtocolGB_T == TRUE){
            LcdData.setData.sup_SupProtocolGB_T = CONFIG_ENABLE_ENUM;
        }else{
            LcdData.setData.sup_SupProtocolGB_T = CONFIG_DISABLE_ENUM;
        }

        if(LcdData.setData.Icon_SupSeveralFrame == TRUE){
            LcdData.setData.sup_SupSeveralFrame = CONFIG_ENABLE_ENUM;
        }else{
            LcdData.setData.sup_SupSeveralFrame = CONFIG_DISABLE_ENUM;
        }
        ...
        UI_SYNC_SINGLE_CFG_DATA(CONFIG_ITEM_SUPORT_PROTOCOL_GB_T, &LcdData.setData.sup_SupProtocolGB_T, sizeof(LcdData.setData.sup_SupProtocolGB_T));
        UI_SYNC_SINGLE_CFG_DATA(CONFIG_ITEM_SUPORT_BMS_SEVERAL_FRAME, &LcdData.setData.sup_SupSeveralFrame, sizeof(LcdData.setData.sup_SupSeveralFrame));
...
void SerialScreen_CmdDebugInfoGet(void)
{
    ...
    /* 国标协议(27930)默认关闭 */
    data = *((u8*) UI_READ_SINGLE_CFG_DATA(CONFIG_ITEM_SUPORT_PROTOCOL_GB_T, 0));
    if(data == CONFIG_ENABLE_ENUM){
        LcdData.setData.sup_SupProtocolGB_T = TRUE;
    }else if(data == CONFIG_DISABLE_ENUM){
        LcdData.setData.sup_SupProtocolGB_T = FALSE;
    }else{
        LcdData.setData.sup_SupProtocolGB_T = FALSE;
    }
    /* BMS多帧默认启用 */
    data = *((u8*) UI_READ_SINGLE_CFG_DATA(CONFIG_ITEM_SUPORT_BMS_SEVERAL_FRAME, 0));
    if(data == CONFIG_ENABLE_ENUM){
        LcdData.setData.sup_SupSeveralFrame = TRUE;
    }else if(data == CONFIG_DISABLE_ENUM){
        LcdData.setData.sup_SupSeveralFrame = FALSE;
    }else{
        LcdData.setData.sup_SupSeveralFrame = TRUE;
    }
    /* 灯语默认使用灯语0 */
    ...
    LcdData.setData.DebugCmdPara_CC12V_Max[LCD_GUN_1] = *((u16*) UI_READ_SINGLE_CFG_DATA(CONFIG_ITEM_GUN1_CC112V_MAX, 0));
    LcdData.setData.DebugCmdPara_CC12V_Max[LCD_GUN_2] = *((u16*) UI_READ_SINGLE_CFG_DATA(CONFIG_ITEM_GUN2_CC112V_MAX, 0));

    LcdData.setData.DebugCmdPara_CC6V_Max[LCD_GUN_1] = *((u16*) UI_READ_SINGLE_CFG_DATA(CONFIG_ITEM_GUN1_CC16V_MAX, 0));
    LcdData.setData.DebugCmdPara_CC6V_Max[LCD_GUN_2] = *((u16*) UI_READ_SINGLE_CFG_DATA(CONFIG_ITEM_GUN2_CC16V_MAX, 0));

    LcdData.setData.DebugCmdPara_CC4V_Max[LCD_GUN_1] = *((u16*) UI_READ_SINGLE_CFG_DATA(CONFIG_ITEM_GUN1_CC14V_MAX, 0));
    LcdData.setData.DebugCmdPara_CC4V_Max[LCD_GUN_2] = *((u16*) UI_READ_SINGLE_CFG_DATA(CONFIG_ITEM_GUN2_CC14V_MAX, 0));

    LcdData.setData.DebugCmdPara_CC12V_Min[LCD_GUN_1] = *((u16*) UI_READ_SINGLE_CFG_DATA(CONFIG_ITEM_GUN1_CC112V_MIN, 0));
    LcdData.setData.DebugCmdPara_CC12V_Min[LCD_GUN_2] = *((u16*) UI_READ_SINGLE_CFG_DATA(CONFIG_ITEM_GUN2_CC112V_MIN, 0));

    LcdData.setData.DebugCmdPara_CC6V_Min[LCD_GUN_1] = *((u16*) UI_READ_SINGLE_CFG_DATA(CONFIG_ITEM_GUN1_CC16V_MIN, 0));
    LcdData.setData.DebugCmdPara_CC6V_Min[LCD_GUN_2] = *((u16*) UI_READ_SINGLE_CFG_DATA(CONFIG_ITEM_GUN2_CC16V_MIN, 0));
    ...
    if(LcdData.setData.DebugCmdPara_Gun1_CurrOffset >= CP_CURRENT_OFFSET_SEPARATE){
        ...
        thaisenModuleSetMSetupCurrOffset(LCD_GUN_1, LcdData.setData.DebugCmdPara_Gun1_CurrOffset);
        LcdData.setData.DebugCmdPara_Gun1_CurrOffset += CP_CURRENT_OFFSET_SEPARATE;
    }
    /** 负偏移值 */
    else{
    ...
    LcdData.setData.Icon_SupProtocolGB_T = FALSE;
    if(LcdData.setData.sup_SupProtocolGB_T){
        LcdData.setData.Icon_SupProtocolGB_T = TRUE;
    }

    LcdData.setData.Icon_SupSeveralFrame = FALSE;
    if(LcdData.setData.sup_SupSeveralFrame){
        LcdData.setData.Icon_SupSeveralFrame = TRUE;
    }

    for(int i = 0; i < LCD_GUN_NUM; i++)
    {
        if(sys_cc1_range_valid(LcdData.setData.DebugCmdPara_CC12V_Max[i], LcdData.setData.DebugCmdPara_CC12V_Min[i], \
                LcdData.setData.DebugCmdPara_CC6V_Max[i], LcdData.setData.DebugCmdPara_CC6V_Min[i], \
                LcdData.setData.DebugCmdPara_CC4V_Max[i], LcdData.setData.DebugCmdPara_CC4V_Min[i]) == 0){

            LcdData.setData.DebugCmdPara_CC12V_Max[i] = CHARGEPILE_CC12V_MAX_DEF;
            LcdData.setData.DebugCmdPara_CC12V_Min[i] = CHARGEPILE_CC12V_MIN_DEF;

            LcdData.setData.DebugCmdPara_CC6V_Max[i] = CHARGEPILE_CC6V_MAX_DEF;
            LcdData.setData.DebugCmdPara_CC6V_Min[i] = CHARGEPILE_CC6V_MIN_DEF;
    ...
    memcpy(LcdData.setData.DebugCmdPara_CC6V_MaxTemp, LcdData.setData.DebugCmdPara_CC6V_Max, sizeof(LcdData.setData.DebugCmdPara_CC6V_Max));
    memcpy(LcdData.setData.DebugCmdPara_CC6V_MinTemp, LcdData.setData.DebugCmdPara_CC6V_Min, sizeof(LcdData.setData.DebugCmdPara_CC6V_Min));
...
void SerialScreen_CmdDebugIssue(void)
{
    u8 used_len = 0, blen = sizeof(LcdData.setData.DebugResult);
    char CmdBuf[32], CmdParaBuf[32];
    char *save_cmdptr = NULL, *CmdStr = NULL;
    char *save_paraptr = NULL, *ParaStr = NULL;

    memset(CmdBuf, 0, sizeof(CmdBuf));
    if(sizeof(LcdData.setData.DebugCmd) > sizeof(CmdBuf)){
        memcpy(CmdBuf, LcdData.setData.DebugCmd, sizeof(CmdBuf));
    }else{
        memcpy(CmdBuf, LcdData.setData.DebugCmd, sizeof(LcdData.setData.DebugCmd));
    }

    memset(CmdParaBuf, 0, sizeof(CmdParaBuf));
    if(sizeof(LcdData.setData.DebugCmdPara) > sizeof(CmdParaBuf)){
        memcpy(CmdParaBuf, LcdData.setData.DebugCmdPara, sizeof(CmdParaBuf));
    }else{
        memcpy(CmdParaBuf, LcdData.setData.DebugCmdPara, sizeof(LcdData.setData.DebugCmdPara));
    }
    save_cmdptr = CmdBuf;
    save_paraptr = CmdParaBuf;

    memset(LcdData.setData.DebugResult, 0, sizeof(LcdData.setData.DebugResult));
    do{
        CmdStr = strtok_r(save_cmdptr, ",", &save_cmdptr);
        ParaStr = strtok_r(save_paraptr, ",", &save_paraptr);
        ...
        /** 多条指令之间使用分隔符分开 */
        if((used_len != 0)){
            used_len = used_len >= blen ? (blen - 1) : used_len;
            LcdData.setData.DebugResult[used_len] = '/';
            used_len++;
        }
        ...
        if((memcmp(CmdStr, "MCMAX", strlen("MCMAX")) == 0) && (strlen(CmdStr) == strlen("MCMAX")))
        {
            if((used_len < blen) && ParaStr){
                ...
                thaisen_get_cmd_debug_result_info(LCD_GUN_1, THA_DEBUG_LANGUAGE_CHINESE, THAISEN_DEBUG_CMD_ISSUE_MODULE_CURR_MAX, (u8*)&para, sizeof(para), \
        ...
        else if((memcmp(CmdStr, "MCMIN", strlen("MCMIN")) == 0) && (strlen(CmdStr) == strlen("MCMIN")))
        {
            if((used_len < blen) && ParaStr){
                ...
                thaisen_get_cmd_debug_result_info(LCD_GUN_1, THA_DEBUG_LANGUAGE_CHINESE, THAISEN_DEBUG_CMD_ISSUE_MODULE_CURR_MIN, (u8*)&para, sizeof(para), \
                        (LcdData.setData.DebugResult + used_len), (blen - used_len));
                /** 保存指令参数 */
        ...
        else if((memcmp(CmdStr, "LED", strlen("LED")) == 0) && (strlen(CmdStr) == strlen("LED")))
        {
            if((used_len < blen) && ParaStr){
                ...
                para -= CP_LED_LANGUAGE_OFFSET;
                thaisen_get_cmd_debug_result_info(LCD_GUN_1, THA_DEBUG_LANGUAGE_CHINESE, THAISEN_DEBUG_CMD_ISSUE_LED_LANGUAGE, (u8*)&para, sizeof(para), \
                        (LcdData.setData.DebugResult + used_len), (blen - used_len));
                /** 保存指令参数 */
                LcdData.setData.DebugCmdPara_LedLanguageTemp = para;
            }
        }
        /** 指令下发：枪1 CC1 12V 上限(0.001V  U1 as uplimit gun1) */
        else if((memcmp(CmdStr, "CC12_U1", strlen("CC12_U1")) == 0) && (strlen(CmdStr) == strlen("CC12_U1")))
        {
            if((used_len < blen) && ParaStr){
                u32 para = atol(ParaStr);

                thaisen_get_cmd_debug_result_info(LCD_GUN_1, THA_DEBUG_LANGUAGE_CHINESE, THAISEN_DEBUG_CMD_ISSUE_CC12V_UPLIMIT, (u8*)&para, sizeof(para), \
                        (LcdData.setData.DebugResult + used_len), (blen - used_len));
                /** 保存指令参数 */
                LcdData.setData.DebugCmdPara_CC12V_MaxTemp[LCD_GUN_1] = para;
            }
        }
        /** 指令下发：枪1 CC1 12V 下限(0.001V  L1 as lower limit gun1) */
        else if((memcmp(CmdStr, "CC12_L1", strlen("CC12_L1")) == 0) && (strlen(CmdStr) == strlen("CC12_L1")))
        {
            if((used_len < blen) && ParaStr){
                u32 para = atol(ParaStr);

                thaisen_get_cmd_debug_result_info(LCD_GUN_1, THA_DEBUG_LANGUAGE_CHINESE, THAISEN_DEBUG_CMD_ISSUE_CC12V_LOWLIMIT, (u8*)&para, sizeof(para), \
                        (LcdData.setData.DebugResult + used_len), (blen - used_len));
                /** 保存指令参数 */
                LcdData.setData.DebugCmdPara_CC12V_MinTemp[LCD_GUN_1] = para;
            }
        }
        /** 指令下发：枪1 CC1 6V 上限(0.001V  U1 as uplimit gun1) */
        else if((memcmp(CmdStr, "CC6_U1", strlen("CC6_U1")) == 0) && (strlen(CmdStr) == strlen("CC6_U1")))
        {
            if((used_len < blen) && ParaStr){
                u32 para = atol(ParaStr);

                thaisen_get_cmd_debug_result_info(LCD_GUN_1, THA_DEBUG_LANGUAGE_CHINESE, THAISEN_DEBUG_CMD_ISSUE_CC6V_UPLIMIT, (u8*)&para, sizeof(para), \
                        (LcdData.setData.DebugResult + used_len), (blen - used_len));
                /** 保存指令参数 */
                LcdData.setData.DebugCmdPara_CC6V_MaxTemp[LCD_GUN_1] = para;
            }
        }
        /** 指令下发：枪1 CC1 6V 下限(0.001V  L1 as lower limit gun1) */
        else if((memcmp(CmdStr, "CC6_L1", strlen("CC6_L1")) == 0) && (strlen(CmdStr) == strlen("CC6_L1")))
        {
            if((used_len < blen) && ParaStr){
                u32 para = atol(ParaStr);

                thaisen_get_cmd_debug_result_info(LCD_GUN_1, THA_DEBUG_LANGUAGE_CHINESE, THAISEN_DEBUG_CMD_ISSUE_CC6V_LOWLIMIT, (u8*)&para, sizeof(para), \
                        (LcdData.setData.DebugResult + used_len), (blen - used_len));
                /** 保存指令参数 */
                LcdData.setData.DebugCmdPara_CC6V_MinTemp[LCD_GUN_1] = para;
            }
        }
        /** 指令下发：枪1 CC1 4V 上限(0.001V  U1 as uplimit gun1) */
        else if((memcmp(CmdStr, "CC4_U1", strlen("CC4_U1")) == 0) && (strlen(CmdStr) == strlen("CC4_U1")))
        {
            if((used_len < blen) && ParaStr){
                u32 para = atol(ParaStr);

                thaisen_get_cmd_debug_result_info(LCD_GUN_1, THA_DEBUG_LANGUAGE_CHINESE, THAISEN_DEBUG_CMD_ISSUE_CC4V_UPLIMIT, (u8*)&para, sizeof(para), \
                        (LcdData.setData.DebugResult + used_len), (blen - used_len));
                /** 保存指令参数 */
                LcdData.setData.DebugCmdPara_CC4V_MaxTemp[LCD_GUN_1] = para;
            }
        }
        /** 指令下发：枪1 CC1 4V 下限(0.001V  L1 as lower limit gun1) */
        else if((memcmp(CmdStr, "CC4_L1", strlen("CC4_L1")) == 0) && (strlen(CmdStr) == strlen("CC4_L1")))
        {
            if((used_len < blen) && ParaStr){
                u32 para = atol(ParaStr);

                thaisen_get_cmd_debug_result_info(LCD_GUN_1, THA_DEBUG_LANGUAGE_CHINESE, THAISEN_DEBUG_CMD_ISSUE_CC4V_LOWLIMIT, (u8*)&para, sizeof(para), \
                        (LcdData.setData.DebugResult + used_len), (blen - used_len));
                /** 保存指令参数 */
                LcdData.setData.DebugCmdPara_CC4V_MinTemp[LCD_GUN_1] = para;
            }
        }
        /** 指令下发：枪2 CC1 12V 上限(0.001V  U2 as uplimit gun2) */
        else if((memcmp(CmdStr, "CC12_U2", strlen("CC12_U2")) == 0) && (strlen(CmdStr) == strlen("CC12_U2")))
        {
            if((used_len < blen) && ParaStr){
                u32 para = atol(ParaStr);

                thaisen_get_cmd_debug_result_info(LCD_GUN_2, THA_DEBUG_LANGUAGE_CHINESE, THAISEN_DEBUG_CMD_ISSUE_CC12V_UPLIMIT, (u8*)&para, sizeof(para), \
                        (LcdData.setData.DebugResult + used_len), (blen - used_len));
                /** 保存指令参数 */
                LcdData.setData.DebugCmdPara_CC12V_MaxTemp[LCD_GUN_2] = para;
            }
        }
        /** 指令下发：枪2 CC1 12V 下限(0.001V  L2 as lower limit gun2) */
        else if((memcmp(CmdStr, "CC12_L2", strlen("CC12_L2")) == 0) && (strlen(CmdStr) == strlen("CC12_L2")))
        {
            if((used_len < blen) && ParaStr){
                u32 para = atol(ParaStr);

                thaisen_get_cmd_debug_result_info(LCD_GUN_2, THA_DEBUG_LANGUAGE_CHINESE, THAISEN_DEBUG_CMD_ISSUE_CC12V_LOWLIMIT, (u8*)&para, sizeof(para), \
                        (LcdData.setData.DebugResult + used_len), (blen - used_len));
                /** 保存指令参数 */
                LcdData.setData.DebugCmdPara_CC12V_MinTemp[LCD_GUN_2] = para;
            }
        }
        /** 指令下发：枪2 CC1 6V 上限(0.001V  U2 as uplimit gun2) */
        else if((memcmp(CmdStr, "CC6_U2", strlen("CC6_U2")) == 0) && (strlen(CmdStr) == strlen("CC6_U2")))
        {
            if((used_len < blen) && ParaStr){
                u32 para = atol(ParaStr);

                thaisen_get_cmd_debug_result_info(LCD_GUN_2, THA_DEBUG_LANGUAGE_CHINESE, THAISEN_DEBUG_CMD_ISSUE_CC6V_UPLIMIT, (u8*)&para, sizeof(para), \
                        (LcdData.setData.DebugResult + used_len), (blen - used_len));
                /** 保存指令参数 */
                LcdData.setData.DebugCmdPara_CC6V_MaxTemp[LCD_GUN_2] = para;
            }
        }
        /** 指令下发：枪2 CC1 6V 下限(0.001V  L2 as lower limit gun2) */
        else if((memcmp(CmdStr, "CC6_L2", strlen("CC6_L2")) == 0) && (strlen(CmdStr) == strlen("CC6_L2")))
        {
            if((used_len < blen) && ParaStr){
                u32 para = atol(ParaStr);

                thaisen_get_cmd_debug_result_info(LCD_GUN_2, THA_DEBUG_LANGUAGE_CHINESE, THAISEN_DEBUG_CMD_ISSUE_CC6V_LOWLIMIT, (u8*)&para, sizeof(para), \
                        (LcdData.setData.DebugResult + used_len), (blen - used_len));
                /** 保存指令参数 */
                LcdData.setData.DebugCmdPara_CC6V_MinTemp[LCD_GUN_2] = para;
            }
        }
        /** 指令下发：枪2 CC1 4V 上限(0.001V  U2 as uplimit gun2) */
        else if((memcmp(CmdStr, "CC4_U2", strlen("CC4_U2")) == 0) && (strlen(CmdStr) == strlen("CC4_U2")))
        {
            if((used_len < blen) && ParaStr){
                u32 para = atol(ParaStr);

                thaisen_get_cmd_debug_result_info(LCD_GUN_2, THA_DEBUG_LANGUAGE_CHINESE, THAISEN_DEBUG_CMD_ISSUE_CC4V_UPLIMIT, (u8*)&para, sizeof(para), \
                        (LcdData.setData.DebugResult + used_len), (blen - used_len));
                /** 保存指令参数 */
                LcdData.setData.DebugCmdPara_CC4V_MaxTemp[LCD_GUN_2] = para;
            }
        }
        /** 指令下发：枪2 CC1 4V 下限(0.001V  L2 as lower limit gun2) */
        else if((memcmp(CmdStr, "CC4_L2", strlen("CC4_L2")) == 0) && (strlen(CmdStr) == strlen("CC4_L2")))
        {
            if((used_len < blen) && ParaStr){
                u32 para = atol(ParaStr);

                thaisen_get_cmd_debug_result_info(LCD_GUN_2, THA_DEBUG_LANGUAGE_CHINESE, THAISEN_DEBUG_CMD_ISSUE_CC4V_LOWLIMIT, (u8*)&para, sizeof(para), \
                        (LcdData.setData.DebugResult + used_len), (blen - used_len));
                /** 保存指令参数 */
...
void SerialScreen_CmdDebugRead(void)
{
    u8 used_len = 0, blen = sizeof(LcdData.setData.DebugResult);
    char CmdBuf[32];
    char *save_cmdptr = NULL, *CmdStr = NULL;
    ...
    do{
        ...
        if((memcmp(CmdStr, "MCMAX", strlen("MCMAX")) == 0) && (strlen(CmdStr) == strlen("MCMAX")))
        {
            if(used_len < blen){
                ...
                thaisen_get_cmd_debug_result_info(LCD_GUN_1, THA_DEBUG_LANGUAGE_CHINESE, THAISEN_DEBUG_CMD_READ_MODULE_CURR_MAX, (u8*)&para, sizeof(para), \
                        (LcdData.setData.DebugResult + used_len), (blen - used_len));
            }
        }
        /** 指令读取：模块最小输出电流值 */
        else if((memcmp(CmdStr, "MCMIN", strlen("MCMIN")) == 0) && (strlen(CmdStr) == strlen("MCMIN")))
        {
            if(used_len < blen){
                ...
                thaisen_get_cmd_debug_result_info(LCD_GUN_1, THA_DEBUG_LANGUAGE_CHINESE, THAISEN_DEBUG_CMD_READ_MODULE_CURR_MIN, (u8*)&para, sizeof(para), \
                        (LcdData.setData.DebugResult + used_len), (blen - used_len));
            }
        }
        /** 指令读取：灯语 */
        else if(strstr((const char*)CmdStr, "LED"))
        {
            ...
            if(used_len < blen){
                thaisen_get_cmd_debug_result_info(LCD_GUN_1, THA_DEBUG_LANGUAGE_CHINESE, THAISEN_DEBUG_CMD_READ_LED_LANGUAGE, (u8*)&number, sizeof(number), \
                        (LcdData.setData.DebugResult + used_len), (blen - used_len));
            }
        }
        /** 指令读取：枪1 CC1 12V 上限(0.001V  U1 as uplimit gun1) */
        else if((memcmp(CmdStr, "CC12_U1", strlen("CC12_U1")) == 0) && (strlen(CmdStr) == strlen("CC12_U1")))
        {
            if(used_len < blen){
                u32 para = LcdData.setData.DebugCmdPara_CC12V_MaxTemp[LCD_GUN_1];
                thaisen_get_cmd_debug_result_info(LCD_GUN_1, THA_DEBUG_LANGUAGE_CHINESE, THAISEN_DEBUG_CMD_READ_CC12V_UPLIMIT, (u8*)&para, sizeof(para), \
                        (LcdData.setData.DebugResult + used_len), (blen - used_len));
            }
        }
        /** 指令读取：枪1 CC1 12V 下限(0.001V  L1 as lower limit gun1) */
        else if((memcmp(CmdStr, "CC12_L1", strlen("CC12_L1")) == 0) && (strlen(CmdStr) == strlen("CC12_L1")))
        {
            if(used_len < blen){
                u32 para = LcdData.setData.DebugCmdPara_CC12V_MinTemp[LCD_GUN_1];
                thaisen_get_cmd_debug_result_info(LCD_GUN_1, THA_DEBUG_LANGUAGE_CHINESE, THAISEN_DEBUG_CMD_READ_CC12V_LOWLIMIT, (u8*)&para, sizeof(para), \
                        (LcdData.setData.DebugResult + used_len), (blen - used_len));
            }
        }
        /** 指令读取：枪1 CC1 6V 上限(0.001V  U1 as uplimit gun1) */
        else if((memcmp(CmdStr, "CC6_U1", strlen("CC6_U1")) == 0) && (strlen(CmdStr) == strlen("CC6_U1")))
        {
            if(used_len < blen){
                u32 para = LcdData.setData.DebugCmdPara_CC6V_MaxTemp[LCD_GUN_1];
                thaisen_get_cmd_debug_result_info(LCD_GUN_1, THA_DEBUG_LANGUAGE_CHINESE, THAISEN_DEBUG_CMD_READ_CC6V_UPLIMIT, (u8*)&para, sizeof(para), \
                        (LcdData.setData.DebugResult + used_len), (blen - used_len));
            }
        }
        /** 指令读取：枪1 CC1 6V 下限(0.001V  L1 as lower limit gun1) */
        else if((memcmp(CmdStr, "CC6_L1", strlen("CC6_L1")) == 0) && (strlen(CmdStr) == strlen("CC6_L1")))
        {
            if(used_len < blen){
                u32 para = LcdData.setData.DebugCmdPara_CC6V_MinTemp[LCD_GUN_1];
                thaisen_get_cmd_debug_result_info(LCD_GUN_1, THA_DEBUG_LANGUAGE_CHINESE, THAISEN_DEBUG_CMD_READ_CC6V_LOWLIMIT, (u8*)&para, sizeof(para), \
                        (LcdData.setData.DebugResult + used_len), (blen - used_len));
            }
        }
        /** 指令读取：枪1 CC1 4V 上限(0.001V  U1 as uplimit gun1) */
        else if((memcmp(CmdStr, "CC4_U1", strlen("CC4_U1")) == 0) && (strlen(CmdStr) == strlen("CC4_U1")))
        {
            if(used_len < blen){
                u32 para = LcdData.setData.DebugCmdPara_CC4V_MaxTemp[LCD_GUN_1];
                thaisen_get_cmd_debug_result_info(LCD_GUN_1, THA_DEBUG_LANGUAGE_CHINESE, THAISEN_DEBUG_CMD_READ_CC4V_UPLIMIT, (u8*)&para, sizeof(para), \
                        (LcdData.setData.DebugResult + used_len), (blen - used_len));
            }
        }
        /** 指令读取：枪1 CC1 4V 下限(0.001V  L1 as lower limit gun1) */
        else if((memcmp(CmdStr, "CC4_L1", strlen("CC4_L1")) == 0) && (strlen(CmdStr) == strlen("CC4_L1")))
        {
            if(used_len < blen){
                u32 para = LcdData.setData.DebugCmdPara_CC4V_MinTemp[LCD_GUN_1];
                thaisen_get_cmd_debug_result_info(LCD_GUN_1, THA_DEBUG_LANGUAGE_CHINESE, THAISEN_DEBUG_CMD_READ_CC4V_LOWLIMIT, (u8*)&para, sizeof(para), \
                        (LcdData.setData.DebugResult + used_len), (blen - used_len));
            }
        }
        /** 指令读取：枪2 CC1 12V 上限(0.001V  U2 as uplimit gun2) */
        else if((memcmp(CmdStr, "CC12_U2", strlen("CC12_U2")) == 0) && (strlen(CmdStr) == strlen("CC12_U2")))
        {
            if(used_len < blen){
                u32 para = LcdData.setData.DebugCmdPara_CC12V_MaxTemp[LCD_GUN_2];
                thaisen_get_cmd_debug_result_info(LCD_GUN_2, THA_DEBUG_LANGUAGE_CHINESE, THAISEN_DEBUG_CMD_READ_CC12V_UPLIMIT, (u8*)&para, sizeof(para), \
                        (LcdData.setData.DebugResult + used_len), (blen - used_len));
            }
        }
        /** 指令读取：枪2 CC1 12V 下限(0.001V  L2 as lower limit gun2) */
        else if((memcmp(CmdStr, "CC12_L2", strlen("CC12_L2")) == 0) && (strlen(CmdStr) == strlen("CC12_L2")))
        {
            if(used_len < blen){
                u32 para = LcdData.setData.DebugCmdPara_CC12V_MinTemp[LCD_GUN_2];
                thaisen_get_cmd_debug_result_info(LCD_GUN_2, THA_DEBUG_LANGUAGE_CHINESE, THAISEN_DEBUG_CMD_READ_CC12V_LOWLIMIT, (u8*)&para, sizeof(para), \
                        (LcdData.setData.DebugResult + used_len), (blen - used_len));
            }
        }
        /** 指令读取：枪2 CC1 6V 上限(0.001V  U2 as uplimit gun2) */
        else if((memcmp(CmdStr, "CC6_U2", strlen("CC6_U2")) == 0) && (strlen(CmdStr) == strlen("CC6_U2")))
        {
            if(used_len < blen){
                u32 para = LcdData.setData.DebugCmdPara_CC6V_MaxTemp[LCD_GUN_2];
                thaisen_get_cmd_debug_result_info(LCD_GUN_2, THA_DEBUG_LANGUAGE_CHINESE, THAISEN_DEBUG_CMD_READ_CC6V_UPLIMIT, (u8*)&para, sizeof(para), \
                        (LcdData.setData.DebugResult + used_len), (blen - used_len));
            }
        }
        /** 指令读取：枪2 CC1 6V 下限(0.001V  L2 as lower limit gun2) */
        else if((memcmp(CmdStr, "CC6_L2", strlen("CC6_L2")) == 0) && (strlen(CmdStr) == strlen("CC6_L2")))
        {
            if(used_len < blen){
                u32 para = LcdData.setData.DebugCmdPara_CC6V_MinTemp[LCD_GUN_2];
                thaisen_get_cmd_debug_result_info(LCD_GUN_2, THA_DEBUG_LANGUAGE_CHINESE, THAISEN_DEBUG_CMD_READ_CC6V_LOWLIMIT, (u8*)&para, sizeof(para), \
                        (LcdData.setData.DebugResult + used_len), (blen - used_len));
            }
        }
        /** 指令读取：枪2 CC1 4V 上限(0.001V  U2 as uplimit gun2) */
        else if((memcmp(CmdStr, "CC4_U2", strlen("CC4_U2")) == 0) && (strlen(CmdStr) == strlen("CC4_U2")))
        {
            if(used_len < blen){
                u32 para = LcdData.setData.DebugCmdPara_CC4V_MaxTemp[LCD_GUN_2];
                thaisen_get_cmd_debug_result_info(LCD_GUN_2, THA_DEBUG_LANGUAGE_CHINESE, THAISEN_DEBUG_CMD_READ_CC4V_UPLIMIT, (u8*)&para, sizeof(para), \
                        (LcdData.setData.DebugResult + used_len), (blen - used_len));
            }
        }
        /** 指令读取：枪2 CC1 4V 下限(0.001V  L2 as lower limit gun2) */
        else if((memcmp(CmdStr, "CC4_L2", strlen("CC4_L2")) == 0) && (strlen(CmdStr) == strlen("CC4_L2")))
        {
            if(used_len < blen){
...
#define SSCREEN_CARD_BLOCK_SN_POSITION                        14   /* 卡号所在块号在结构体 thaisen_cfg_info_function 中的成员次序(从0开始)  */
    ...
    LcdData.setData.sup_mode_select = LcdData.setData.Icon_SupModeSelect;
#ifdef SCREEN_USING_V2G
...
{
    u8 mode = THAISEN_CHARGE_MODE_FULL;
#ifdef SCREEN_USING_V2G
    u8 longTermValid = FALSE;
#endif /* SCREEN_USING_V2G */
#if 0
    switch(thaisen_app_get_ofsm_charge_state(port)){
    case APP_OFSM_STATE_RESERVATION:
        /* 弹出提示：预约中不可修改策略 */
        if((LcdData.setData.CurrentMode[LCD_GUN_NUM][THAISEN_CHARGE_MODE_LIMIT_RESERVATION] == TRUE) &&  \
                (LcdData.setData.CurrentMode[port][THAISEN_CHARGE_MODE_LIMIT_RESERVATION] == TRUE)){
            if(LcdData.setData.CurrentModePara[port] != (LcdData.setData.MSLimitReservationHour[LCD_GUN_NUM] *3600 + LcdData.setData.MSLimitReservationMin[LCD_GUN_NUM] *60)){
                LcdData.setData.Modewarning = TRUE;
                /* 此时不进行切页 */
                if(LcdData.CurrentPage != LcdData.CurrentPageBack){
                    LcdData.CurrentPage = LcdData.CurrentPageBack;
                }
    ...
    if(LcdData.setData.CurrentMode[LCD_GUN_NUM][THAISEN_CHARGE_MODE_LIMIT_MONEY]){
        mode = THAISEN_CHARGE_MODE_LIMIT_MONEY;
...
void SerialScreen_BtnModeInfoGet(u8 port)
{
    ...
    LcdData.setData.Modewarning = FALSE;
    ...
    case CP_MODE_LIMIT_MONEY:
        LcdData.setData.CurrentMode[LCD_GUN_NUM][THAISEN_CHARGE_MODE_LIMIT_MONEY] = TRUE;
    ...
    case CP_MODE_LIMIT_ELECT:
        LcdData.setData.CurrentMode[LCD_GUN_NUM][THAISEN_CHARGE_MODE_LIMIT_ELECT] = TRUE;
    ...
    case CP_MODE_LIMIT_TIMING:
        LcdData.setData.CurrentMode[LCD_GUN_NUM][THAISEN_CHARGE_MODE_LIMIT_TIMING] = TRUE;
    ...
    case CP_MODE_LIMIT_RESERVATION:
        LcdData.setData.CurrentMode[LCD_GUN_NUM][THAISEN_CHARGE_MODE_LIMIT_RESERVATION] = TRUE;
        ...
        break;
    default:
        LcdData.setData.CurrentMode[LCD_GUN_NUM][THAISEN_CHARGE_MODE_FULL] = TRUE;
...
#ifdef SCREEN_USING_V2G
void SerialScreen_BtnV2GInfoStorage(int port)
{
    u8 mode = CP_V2G_MODE_NULL, longTermValid = FALSE;
    ...
    case APP_OFSM_STATE_RESERVATION:
        /* 弹出提示：预约中不可修改策略 */
        if((LcdData.setData.V2G_CurrentMode[LCD_GUN_NUM][THAISEN_CHARGE_MODE_LIMIT_RESERVATION] == TRUE) &&  \
                (LcdData.setData.V2G_CurrentMode[port][THAISEN_CHARGE_MODE_LIMIT_RESERVATION] == TRUE)){
            if(LcdData.setData.V2G_CurrentModePara[port] != (LcdData.setData.V2G_MSLimitReservationHour[LCD_GUN_NUM] *3600 + LcdData.setData.V2G_MSLimitReservationMin[LCD_GUN_NUM] *60)){
                LcdData.setData.Modewarning = TRUE;
                /* 此时不进行切页 */
                if(LcdData.CurrentPage != LcdData.CurrentPageBack){
                    LcdData.CurrentPage = LcdData.CurrentPageBack;
    ...
    LcdAssistantData.Flag.IsConfigFail = FALSE;
    /** 已进行预约充电，不可修改V2G模式 */
    if(SerialScreen_Screen_GetCurrentChargeMode(port) == THAISEN_CHARGE_MODE_LIMIT_RESERVATION){
        /** 弹出提示：已进行预约充电，不可修改V2G模式 */
        LcdData.setData.Modewarning = TRUE;
        /** 此时不进行切页 */
        if(LcdData.CurrentPage != LcdData.CurrentPageBack){
    ...
    if(LcdData.setData.V2G_CurrentMode[LCD_GUN_NUM][THAISEN_V2G_MODE_LIMIT_MONEY]){
        mode = CP_V2G_MODE_LIMIT_MONEY;
        if((LcdData.setData.V2G_MSLimitMoney[LCD_GUN_NUM] > CP_V2G_MODE_PARA_MONEY_MAX) || (LcdData.setData.V2G_MSLimitMoney[LCD_GUN_NUM] < CP_V2G_MODE_PARA_MONEY_MIN)){
            LcdData.setData.V2G_MSLimitMoney[LCD_GUN_NUM] = CP_V2G_MODE_PARA_MONEY_DEF;
        }
        ...
        if((LcdData.setData.V2G_MSLimitElect[LCD_GUN_NUM] > CP_V2G_MODE_PARA_ELECT_MAX) || (LcdData.setData.V2G_MSLimitElect[LCD_GUN_NUM] < CP_V2G_MODE_PARA_ELECT_MIN)){
            LcdData.setData.V2G_MSLimitElect[LCD_GUN_NUM] = CP_V2G_MODE_PARA_ELECT_DEF;
        }
        ...
        if((LcdData.setData.V2G_MSLimitTiming[LCD_GUN_NUM] > CP_V2G_MODE_PARA_TIMING_MAX) || (LcdData.setData.V2G_MSLimitTiming[LCD_GUN_NUM] < CP_V2G_MODE_PARA_TIMING_MIN)){
            LcdData.setData.V2G_MSLimitTiming[LCD_GUN_NUM] = CP_V2G_MODE_PARA_TIMING_DEF;
        }
        ...
        mode = THAISEN_CHARGE_MODE_LIMIT_RESERVATION;
        if(LcdData.setData.V2G_MSLimitReservationHour[LCD_GUN_NUM] >= 23){
            LcdData.setData.V2G_MSLimitReservationHour[LCD_GUN_NUM] = 23;
        }
        if(LcdData.setData.V2G_MSLimitReservationMin[LCD_GUN_NUM] >= 60){
            LcdData.setData.V2G_MSLimitReservationMin[LCD_GUN_NUM] = 0;
        }
        LcdData.setData.V2G_CurrentModePara[port] = LcdData.setData.V2G_MSLimitReservationHour[LCD_GUN_NUM] *3600 + LcdData.setData.V2G_MSLimitReservationMin[LCD_GUN_NUM] *60;
        LcdAssistantData.SeveralGunFlag[port].IsSetReservation = TRUE;
    }
#endif
    SerialScreen_JumpPage(&SerialScreen, LCD_PAGE_STORAGE_WAITING);  /* 目前放电模式都是单次充电有效，不需要保存 */

    memcpy(LcdData.setData.V2G_CurrentMode[port], LcdData.setData.V2G_CurrentMode[LCD_GUN_NUM], sizeof(LcdData.setData.V2G_CurrentMode[port]));
    if(port == LCD_GUN_1){
        UI_SYNC_SINGLE_CFG_DATA(CONFIG_ITEM_CURRENT_V2G_MODE_A, &mode, sizeof(mode));
        UI_SYNC_SINGLE_CFG_DATA(CONFIG_ITEM_V2G_MODE_PARAMETER_A, (u8*)&LcdData.setData.V2G_CurrentModePara[port], sizeof(LcdData.setData.V2G_CurrentModePara[port]));
    }else{
        UI_SYNC_SINGLE_CFG_DATA(CONFIG_ITEM_CURRENT_V2G_MODE_B, &mode, sizeof(mode));
        UI_SYNC_SINGLE_CFG_DATA(CONFIG_ITEM_V2G_MODE_PARAMETER_B, (u8*)&LcdData.setData.V2G_CurrentModePara[port], sizeof(LcdData.setData.V2G_CurrentModePara[port]));
    }
    /** 本次修改的V2G模式是长期有效的模式，需要复位充电模式相关信息 */
    if(longTermValid == TRUE){
        /** 复位充电模式相关信息 */
        u8 mode = 0;
        u32 parameter = 0;
...
void SerialScreen_BtnV2GInfoGet(u8 port)
{
    u8 mode = 0x00;
    u32 mpara = 0x00;
    ...
    if(port == LCD_GUN_1){
        mode = *(UI_READ_SINGLE_CFG_DATA(CONFIG_ITEM_CURRENT_V2G_MODE_A, 0));
        mpara = *((u32*)(UI_READ_SINGLE_CFG_DATA(CONFIG_ITEM_V2G_MODE_PARAMETER_A, 0)));
    }else if(port == LCD_GUN_2){
        mode = *(UI_READ_SINGLE_CFG_DATA(CONFIG_ITEM_CURRENT_V2G_MODE_B, 0));
        mpara = *((u32*)(UI_READ_SINGLE_CFG_DATA(CONFIG_ITEM_V2G_MODE_PARAMETER_B, 0)));
    }else{
        return;
    }
    ...
    LcdData.setData.V2G_MSLimitMoney[LCD_GUN_NUM] = 0;
    LcdData.setData.V2G_MSLimitElect[LCD_GUN_NUM] = 0;
    LcdData.setData.V2G_MSLimitTiming[LCD_GUN_NUM] = 0;
    LcdData.setData.V2G_CurrentModePara[LCD_GUN_NUM] = 0;

    switch(mode){
    case CP_V2G_MODE_LIMIT_MONEY:
        LcdData.setData.V2G_CurrentMode[LCD_GUN_NUM][THAISEN_V2G_MODE_LIMIT_MONEY] = TRUE;
        if((mpara > CP_V2G_MODE_PARA_MONEY_MAX) || (mpara < CP_V2G_MODE_PARA_MONEY_MIN)){
            mpara = CP_V2G_MODE_PARA_MONEY_DEF;
        }
        LcdData.setData.V2G_CurrentModePara[LCD_GUN_NUM] = mpara;
        LcdData.setData.V2G_MSLimitMoney[LCD_GUN_NUM] = LcdData.setData.V2G_CurrentModePara[LCD_GUN_NUM];
        break;
    case CP_V2G_MODE_LIMIT_ELECT:
        LcdData.setData.V2G_CurrentMode[LCD_GUN_NUM][THAISEN_V2G_MODE_LIMIT_ELECT] = TRUE;
        if((mpara > CP_V2G_MODE_PARA_ELECT_MAX) || (mpara < CP_V2G_MODE_PARA_ELECT_MIN)){
            mpara = CP_V2G_MODE_PARA_ELECT_DEF;
        }
        LcdData.setData.V2G_CurrentModePara[LCD_GUN_NUM] = mpara;
        LcdData.setData.V2G_MSLimitElect[LCD_GUN_NUM] = LcdData.setData.V2G_CurrentModePara[LCD_GUN_NUM];
        break;
    case CP_V2G_MODE_LIMIT_TIMING:
        LcdData.setData.V2G_CurrentMode[LCD_GUN_NUM][THAISEN_V2G_MODE_LIMIT_TIMING] = TRUE;
        if(mpara > CP_V2G_MODE_PARA_TIMING_MAX){
            mpara = CP_V2G_MODE_PARA_TIMING_MAX;
        }
        LcdData.setData.V2G_CurrentModePara[LCD_GUN_NUM] = mpara;
        LcdData.setData.V2G_MSLimitTiming[LCD_GUN_NUM] = LcdData.setData.V2G_CurrentModePara[LCD_GUN_NUM];
        break;
    case CP_V2G_MODE_AUTO:
        LcdData.setData.V2G_CurrentMode[LCD_GUN_NUM][THAISEN_V2G_MODE_AUTO] = TRUE;
        LcdData.setData.V2G_CurrentModePara[LCD_GUN_NUM] = 0x00;
        break;
    default:
        LcdData.setData.V2G_CurrentMode[LCD_GUN_NUM][THAISEN_V2G_MODE_SIZE] = TRUE;
        LcdData.setData.V2G_CurrentModePara[LCD_GUN_NUM] = 0;
        break;
    }

    LcdData.setData.V2G_MSLimitMoney[port] = LcdData.setData.V2G_MSLimitMoney[LCD_GUN_NUM];
    LcdData.setData.V2G_MSLimitElect[port] = LcdData.setData.V2G_MSLimitElect[LCD_GUN_NUM];
    LcdData.setData.V2G_MSLimitTiming[port] = LcdData.setData.V2G_MSLimitTiming[LCD_GUN_NUM];
    LcdData.setData.V2G_CurrentModePara[port] = LcdData.setData.V2G_CurrentModePara[LCD_GUN_NUM];
    memcpy(LcdData.setData.V2G_CurrentMode[port], LcdData.setData.V2G_CurrentMode[LCD_GUN_NUM], sizeof(LcdData.setData.V2G_CurrentMode[port]));

    LcdData.setData.DisChargeWay[LCD_GUN_NUM] = LcdData.setData.DisChargeWay[port];
}
#endif /* SCREEN_USING_V2G */

void SerialScreen_BtnModeSelectBack(void)
{
    LcdData.setData.Modewarning = FALSE;
}
...
void SerialScreen_IsSupportGet(void)
{
    ...
    if(TRUE == *(u8 *)(UI_READ_SINGLE_CFG_DATA(CONFIG_ITEM_SUPORT_LOCAL_STOP, 0)))       /* 本地停止默认不启用 */
        LcdData.setData.sup_Local_stop = TRUE;
    ...
    if(TRUE == *(u8 *)(UI_READ_SINGLE_CFG_DATA(CONFIG_ITEM_SUPORT_PASSWORD_START, 0)))   /* 密码启动配置默认不启用 */
        LcdData.setData.sup_pw_start = TRUE;

    LcdData.setData.sup_offline_card = TRUE;
    if(FALSE == *(u8 *)(UI_READ_SINGLE_CFG_DATA(CONFIG_ITEM_SUPORT_OFFLINE_CARD, 0)))     /* 离线刷卡配置默认启用 */
        LcdData.setData.sup_offline_card = FALSE;

    LcdData.setData.sup_mode_select = FALSE;
    if(TRUE == *(u8 *)(UI_READ_SINGLE_CFG_DATA(CONFIG_ITEM_SUPORT_MODE_SELECT, 0)))     /* 模式选择配置默认关闭 */
        LcdData.setData.sup_mode_select = TRUE;
#ifdef SCREEN_USING_V2G
...
#ifdef SCREEN_USING_V2G
    LcdData.setData.Modewarning = FALSE;
    SerialScreen_SendIco(&SerialScreen, 0x65C4, LcdData.setData.Modewarning);

    if(LcdData.gunIndex == LCD_GUN_1){
        SerialScreen_BtnV2GInfoGet(LCD_GUN_2);
        SerialScreen_BtnV2GInfoGet(LCD_GUN_1);
    }else{
        SerialScreen_BtnV2GInfoGet(LCD_GUN_1);
        SerialScreen_BtnV2GInfoGet(LCD_GUN_2);
    }
...
void SerialScreen_GetIOStatusA(void)
{
    ...
    SerialScreen_SendTxt(&SerialScreen, 0x6715, LcdData.setData.selfCheck_Info[0], sizeof(LcdData.setData.selfCheck_Info[0]));
    rt_thread_mdelay(100);
    SerialScreen_SendTxt(&SerialScreen, 0x6735, LcdData.setData.selfCheck_Info[1], sizeof(LcdData.setData.selfCheck_Info[1]));
...
void SerialScreen_GetIOStatus(int port)
{
    ...
    if(thaisen_get_debug_mode() == thaisen_debug_mode_enter)
        return;
...
void SerialScreen_BtnParaSet1(void)
{
	...
	{
	    thaisenParallelRelay_1_Enable_Debug();
	}
...
void SerialScreen_BtnParaSet2(void)
{
    ...
    {
        thaisenParallelRelay_2_Enable_Debug();
    }
...
void SerialScreen_BtnParaSet3(void)
{
    ...
    {
        thaisenParallelRelay_3_Enable_Debug();
    }
...
void SerialScreen_BtnAuxSet(u8 port)
{
	...
	{
		if(port == LCD_GUN_1)
		    thaisenAux_A_12V_Disable_Debug();
		else if(port == LCD_GUN_2)
		    thaisenAux_B_12V_Disable_Debug();
	}
	else
	{
	if(port == LCD_GUN_1)
	    thaisenAux_A_12V_Enable_Debug();
	else if(port == LCD_GUN_2)
	    thaisenAux_B_12V_Enable_Debug();
	}
	sSCREEN_EVENT_DEBUGMSG("BtnFan%dSet = %d\r\n",port,LcdData.setData.s_auxRelay[port]);

}

void SerialScreen_EnterLiquid(void)
{
//    LcdData.debugIOflg = TRUE;
//    thaisen_set_debug_mode(1);
}
...
#ifdef SCREEN_USING_DOUBLE_GUN
        if(port == LCD_GUN_1)
            thaisenAux_A_24V_Enable_Debug();
        else if(port == LCD_GUN_2)
            thaisenAux_B_24V_Enable_Debug();
#endif /* SCREEN_USING_DOUBLE_GUN */
    }
}
...
void SerialScreen_AuxsetA_New()
{
    if(LcdAssistantData.Flag.IsEnableAuxPower24V == TRUE){
        if(LcdData.setData.s_selectaux[LCD_GUN_1] != ICON_AUXPOWER_NONE){
            if(LcdData.setData.s_selectaux[LCD_GUN_1] == (ICON_AUXPOWER_12V + 2 *LCD_GUN_1)){
                LcdAssistantData.SeveralGunFlag[LCD_GUN_1].Aux24VNotice = TRUE;
            }else{
                LcdData.setData.s_selectaux[LCD_GUN_1] = (ICON_AUXPOWER_12V + 2 *LCD_GUN_1);
                thaisenSetAuxPowerTypeA(thaisen_auxPowerType_12V);
...
void SerialScreen_AuxsetB_New()
{
    if(LcdAssistantData.Flag.IsEnableAuxPower24V == TRUE){
        if(LcdData.setData.s_selectaux[LCD_GUN_2] != ICON_AUXPOWER_NONE){
            if(LcdData.setData.s_selectaux[LCD_GUN_2] == (ICON_AUXPOWER_12V + 2 *LCD_GUN_2)){
                LcdAssistantData.SeveralGunFlag[LCD_GUN_2].Aux24VNotice = TRUE;
            }else{
                LcdData.setData.s_selectaux[LCD_GUN_2] = (ICON_AUXPOWER_12V + 2 *LCD_GUN_2);
                thaisenSetAuxPowerTypeB(thaisen_auxPowerType_12V);
...
{
#define SCREEN_WAIT_FB_TIME              200           /** 等待反馈状态时长(ms) */
#define SCREEN_INFO_SEND_INTERVAL        50            /** 自检信息发送间隔(ms) */
#define SCREEN_TEST_ITEM_INTERVAL        1500          /** 两个自检项间隔时长(ms) */
#define SCREEN_RET_SUCCESS               1             /** 自检结果：成功 */
#define SCREEN_RET_FAIL                  0             /** 自检结果：失败  */
    ...
    LcdData.setData.selfCheck_icon = TRUE;
    SerialScreen_SendIco(&SerialScreen, 0x6713, LcdData.setData.selfCheck_icon);
    ...
    if(fb[0] != TRUE){
        ret = SCREEN_RET_FAIL;
        thaisen_selfcheck_debug_info(THA_DEBUG_ITEM_ACRELAY_ON, language, 0, LcdData.setData.selfCheck_Info[index], sizeof(LcdData.setData.selfCheck_Info[index]));
    }else{
        thaisen_selfcheck_debug_info(THA_DEBUG_ITEM_ACRELAY_ON, language, 1, LcdData.setData.selfCheck_Info[index], sizeof(LcdData.setData.selfCheck_Info[index]));
    }
    /** 信息显示 */
    fb[0] += '0';
    ...
    if(fb[0] != FALSE){
        ret = SCREEN_RET_FAIL;
        thaisen_selfcheck_debug_info(THA_DEBUG_ITEM_ACRELAY_OFF, language, 0, LcdData.setData.selfCheck_Info[index], sizeof(LcdData.setData.selfCheck_Info[index]));
    }else{
        thaisen_selfcheck_debug_info(THA_DEBUG_ITEM_ACRELAY_OFF, language, 1, LcdData.setData.selfCheck_Info[index], sizeof(LcdData.setData.selfCheck_Info[index]));
    }
    /** 信息显示 */
    fb[0] += '0';
    SerialScreen_SendTxt(&SerialScreen, 0x4164, &fb[0], sizeof(fb[0]));
    ...
    ret = SCREEN_RET_SUCCESS;
    memset(LcdData.setData.selfCheck_Info[index], 0x00, sizeof(LcdData.setData.selfCheck_Info[index]));
    thaisenParallelRelay_1_Enable_Debug();              /** 闭合 */
    ...
    if(fb[0] != TRUE || fb[1] != TRUE){
        ret = SCREEN_RET_FAIL;
        thaisen_selfcheck_debug_info(THA_DEBUG_ITEM_PARARELAY_1_ON, language, 0, LcdData.setData.selfCheck_Info[index], sizeof(LcdData.setData.selfCheck_Info[index]));
    }else{
        thaisen_selfcheck_debug_info(THA_DEBUG_ITEM_PARARELAY_1_ON, language, 1, LcdData.setData.selfCheck_Info[index], sizeof(LcdData.setData.selfCheck_Info[index]));
    }
    /** 信息显示 */
    fb[0] += '0';
    fb[1] += '0';
    ...
    ret = SCREEN_RET_SUCCESS;
    memset(LcdData.setData.selfCheck_Info[index], 0x00, sizeof(LcdData.setData.selfCheck_Info[index]));
    thaisenParallelRelay_1_Disable_Debug();                     /** 断开 */
    ...
    if(fb[0] != FALSE || fb[1] != FALSE){
        ret = SCREEN_RET_FAIL;
        thaisen_selfcheck_debug_info(THA_DEBUG_ITEM_PARARELAY_1_OFF, language, 0, LcdData.setData.selfCheck_Info[index], sizeof(LcdData.setData.selfCheck_Info[index]));
    }else{
        thaisen_selfcheck_debug_info(THA_DEBUG_ITEM_PARARELAY_1_OFF, language, 1, LcdData.setData.selfCheck_Info[index], sizeof(LcdData.setData.selfCheck_Info[index]));
    }
    /** 信息显示 */
    fb[0] += '0';
    fb[1] += '0';
...
#else
    thaisen_selfcheck_debug_info(THA_DEBUG_ITEM_FAN_ON_A, language, 1, LcdData.setData.selfCheck_Info[index], sizeof(LcdData.setData.selfCheck_Info[index]));
#endif
    /** 信息显示 */
#if 0
    fb[0] += '0';
...
#else
    thaisen_selfcheck_debug_info(THA_DEBUG_ITEM_FAN_OFF_A, language, 1, LcdData.setData.selfCheck_Info[index], sizeof(LcdData.setData.selfCheck_Info[index]));
#endif
    /** 信息显示 */
#if 0
    fb[0] = '0';
...
#else
    thaisen_selfcheck_debug_info(THA_DEBUG_ITEM_FAN_ON_B, language, 1, LcdData.setData.selfCheck_Info[index], sizeof(LcdData.setData.selfCheck_Info[index]));
#endif
    /** 信息显示 */
#if 0
    fb[0] += '0';
...
#else
    thaisen_selfcheck_debug_info(THA_DEBUG_ITEM_FAN_OFF_B, language, 1, LcdData.setData.selfCheck_Info[index], sizeof(LcdData.setData.selfCheck_Info[index]));
#endif
    /** 信息显示 */
#if 0
    fb[0] = '0';
...
//        goto Check_end;
    ...
    if(thaisenGetChargGunRunType() == thaisenDeviceType_average){
        memset(LcdData.setData.selfCheck_Info[index], 0x00, sizeof(LcdData.setData.selfCheck_Info[index]));
        thaisenParallelRelay_2_Enable_Debug();                   /** 闭合 */
        ...
        if(fb[0] != TRUE || fb[1] != TRUE){
            ret = SCREEN_RET_FAIL;
            thaisen_selfcheck_debug_info(THA_DEBUG_ITEM_PARARELAY_2_ON, language, 0, LcdData.setData.selfCheck_Info[index], sizeof(LcdData.setData.selfCheck_Info[index]));
        }else{
            thaisen_selfcheck_debug_info(THA_DEBUG_ITEM_PARARELAY_2_ON, language, 1, LcdData.setData.selfCheck_Info[index], sizeof(LcdData.setData.selfCheck_Info[index]));
        }
        /** 信息显示 */
        fb[0] += '0';
        fb[1] += '0';
        ...
        ret = SCREEN_RET_SUCCESS;
        memset(LcdData.setData.selfCheck_Info[index], 0x00, sizeof(LcdData.setData.selfCheck_Info[index]));
        thaisenParallelRelay_2_Disable_Debug();                           /** 断开 */
        ...
        if(fb[0] != FALSE || fb[1] != FALSE){
            ret = SCREEN_RET_FAIL;
            thaisen_selfcheck_debug_info(THA_DEBUG_ITEM_PARARELAY_2_OFF, language, 0, LcdData.setData.selfCheck_Info[index], sizeof(LcdData.setData.selfCheck_Info[index]));
        }else{
            thaisen_selfcheck_debug_info(THA_DEBUG_ITEM_PARARELAY_2_OFF, language, 1, LcdData.setData.selfCheck_Info[index], sizeof(LcdData.setData.selfCheck_Info[index]));
        }
        /** 信息显示 */
        fb[0] += '0';
        fb[1] += '0';
    ...
    if(thaisenGetChargGunRunType() == thaisenDeviceType_average){
        memset(LcdData.setData.selfCheck_Info[index], 0x00, sizeof(LcdData.setData.selfCheck_Info[index]));
        thaisenParallelRelay_3_Enable_Debug();                   /** 闭合 */
        ...
        if(fb[0] != TRUE || fb[1] != TRUE){
            ret = SCREEN_RET_FAIL;
            thaisen_selfcheck_debug_info(THA_DEBUG_ITEM_PARARELAY_3_ON, language, 0, LcdData.setData.selfCheck_Info[index], sizeof(LcdData.setData.selfCheck_Info[index]));
        }else{
            thaisen_selfcheck_debug_info(THA_DEBUG_ITEM_PARARELAY_3_ON, language, 1, LcdData.setData.selfCheck_Info[index], sizeof(LcdData.setData.selfCheck_Info[index]));
        }
        /** 信息显示 */
        fb[0] += '0';
        fb[1] += '0';
        ...
        ret = SCREEN_RET_SUCCESS;
        memset(LcdData.setData.selfCheck_Info[index], 0x00, sizeof(LcdData.setData.selfCheck_Info[index]));
        thaisenParallelRelay_3_Disable_Debug();                           /** 断开 */
        ...
        if(fb[0] != FALSE || fb[1] != FALSE){
            ret = SCREEN_RET_FAIL;
            thaisen_selfcheck_debug_info(THA_DEBUG_ITEM_PARARELAY_3_OFF, language, 0, LcdData.setData.selfCheck_Info[index], sizeof(LcdData.setData.selfCheck_Info[index]));
        }else{
            thaisen_selfcheck_debug_info(THA_DEBUG_ITEM_PARARELAY_3_OFF, language, 1, LcdData.setData.selfCheck_Info[index], sizeof(LcdData.setData.selfCheck_Info[index]));
        }
        /** 信息显示 */
        fb[0] += '0';
        fb[1] += '0';
    ...
    ret = SCREEN_RET_SUCCESS;
    memset(LcdData.setData.selfCheck_Info[index], 0x00, sizeof(LcdData.setData.selfCheck_Info[index]));
    thaisenDcRelay_A_Enable_Debug();                   /** 闭合 */
    ...
    if(fb[0] != TRUE || fb[1] != TRUE){
        ret = SCREEN_RET_FAIL;
        thaisen_selfcheck_debug_info(THA_DEBUG_ITEM_DCRELAY_A_ON, language, 0, LcdData.setData.selfCheck_Info[index], sizeof(LcdData.setData.selfCheck_Info[index]));
    }else{
        thaisen_selfcheck_debug_info(THA_DEBUG_ITEM_DCRELAY_A_ON, language, 1, LcdData.setData.selfCheck_Info[index], sizeof(LcdData.setData.selfCheck_Info[index]));
    }
    /** 信息显示 */
    fb[0] += '0';
    fb[1] += '0';
    ...
    ret = SCREEN_RET_SUCCESS;
    memset(LcdData.setData.selfCheck_Info[index], 0x00, sizeof(LcdData.setData.selfCheck_Info[index]));
    thaisenDcRelay_A_Disable_Debug();                           /** 断开 */
    ...
    if(fb[0] != FALSE || fb[1] != FALSE){
        ret = SCREEN_RET_FAIL;
        thaisen_selfcheck_debug_info(THA_DEBUG_ITEM_DCRELAY_A_OFF, language, 0, LcdData.setData.selfCheck_Info[index], sizeof(LcdData.setData.selfCheck_Info[index]));
    }else{
        thaisen_selfcheck_debug_info(THA_DEBUG_ITEM_DCRELAY_A_OFF, language, 1, LcdData.setData.selfCheck_Info[index], sizeof(LcdData.setData.selfCheck_Info[index]));
    }
    /** 信息显示 */
    fb[0] += '0';
    fb[1] += '0';
    ...
    ret = SCREEN_RET_SUCCESS;
    memset(LcdData.setData.selfCheck_Info[index], 0x00, sizeof(LcdData.setData.selfCheck_Info[index]));
    thaisenDcRelay_B_Enable_Debug();                   /** 闭合 */
    ...
    if(fb[0] != TRUE || fb[1] != TRUE){
        ret = SCREEN_RET_FAIL;
        thaisen_selfcheck_debug_info(THA_DEBUG_ITEM_DCRELAY_B_ON, language, 0, LcdData.setData.selfCheck_Info[index], sizeof(LcdData.setData.selfCheck_Info[index]));
    }else{
        thaisen_selfcheck_debug_info(THA_DEBUG_ITEM_DCRELAY_B_ON, language, 1, LcdData.setData.selfCheck_Info[index], sizeof(LcdData.setData.selfCheck_Info[index]));
    }
    /** 信息显示 */
    fb[0] += '0';
    fb[1] += '0';
    ...
    ret = SCREEN_RET_SUCCESS;
    memset(LcdData.setData.selfCheck_Info[index], 0x00, sizeof(LcdData.setData.selfCheck_Info[index]));
    thaisenDcRelay_B_Disable_Debug();                           /** 断开 */
    ...
    if(fb[0] != FALSE || fb[1] != FALSE){
        ret = SCREEN_RET_FAIL;
        thaisen_selfcheck_debug_info(THA_DEBUG_ITEM_DCRELAY_B_OFF, language, 0, LcdData.setData.selfCheck_Info[index], sizeof(LcdData.setData.selfCheck_Info[index]));
    }else{
        thaisen_selfcheck_debug_info(THA_DEBUG_ITEM_DCRELAY_B_OFF, language, 1, LcdData.setData.selfCheck_Info[index], sizeof(LcdData.setData.selfCheck_Info[index]));
    }
    /** 信息显示 */
    fb[0] += '0';
    fb[1] += '0';
...
//        rt_thread_mdelay(SCREEN_INFO_SEND_INTERVAL);
    ...
    ret = SCREEN_RET_SUCCESS;
    memset(LcdData.setData.selfCheck_Info[index], 0x00, sizeof(LcdData.setData.selfCheck_Info[index]));
    thaisenElectLockA_Directly();                  /** 上锁 */
    SerialScreen_SendIco(&SerialScreen, 0x4178, TRUE);
    rt_thread_mdelay(2000);                        /** 等待状态 */
    ...
    if(fb[0] != TRUE){
        ...
        thaisen_selfcheck_debug_info(THA_DEBUG_ITEM_ELOCK_A_ON, language, 1, LcdData.setData.selfCheck_Info[index], sizeof(LcdData.setData.selfCheck_Info[index]));
    }
    /** 信息显示 */
    fb[0] += '0';
    ...
    ret = SCREEN_RET_SUCCESS;
    memset(LcdData.setData.selfCheck_Info[index], 0x00, sizeof(LcdData.setData.selfCheck_Info[index]));
    thaisenElectUnlockA_Directly();                /** 解锁 */
    ...
    if(fb[0] != FALSE){
        ...
        thaisen_selfcheck_debug_info(THA_DEBUG_ITEM_ELOCK_A_OFF, language, 1, LcdData.setData.selfCheck_Info[index], sizeof(LcdData.setData.selfCheck_Info[index]));
    }
    /** 信息显示 */
    fb[0] += '0';
    SerialScreen_SendTxt(&SerialScreen, 0x416A, &fb[0], sizeof(fb[0]));
    ...
    if(fb[0] != TRUE){
        ...
        thaisen_selfcheck_debug_info(THA_DEBUG_ITEM_ELOCK_B_ON, language, 1, LcdData.setData.selfCheck_Info[index], sizeof(LcdData.setData.selfCheck_Info[index]));
    }
    /** 信息显示 */
    fb[0] += '0';
    ...
    ret = SCREEN_RET_SUCCESS;
    memset(LcdData.setData.selfCheck_Info[index], 0x00, sizeof(LcdData.setData.selfCheck_Info[index]));
    thaisenElectUnlockB_Directly();                /** 解锁 */
    ...
    if(fb[0] != FALSE){
        ...
        thaisen_selfcheck_debug_info(THA_DEBUG_ITEM_ELOCK_B_OFF, language, 1, LcdData.setData.selfCheck_Info[index], sizeof(LcdData.setData.selfCheck_Info[index]));
    }
    /** 信息显示 */
    fb[0] += '0';
    SerialScreen_SendTxt(&SerialScreen, 0x516A, &fb[0], sizeof(fb[0]));
    ...
    ret = SCREEN_RET_SUCCESS;
    memset(LcdData.setData.selfCheck_Info[index], 0x00, sizeof(LcdData.setData.selfCheck_Info[index]));
    thaisenAux_A_12V_Enable_Debug();                      /** 闭合 */
    ...
    if(fb[0] != TRUE){
        ret = SCREEN_RET_FAIL;
        thaisen_selfcheck_debug_info(THA_DEBUG_ITEM_AUX12V_A_ON, language, 0, LcdData.setData.selfCheck_Info[index], sizeof(LcdData.setData.selfCheck_Info[index]));
    }else{
        thaisen_selfcheck_debug_info(THA_DEBUG_ITEM_AUX12V_A_ON, language, 1, LcdData.setData.selfCheck_Info[index], sizeof(LcdData.setData.selfCheck_Info[index]));
    }
#else
    thaisen_selfcheck_debug_info(THA_DEBUG_ITEM_AUX12V_A_ON, language, 1, LcdData.setData.selfCheck_Info[index], sizeof(LcdData.setData.selfCheck_Info[index]));
#endif
...
//        goto Check_end;
    ...
    ret = SCREEN_RET_SUCCESS;
    memset(LcdData.setData.selfCheck_Info[index], 0x00, sizeof(LcdData.setData.selfCheck_Info[index]));
    thaisenAux_A_12V_Disable_Debug();                     /** 断开 */
    ...
    if(fb[0] != FALSE){
        ret = SCREEN_RET_FAIL;
        thaisen_selfcheck_debug_info(THA_DEBUG_ITEM_AUX12V_A_OFF, language, 0, LcdData.setData.selfCheck_Info[index], sizeof(LcdData.setData.selfCheck_Info[index]));
    }else{
        thaisen_selfcheck_debug_info(THA_DEBUG_ITEM_AUX12V_A_OFF, language, 1, LcdData.setData.selfCheck_Info[index], sizeof(LcdData.setData.selfCheck_Info[index]));
    }
#else
    thaisen_selfcheck_debug_info(THA_DEBUG_ITEM_AUX12V_A_OFF, language, 1, LcdData.setData.selfCheck_Info[index], sizeof(LcdData.setData.selfCheck_Info[index]));
#endif
...
//        goto Check_end;
    ...
    ret = SCREEN_RET_SUCCESS;
    memset(LcdData.setData.selfCheck_Info[index], 0x00, sizeof(LcdData.setData.selfCheck_Info[index]));
    thaisenAux_B_12V_Enable_Debug();                      /** 闭合 */
    ...
    if(fb[0] != TRUE){
        ret = SCREEN_RET_FAIL;
        thaisen_selfcheck_debug_info(THA_DEBUG_ITEM_AUX12V_B_ON, language, 0, LcdData.setData.selfCheck_Info[index], sizeof(LcdData.setData.selfCheck_Info[index]));
    }else{
        thaisen_selfcheck_debug_info(THA_DEBUG_ITEM_AUX12V_B_ON, language, 1, LcdData.setData.selfCheck_Info[index], sizeof(LcdData.setData.selfCheck_Info[index]));
    }
#else
    thaisen_selfcheck_debug_info(THA_DEBUG_ITEM_AUX12V_B_ON, language, 1, LcdData.setData.selfCheck_Info[index], sizeof(LcdData.setData.selfCheck_Info[index]));
#endif
...
//        goto Check_end;
    ...
    ret = SCREEN_RET_SUCCESS;
    memset(LcdData.setData.selfCheck_Info[index], 0x00, sizeof(LcdData.setData.selfCheck_Info[index]));
    thaisenAux_B_12V_Disable_Debug();                     /** 断开 */
    ...
    if(fb[0] != FALSE){
        ret = SCREEN_RET_FAIL;
        thaisen_selfcheck_debug_info(THA_DEBUG_ITEM_AUX12V_B_OFF, language, 0, LcdData.setData.selfCheck_Info[index], sizeof(LcdData.setData.selfCheck_Info[index]));
    }else{
        thaisen_selfcheck_debug_info(THA_DEBUG_ITEM_AUX12V_B_OFF, language, 1, LcdData.setData.selfCheck_Info[index], sizeof(LcdData.setData.selfCheck_Info[index]));
    }
#else
    thaisen_selfcheck_debug_info(THA_DEBUG_ITEM_AUX12V_B_OFF, language, 1, LcdData.setData.selfCheck_Info[index], sizeof(LcdData.setData.selfCheck_Info[index]));
#endif
...
//        goto Check_end;
    ...
    ret = SCREEN_RET_SUCCESS;
    memset(LcdData.setData.selfCheck_Info[index], 0x00, sizeof(LcdData.setData.selfCheck_Info[index]));
    thaisenAux_A_24V_Enable_Debug();                      /** 闭合 */
    ...
    if(fb[0] != TRUE){
        ret = SCREEN_RET_FAIL;
        thaisen_selfcheck_debug_info(THA_DEBUG_ITEM_AUX24V_A_ON, language, 0, LcdData.setData.selfCheck_Info[index], sizeof(LcdData.setData.selfCheck_Info[index]));
    }else{
        thaisen_selfcheck_debug_info(THA_DEBUG_ITEM_AUX24V_A_ON, language, 1, LcdData.setData.selfCheck_Info[index], sizeof(LcdData.setData.selfCheck_Info[index]));
    }
#else
    thaisen_selfcheck_debug_info(THA_DEBUG_ITEM_AUX24V_A_ON, language, 1, LcdData.setData.selfCheck_Info[index], sizeof(LcdData.setData.selfCheck_Info[index]));
#endif
...
//        goto Check_end;
    ...
    ret = SCREEN_RET_SUCCESS;
    memset(LcdData.setData.selfCheck_Info[index], 0x00, sizeof(LcdData.setData.selfCheck_Info[index]));
    thaisenAux_A_24V_Disable_Debug();                     /** 断开 */
    ...
    if(fb[0] != FALSE){
        ret = SCREEN_RET_FAIL;
        thaisen_selfcheck_debug_info(THA_DEBUG_ITEM_AUX24V_A_OFF, language, 0, LcdData.setData.selfCheck_Info[index], sizeof(LcdData.setData.selfCheck_Info[index]));
    }else{
        thaisen_selfcheck_debug_info(THA_DEBUG_ITEM_AUX24V_A_OFF, language, 1, LcdData.setData.selfCheck_Info[index], sizeof(LcdData.setData.selfCheck_Info[index]));
    }
#else
    thaisen_selfcheck_debug_info(THA_DEBUG_ITEM_AUX24V_A_OFF, language, 1, LcdData.setData.selfCheck_Info[index], sizeof(LcdData.setData.selfCheck_Info[index]));
#endif
...
//        goto Check_end;
    ...
    ret = SCREEN_RET_SUCCESS;
    memset(LcdData.setData.selfCheck_Info[index], 0x00, sizeof(LcdData.setData.selfCheck_Info[index]));
    thaisenAux_B_24V_Enable_Debug();                      /** 闭合 */
    ...
    if(fb[0] != TRUE){
        ret = SCREEN_RET_FAIL;
        thaisen_selfcheck_debug_info(THA_DEBUG_ITEM_AUX24V_B_ON, language, 0, LcdData.setData.selfCheck_Info[index], sizeof(LcdData.setData.selfCheck_Info[index]));
    }else{
        thaisen_selfcheck_debug_info(THA_DEBUG_ITEM_AUX24V_B_ON, language, 1, LcdData.setData.selfCheck_Info[index], sizeof(LcdData.setData.selfCheck_Info[index]));
    }
#else
    thaisen_selfcheck_debug_info(THA_DEBUG_ITEM_AUX24V_B_ON, language, 1, LcdData.setData.selfCheck_Info[index], sizeof(LcdData.setData.selfCheck_Info[index]));
#endif
...
//        goto Check_end;
    ...
    ret = SCREEN_RET_SUCCESS;
    memset(LcdData.setData.selfCheck_Info[index], 0x00, sizeof(LcdData.setData.selfCheck_Info[index]));
    thaisenAux_B_24V_Disable_Debug();                     /** 断开 */
    ...
    if(fb[0] != FALSE){
        ret = SCREEN_RET_FAIL;
        thaisen_selfcheck_debug_info(THA_DEBUG_ITEM_AUX24V_B_OFF, language, 0, LcdData.setData.selfCheck_Info[index], sizeof(LcdData.setData.selfCheck_Info[index]));
    }else{
        thaisen_selfcheck_debug_info(THA_DEBUG_ITEM_AUX24V_B_OFF, language, 1, LcdData.setData.selfCheck_Info[index], sizeof(LcdData.setData.selfCheck_Info[index]));
    }
#else
    thaisen_selfcheck_debug_info(THA_DEBUG_ITEM_AUX24V_B_OFF, language, 1, LcdData.setData.selfCheck_Info[index], sizeof(LcdData.setData.selfCheck_Info[index]));
#endif
...
Check_end:
    ...
    /*************************************************** 自检完成 ***************************************************/
    memset(LcdData.setData.selfCheck_Info[index], 0x00, sizeof(LcdData.setData.selfCheck_Info[index]));
    thaisen_selfcheck_debug_info(THA_DEBUG_ITEM_COMPLETE, language, 1, LcdData.setData.selfCheck_Info[index], sizeof(LcdData.setData.selfCheck_Info[index]));
...
#undef SCREEN_WAIT_FB_TIME
#undef SCREEN_INFO_SEND_INTERVAL
#undef SCREEN_TEST_ITEM_INTERVAL
#undef SCREEN_RET_SUCCESS
#undef SCREEN_RET_FAIL
}

void SerialScreen_BtnModeLimitMoneySet(void)
{
    if(LcdData.setData.CurrentMode[LCD_GUN_NUM][THAISEN_CHARGE_MODE_LIMIT_MONEY] == TRUE){
        LcdData.setData.CurrentMode[LCD_GUN_NUM][THAISEN_CHARGE_MODE_LIMIT_MONEY] = FALSE;
    }else{
        memset(LcdData.setData.CurrentMode[LCD_GUN_NUM], 0x00, sizeof(LcdData.setData.CurrentMode[LCD_GUN_NUM]));
        LcdData.setData.CurrentMode[LCD_GUN_NUM][THAISEN_CHARGE_MODE_LIMIT_MONEY] = TRUE;
    }
...
{
    if(LcdData.setData.CurrentMode[LCD_GUN_NUM][THAISEN_CHARGE_MODE_LIMIT_ELECT] == TRUE){
        LcdData.setData.CurrentMode[LCD_GUN_NUM][THAISEN_CHARGE_MODE_LIMIT_ELECT] = FALSE;
    }else{
        memset(LcdData.setData.CurrentMode[LCD_GUN_NUM], 0x00, sizeof(LcdData.setData.CurrentMode[LCD_GUN_NUM]));
        LcdData.setData.CurrentMode[LCD_GUN_NUM][THAISEN_CHARGE_MODE_LIMIT_ELECT] = TRUE;
    }
...
{
    if(LcdData.setData.CurrentMode[LCD_GUN_NUM][THAISEN_CHARGE_MODE_FULL] == TRUE){
        LcdData.setData.CurrentMode[LCD_GUN_NUM][THAISEN_CHARGE_MODE_FULL] = FALSE;
    }else{
        memset(LcdData.setData.CurrentMode[LCD_GUN_NUM], 0x00, sizeof(LcdData.setData.CurrentMode[LCD_GUN_NUM]));
        LcdData.setData.CurrentMode[LCD_GUN_NUM][THAISEN_CHARGE_MODE_FULL] = TRUE;
    }
...
{
    if(LcdData.setData.CurrentMode[LCD_GUN_NUM][THAISEN_CHARGE_MODE_LIMIT_TIMING] == TRUE){
        LcdData.setData.CurrentMode[LCD_GUN_NUM][THAISEN_CHARGE_MODE_LIMIT_TIMING] = FALSE;
    }else{
        memset(LcdData.setData.CurrentMode[LCD_GUN_NUM], 0x00, sizeof(LcdData.setData.CurrentMode[LCD_GUN_NUM]));
        LcdData.setData.CurrentMode[LCD_GUN_NUM][THAISEN_CHARGE_MODE_LIMIT_TIMING] = TRUE;
    }
...
{
    if(LcdData.setData.CurrentMode[LCD_GUN_NUM][THAISEN_CHARGE_MODE_LIMIT_RESERVATION] == TRUE){
        LcdData.setData.CurrentMode[LCD_GUN_NUM][THAISEN_CHARGE_MODE_LIMIT_RESERVATION] = FALSE;
    }else{
        memset(LcdData.setData.CurrentMode[LCD_GUN_NUM], 0x00, sizeof(LcdData.setData.CurrentMode[LCD_GUN_NUM]));
        LcdData.setData.CurrentMode[LCD_GUN_NUM][THAISEN_CHARGE_MODE_LIMIT_RESERVATION] = TRUE;
    }
...
{
#ifdef SCREEN_USING_V2G
    if(LcdData.setData.V2G_CurrentMode[LCD_GUN_NUM][THAISEN_V2G_MODE_LIMIT_MONEY] == TRUE){
        LcdData.setData.V2G_CurrentMode[LCD_GUN_NUM][THAISEN_V2G_MODE_LIMIT_MONEY] = FALSE;
    }else{
        memset(LcdData.setData.V2G_CurrentMode[LCD_GUN_NUM], 0x00, sizeof(LcdData.setData.V2G_CurrentMode[LCD_GUN_NUM]));
        LcdData.setData.V2G_CurrentMode[LCD_GUN_NUM][THAISEN_V2G_MODE_LIMIT_MONEY] = TRUE;
    }
...
{
#ifdef SCREEN_USING_V2G
    if(LcdData.setData.V2G_CurrentMode[LCD_GUN_NUM][THAISEN_V2G_MODE_LIMIT_ELECT] == TRUE){
        LcdData.setData.V2G_CurrentMode[LCD_GUN_NUM][THAISEN_V2G_MODE_LIMIT_ELECT] = FALSE;
    }else{
        memset(LcdData.setData.V2G_CurrentMode[LCD_GUN_NUM], 0x00, sizeof(LcdData.setData.V2G_CurrentMode[LCD_GUN_NUM]));
        LcdData.setData.V2G_CurrentMode[LCD_GUN_NUM][THAISEN_V2G_MODE_LIMIT_ELECT] = TRUE;
    }
...
{
#ifdef SCREEN_USING_V2G
    if(LcdData.setData.V2G_CurrentMode[LCD_GUN_NUM][THAISEN_V2G_MODE_AUTO] == TRUE){
        LcdData.setData.V2G_CurrentMode[LCD_GUN_NUM][THAISEN_V2G_MODE_AUTO] = FALSE;
    }else{
        memset(LcdData.setData.V2G_CurrentMode[LCD_GUN_NUM], 0x00, sizeof(LcdData.setData.V2G_CurrentMode[LCD_GUN_NUM]));
        LcdData.setData.V2G_CurrentMode[LCD_GUN_NUM][THAISEN_V2G_MODE_AUTO] = TRUE;
    }
...
{
#ifdef SCREEN_USING_V2G
    if(LcdData.setData.V2G_CurrentMode[LCD_GUN_NUM][THAISEN_V2G_MODE_LIMIT_TIMING] == TRUE){
        LcdData.setData.V2G_CurrentMode[LCD_GUN_NUM][THAISEN_V2G_MODE_LIMIT_TIMING] = FALSE;
    }else{
        memset(LcdData.setData.V2G_CurrentMode[LCD_GUN_NUM], 0x00, sizeof(LcdData.setData.V2G_CurrentMode[LCD_GUN_NUM]));
        LcdData.setData.V2G_CurrentMode[LCD_GUN_NUM][THAISEN_V2G_MODE_LIMIT_TIMING] = TRUE;
    }
...
#ifdef SCREEN_USING_V2G
#if 0
    if(LcdData.setData.V2G_CurrentMode[LCD_GUN_NUM][THAISEN_V2G_MODE_LIMIT_RESERVATION] == TRUE){
        LcdData.setData.V2G_CurrentMode[LCD_GUN_NUM][THAISEN_V2G_MODE_LIMIT_RESERVATION] = FALSE;
    }else{
        memset(LcdData.setData.V2G_CurrentMode[LCD_GUN_NUM], 0x00, sizeof(LcdData.setData.V2G_CurrentMode[LCD_GUN_NUM]));
        LcdData.setData.V2G_CurrentMode[LCD_GUN_NUM][THAISEN_V2G_MODE_LIMIT_RESERVATION] = TRUE;
    }
#endif
#endif /* SCREEN_USING_V2G */
}

void SerialScreen_BtnV2G_On_ContravariantSet(int port)
{
#ifdef SCREEN_USING_V2G
    if(port >= LCD_GUN_NUM)
        return;
    /** 开启了V2G功能 */
    if(LcdData.setData.sup_V2G == TRUE){
        LcdData.setData.DisChargeWay[port] = SS_DISCHARGE_WAY_ON_CONTRAVARIANT;
    }else{
        LcdData.setData.DisChargeWay[port] = SS_DISCHARGE_WAY_NULL;
    }
    LcdData.setData.DisChargeWay[LCD_GUN_NUM] = LcdData.setData.DisChargeWay[port];
#endif /* SCREEN_USING_V2G */
}

void SerialScreen_BtnV2G_Off_ContravariantSet(int port)
{
#ifdef SCREEN_USING_V2G
    if(port >= LCD_GUN_NUM)
        return;
...
#define SERIALSCREEN_AMMETER_BAUD_115200     4      //电表波特率：115200
    ...
    LcdData.setData.sup_SupProtocolGB_T = *((u8*) UI_READ_SINGLE_CFG_DATA(CONFIG_ITEM_SUPORT_PROTOCOL_GB_T, 0));
    LcdData.setData.sup_SupSeveralFrame = *((u8*) UI_READ_SINGLE_CFG_DATA(CONFIG_ITEM_SUPORT_BMS_SEVERAL_FRAME, 0));
#ifdef SCREEN_USING_V2G
...
#ifdef SCREEN_USING_V2G
    if(LcdData.setData.sup_V2G > TRUE)           /* V2G默认不启用 */
        LcdData.setData.sup_V2G = FALSE;
#endif /* SCREEN_USING_V2G */
    ...
    if(LcdData.setData.sup_SupProtocolGB_T == CONFIG_ENABLE_ENUM){          /* 国标协议(27930)默认关闭 */
        LcdData.setData.sup_SupProtocolGB_T = TRUE;
    }else if(LcdData.setData.sup_SupProtocolGB_T == CONFIG_DISABLE_ENUM){
        LcdData.setData.sup_SupProtocolGB_T = FALSE;
    }else{
        LcdData.setData.sup_SupProtocolGB_T = FALSE;
    }

    if(LcdData.setData.sup_SupSeveralFrame == CONFIG_ENABLE_ENUM){          /* BMS多帧默认启用 */
        LcdData.setData.sup_SupSeveralFrame = TRUE;
    }else if(LcdData.setData.sup_SupSeveralFrame == CONFIG_DISABLE_ENUM){
        LcdData.setData.sup_SupSeveralFrame = FALSE;
    }else{
        LcdData.setData.sup_SupSeveralFrame = TRUE;
    }
...
//    thaisenSetAllocateStrategy(LcdData.setData.AllocWay);
	...
	//	LcdData.gun[i].Aux12v = 1;
	    ...
	    if(SerialScreen_Screen_GetCurrentChargeMode(i) == THAISEN_CHARGE_MODE_LIMIT_RESERVATION){
    ...
    if(LcdData.setData.sup_V2G == TRUE){
        for(u8 i = 0; i <= LCD_GUN_NUM; i++){
            LcdData.setData.DisChargeWay[i] = SS_DISCHARGE_WAY_OFF_CONTRAVARIANT;
        }
    }else{
        for(u8 i = 0; i <= LCD_GUN_NUM; i++){
            LcdData.setData.DisChargeWay[i] = SS_DISCHARGE_WAY_NULL;
        }
    }
#endif /* SCREEN_USING_V2G */
		...
		else
		{
			...
			if(pPageIndex->item[i].type == LCD_inputPwdType)
			{
				...
				if(pPageIndex->item[i].regaddr == keyreg)
				{
                    ...
                    if(strlen((char*)LcdData.KeyInput) == strlen((char*)LcdData.setData.UserPasswd)){
                    ...
                    if((is_pw_correct == 0) && (strlen((char*)LcdData.KeyInput) == 4)){   /** 超级密码是4位 */
                        u8 super_pw[5];   /** 超级密码 */
                        struct tm _tm;
...
#if (defined(SCREEN_USING_V2G) || defined(SCREEN_USING_OFFLINE_BILLING))
                            else if(LcdData.CurrentPage == LCD_PAGE_OFFLINE_BILLING){
                                SerialScreen_CombineData(pPageIndex->item[i].valtype,pPageIndex->item[i].valaddr,LcdData.KeyInput,4,(1 <<0));
                            }
#endif /* (defined(SCREEN_USING_V2G) || defined(SCREEN_USING_OFFLINE_BILLING)) */
#ifdef SCREEN_USING_V2G
                            else if(LcdData.CurrentPage == LCD_PAGE_DISCHARGE_MODE){
                                SerialScreen_CombineData(pPageIndex->item[i].valtype,pPageIndex->item[i].valaddr,LcdData.KeyInput,4,(1 <<0));
                            }
#endif /* SCREEN_USING_V2G */
						    else{
			...
			//页面切换
				...
				||(pPageIndex->item[i].type == LCD_ModeSelectBack))
			{
					...
					&&(pPageIndex->item[i].regaddr == keyreg))
				{
					...
					//如果目标页面为0，未知
					{
					    u8 page_selected = 0;
                         ...
                         if(LcdData.CurrentPage == LCD_PAGE_STANDBY){        /* 只有第一页能进入系统信息页面 */
                             if(page_selected == 0){
                                 if((keyreg == 0x1003) && (keyval == 0x0001)){
                                     ...
                                     LcdData.CurrentPage = pPageIndex->item[i].vallen;
                                     page_selected = 1;
                                 }else if((keyreg == SCREEN_AUX_A_SELCT_REG_NEW) && (keyval == SCREEN_AUX_A_SELCT_KEY_NEW)){
                                     if(LcdAssistantData.SeveralGunFlag[LCD_GUN_1].Aux24VNotice != TRUE){
                                         LcdData.CurrentPage = pPageIndex->item[i].vallen;
                                         page_selected = 1;
                                     }
                                 }else if((keyreg == SCREEN_AUX_B_SELCT_REG_NEW) && (keyval == SCREEN_AUX_B_SELCT_KEY_NEW)){
                                     if(LcdAssistantData.SeveralGunFlag[LCD_GUN_2].Aux24VNotice != TRUE){
                                         LcdData.CurrentPage = pPageIndex->item[i].vallen;
                                         page_selected = 1;
                                     }
                          ...
                          }else if(LcdData.CurrentPage == LCD_PAGE_A_SELECT){
                              if(page_selected == 0){
                                  if(((keyreg == 0x1003) && (keyval == 0x0069)) && (page_selected == 0)){        //A枪模式选择
                                      if(LcdData.setData.sup_mode_select){
                                          LcdData.CurrentPage = pPageIndex->item[i].vallen;
                                          page_selected = 1;
                                      }
                         ...
                         }else if(LcdData.CurrentPage == LCD_PAGE_B_SELECT){
                             if(page_selected == 0){
                                 if(((keyreg == 0x1003) && (keyval == 0x006A)) && (page_selected == 0)){        //B枪模式选择
                                     if(LcdData.setData.sup_mode_select){
                                         LcdData.CurrentPage = pPageIndex->item[i].vallen;
                                         page_selected = 1;
                                     }
...
#ifdef SCREEN_USING_V2G
                         else if(LcdData.CurrentPage == LCD_PAGE_MENU_MODE_SELECT){
                              if(page_selected == 0){
                                  /** 点击了放电模式 */
                                  if((keyreg == 0x1003) && (keyval == 0x006C)){
                                      if(LcdData.setData.sup_V2G){
                                          LcdData.CurrentPage = pPageIndex->item[i].vallen;
                                          page_selected = 1;
                                      }
...
//	u32 timesec = thaisen_app_get_current_timestamp();
	...
	for(i=0;i<LCD_GUN_NUM;i++)
	{
        ...
        if(LcdData.gun[i].workState != SysMainStatus_PlugIn){
            LcdAssistantData.SeveralGunFlag[i].Aux24VNotice = FALSE;
        }
        if(LcdAssistantData.SeveralGunFlag[i].Aux24VNotice){
            LcdData.setData.selectAuxNotice[i] = TRUE;
        }else{
            LcdData.setData.selectAuxNotice[i] = FALSE;
        }
	}
	...
	if(++timesecbak>1000/BASE_SYS_TIMER)
	{
        ...
        if(LcdData.setData.sup_mode_select){
            enum thaisen_mode mode = 0;
            if(LcdData.gunIndex == LCD_GUN_1){
#ifdef SCREEN_USING_V2G
                if((LcdData.runData.GunRunMode[LCD_GUN_1] == THAISEN_GUN_RUNING_MODE_V2G) && (SerialScreen_Screen_GetCurrentV2GMode(LCD_GUN_1) >= THAISEN_V2G_MODE_SIZE)){
                    LcdData.runData.GunRunMode[LCD_GUN_1] = THAISEN_GUN_RUNING_MODE_CHARGE;
                }
                if(LcdData.setData.sup_V2G == FALSE){
                    LcdData.runData.GunRunMode[LCD_GUN_1] = THAISEN_GUN_RUNING_MODE_CHARGE;
                }
                if(LcdData.runData.GunRunMode[LCD_GUN_1] == THAISEN_GUN_RUNING_MODE_V2G){
                    mode = SerialScreen_Screen_ConvertMode(LCD_GUN_1);
                    thaisen_get_mode_info(LCD_GUN_1, THA_DEBUG_LANGUAGE_CHINESE, mode, SerialScreen_Screen_GetV2GParameter(LCD_GUN_1), \
                ...
                {
                    mode = SerialScreen_Screen_ConvertMode(LCD_GUN_1);
                    thaisen_get_mode_info(LCD_GUN_1, THA_DEBUG_LANGUAGE_CHINESE, mode, SerialScreen_Screen_GetChargeModeParameter(LCD_GUN_1), \
                            LcdData.setData.CurrentModeInfo[LCD_GUN_1], sizeof(LcdData.setData.CurrentModeInfo[LCD_GUN_1]));
                }
            }else if(LcdData.gunIndex == LCD_GUN_2){
#ifdef SCREEN_USING_V2G
                if((LcdData.runData.GunRunMode[LCD_GUN_2] == THAISEN_GUN_RUNING_MODE_V2G) && (SerialScreen_Screen_GetCurrentV2GMode(LCD_GUN_2) >= THAISEN_V2G_MODE_SIZE)){
                    LcdData.runData.GunRunMode[LCD_GUN_2] = THAISEN_GUN_RUNING_MODE_CHARGE;
                }
                if(LcdData.setData.sup_V2G == FALSE){
                    LcdData.runData.GunRunMode[LCD_GUN_2] = THAISEN_GUN_RUNING_MODE_CHARGE;
                }
                if(LcdData.runData.GunRunMode[LCD_GUN_2] == THAISEN_GUN_RUNING_MODE_V2G){
                    mode = SerialScreen_Screen_ConvertMode(LCD_GUN_2);
                    thaisen_get_mode_info(LCD_GUN_2, THA_DEBUG_LANGUAGE_CHINESE, mode, SerialScreen_Screen_GetV2GParameter(LCD_GUN_2), \
                ...
                {
                    mode = SerialScreen_Screen_ConvertMode(LCD_GUN_2);
                    thaisen_get_mode_info(LCD_GUN_2, THA_DEBUG_LANGUAGE_CHINESE, mode, SerialScreen_Screen_GetChargeModeParameter(LCD_GUN_2), \
                            LcdData.setData.CurrentModeInfo[LCD_GUN_2], sizeof(LcdData.setData.CurrentModeInfo[LCD_GUN_2]));
                }
            }
        }
...
struct LCD_DATA_FIFO_TYPE *serialScreen_ObjectAi_Init(void)
{
    ...
    SerialScreen_ItemSetUp(LCD_PAGE_STANDBY, NULL, "auxA noticeA", LCD_IconType, LCD_10sReflash, 0x6C00, pu8_type, sizeof(LcdData.setData.selectAuxNotice[LCD_GUN_1]), (void *)&LcdData.setData.selectAuxNotice[LCD_GUN_1]);
    SerialScreen_ItemSetUp(LCD_PAGE_STANDBY, NULL, "auxB noticeB", LCD_IconType, LCD_10sReflash, 0x6C01, pu8_type, sizeof(LcdData.setData.selectAuxNotice[LCD_GUN_2]), (void *)&LcdData.setData.selectAuxNotice[LCD_GUN_2]);
    ...
    SerialScreen_ItemSetUp(LCD_PAGE_STANDBY, NULL, "auxA set", LCD_BtnType, 0x000D, 0x1007, page_type, LCD_PAGE_STANDBY, (void *)SerialScreen_AuxsetA_New);
    SerialScreen_ItemSetUp(LCD_PAGE_STANDBY, NULL, "auxB set", LCD_BtnType, 0x000E, 0x1007, page_type, LCD_PAGE_STANDBY, (void *)SerialScreen_AuxsetB_New);
    SerialScreen_ItemSetUp(LCD_PAGE_STANDBY, NULL, "auxA verifyA", LCD_BtnType, 0x0009, 0x1007, page_type, LCD_PAGE_STANDBY, (void *)SerialScreen_AuxSelectVerifyA);
    SerialScreen_ItemSetUp(LCD_PAGE_STANDBY, NULL, "auxA cancelA", LCD_BtnType, 0x000A, 0x1007, page_type, LCD_PAGE_STANDBY, (void *)SerialScreen_AuxSelectCancelA);
    SerialScreen_ItemSetUp(LCD_PAGE_STANDBY, NULL, "auxB verifyB", LCD_BtnType, 0x000B, 0x1007, page_type, LCD_PAGE_STANDBY, (void *)SerialScreen_AuxSelectVerifyB);
    SerialScreen_ItemSetUp(LCD_PAGE_STANDBY, NULL, "auxB cancelB", LCD_BtnType, 0x000C, 0x1007, page_type, LCD_PAGE_STANDBY, (void *)SerialScreen_AuxSelectCancelB);
    ...
    SerialScreen_ItemSetUp(LCD_PAGE_A_SELECT, NULL, "auxA noticeA", LCD_IconType, LCD_10sReflash, 0x6C00, pu8_type, sizeof(LcdData.setData.selectAuxNotice[LCD_GUN_1]), (void *)&LcdData.setData.selectAuxNotice[LCD_GUN_1]);
    SerialScreen_ItemSetUp(LCD_PAGE_A_SELECT, NULL, "Icon ModeA", LCD_IconType, LCD_10sReflash, 0x65C2, pu8_type, sizeof(LcdData.setData.selectmode[LCD_GUN_1]), (void *)&LcdData.setData.selectmode[LCD_GUN_1]);
    ...
    SerialScreen_ItemSetUp(LCD_PAGE_A_SELECT, NULL, "auxA set", LCD_BtnType, 0x000D, 0x1007, page_type, LCD_PAGE_A_SELECT, (void *)SerialScreen_AuxsetA_New);
    SerialScreen_ItemSetUp(LCD_PAGE_A_SELECT, NULL, "auxA verifyA", LCD_BtnType, 0x0009, 0x1007, page_type, LCD_PAGE_A_SELECT, (void *)SerialScreen_AuxSelectVerifyA);
    SerialScreen_ItemSetUp(LCD_PAGE_A_SELECT, NULL, "auxA cancelA", LCD_BtnType, 0x000A, 0x1007, page_type, LCD_PAGE_A_SELECT, (void *)SerialScreen_AuxSelectCancelA);
    ...
    SerialScreen_ItemSetUp(LCD_PAGE_B_SELECT, NULL, "auxB noticeB", LCD_IconType, LCD_10sReflash, 0x6C01, pu8_type, sizeof(LcdData.setData.selectAuxNotice[LCD_GUN_2]), (void *)&LcdData.setData.selectAuxNotice[LCD_GUN_2]);
    SerialScreen_ItemSetUp(LCD_PAGE_B_SELECT, NULL, "Icon ModeB", LCD_IconType, LCD_10sReflash, 0x65C3, pu8_type, sizeof(LcdData.setData.selectmode[LCD_GUN_2]), (void *)&LcdData.setData.selectmode[LCD_GUN_2]);
    ...
    SerialScreen_ItemSetUp(LCD_PAGE_B_SELECT, NULL, "auxB set", LCD_BtnType, 0x000E, 0x1007, page_type, LCD_PAGE_B_SELECT, (void *)SerialScreen_AuxsetB_New);
    SerialScreen_ItemSetUp(LCD_PAGE_B_SELECT, NULL, "auxB verifyB", LCD_BtnType, 0x000B, 0x1007, page_type, LCD_PAGE_B_SELECT, (void *)SerialScreen_AuxSelectVerifyB);
    SerialScreen_ItemSetUp(LCD_PAGE_B_SELECT, NULL, "auxB cancelB", LCD_BtnType, 0x000C, 0x1007, page_type, LCD_PAGE_B_SELECT, (void *)SerialScreen_AuxSelectCancelB);
    ...
    /** 16.指令调试控制 [page:16] */
    SerialScreen_ItemSetUp(LCD_PAGE_CMD_DEBUG, NULL, "menu input", LCD_BtnType, 0x001E, 0x1000, page_type, LCD_PAGE_MENU_INPUT, (void *)SerialScreen_InputInfoGet);
    SerialScreen_ItemSetUp(LCD_PAGE_CMD_DEBUG, NULL, "menu protect", LCD_BtnType, 0x0020, 0x1000, page_type, LCD_PAGE_MENU_PROTECT, (void *)SerialScreen_BtnProtectInfoGet);
    SerialScreen_ItemSetUp(LCD_PAGE_CMD_DEBUG, NULL, "menu config", LCD_BtnType, 0x0021, 0x1000, page_type, LCD_PAGE_MENU_CONFIG, (void *)SerialScreen_IsSupportGet);
#if (defined(SCREEN_USING_V2G) || defined(SCREEN_USING_OFFLINE_BILLING))
    ...
    SerialScreen_ItemSetUp(LCD_PAGE_CMD_DEBUG, NULL, "IconProtocolGB-T", LCD_IconType, LCD_10sReflash, 0x6CB2, pu8_type, sizeof(LcdData.setData.Icon_SupProtocolGB_T), (void *)&LcdData.setData.Icon_SupProtocolGB_T);
    SerialScreen_ItemSetUp(LCD_PAGE_CMD_DEBUG, NULL, "IconBMSSFrame", LCD_IconType, LCD_10sReflash, 0x6CB0, pu8_type, sizeof(LcdData.setData.Icon_SupSeveralFrame), (void *)&LcdData.setData.Icon_SupSeveralFrame);
    ...
    SerialScreen_ItemSetUp(LCD_PAGE_CMD_DEBUG, NULL, "ProtocolGB-TSet", LCD_BtnType, 0x006A, 0x1009, page_type, LCD_PAGE_CMD_DEBUG, (void *)SerialScreen_IsSupportProtocolGB_TSet);
    SerialScreen_ItemSetUp(LCD_PAGE_CMD_DEBUG, NULL, "BMSSFrameSet", LCD_BtnType, 0x0069, 0x1009, page_type, LCD_PAGE_CMD_DEBUG, (void *)SerialScreen_IsSupportBMSSeveralFrameSet);

    SerialScreen_ItemSetUp(LCD_PAGE_CMD_DEBUG, NULL, "CmdIssue", LCD_BtnType, 0x0062, 0x1009, page_type, LCD_PAGE_CMD_DEBUG, (void *)SerialScreen_CmdDebugIssue);
    SerialScreen_ItemSetUp(LCD_PAGE_CMD_DEBUG, NULL, "CmdRead", LCD_BtnType, 0x0063, 0x1009, page_type, LCD_PAGE_CMD_DEBUG, (void *)SerialScreen_CmdDebugRead);

    SerialScreen_ItemSetUp(LCD_PAGE_CMD_DEBUG, NULL, "Cmd", LCD_InputType, 0, 0x6C24, pstr_type, (sizeof(LcdData.setData.DebugCmd) - 2), (void *)LcdData.setData.DebugCmd);
    SerialScreen_ItemSetUp(LCD_PAGE_CMD_DEBUG, NULL, "CmdPara", LCD_InputType, 0, 0x6C34, pstr_type, (sizeof(LcdData.setData.DebugCmdPara) - 2), (void *)LcdData.setData.DebugCmdPara);
    SerialScreen_ItemSetUp(LCD_PAGE_CMD_DEBUG, NULL, "DebugResult", LCD_TextType, LCD_1sReflash, 0x6C44, pstr_type, sizeof(LcdData.setData.DebugResult), (void *)LcdData.setData.DebugResult);
    ...
    /** 17.其它配置 [page:17] */
    SerialScreen_ItemSetUp(LCD_PAGE_OTHER_CONFIG, NULL, "menu input", LCD_BtnType, 0x001E, 0x1000, page_type, LCD_PAGE_MENU_INPUT, (void *)SerialScreen_InputInfoGet);
    SerialScreen_ItemSetUp(LCD_PAGE_OTHER_CONFIG, NULL, "menu protect", LCD_BtnType, 0x0020, 0x1000, page_type, LCD_PAGE_MENU_PROTECT, (void *)SerialScreen_BtnProtectInfoGet);
    SerialScreen_ItemSetUp(LCD_PAGE_OTHER_CONFIG, NULL, "menu config", LCD_BtnType, 0x0021, 0x1000, page_type, LCD_PAGE_MENU_CONFIG, (void *)SerialScreen_IsSupportGet);
#if (defined(SCREEN_USING_V2G) || defined(SCREEN_USING_OFFLINE_BILLING))
    SerialScreen_ItemSetUp(LCD_PAGE_OTHER_CONFIG, NULL, "menu offbilling", LCD_BtnType, 0x0053, 0x1000, page_type, LCD_PAGE_OFFLINE_BILLING, (void *)SerialScreen_OfflineBillingGet);
#endif /* (defined(SCREEN_USING_V2G) || defined(SCREEN_USING_OFFLINE_BILLING)) */
...
#ifdef SCREEN_USING_V2G
    SerialScreen_ItemSetUp(LCD_PAGE_DISCHARGE_MODE, NULL, "Mode Money", LCD_BtnType, 0x0072, 0x1009, page_type, LCD_PAGE_DISCHARGE_MODE, (void *)SerialScreen_BtnV2GLimitMoneySet);
    SerialScreen_ItemSetUp(LCD_PAGE_DISCHARGE_MODE, NULL, "Mode Elect", LCD_BtnType, 0x0073, 0x1009, page_type, LCD_PAGE_DISCHARGE_MODE, (void *)SerialScreen_BtnV2GLimitElectSet);
    SerialScreen_ItemSetUp(LCD_PAGE_DISCHARGE_MODE, NULL, "Mode Auto", LCD_BtnType, 0x0075, 0x1009, page_type, LCD_PAGE_DISCHARGE_MODE, (void *)SerialScreen_BtnV2GChargeAutoSet);
    SerialScreen_ItemSetUp(LCD_PAGE_DISCHARGE_MODE, NULL, "Mode Timing", LCD_BtnType, 0x0074, 0x1009, page_type, LCD_PAGE_DISCHARGE_MODE, (void *)SerialScreen_BtnV2GLimitTimingSet);
    SerialScreen_ItemSetUp(LCD_PAGE_DISCHARGE_MODE, NULL, "OnContravariant", LCD_BtnType, 0x0089, 0x1009, page_type, LCD_PAGE_DISCHARGE_MODE, (void *)SerialScreen_BtnV2G_On_ContravariantSet);
    SerialScreen_ItemSetUp(LCD_PAGE_DISCHARGE_MODE, NULL, "OffContravariant", LCD_BtnType, 0x008A, 0x1009, page_type, LCD_PAGE_DISCHARGE_MODE, (void *)SerialScreen_BtnV2G_Off_ContravariantSet);

    SerialScreen_ItemSetUp(LCD_PAGE_DISCHARGE_MODE, NULL, "Icon warn", LCD_IconType, LCD_10sReflash, 0x65C4, pu8_type, sizeof(LcdData.setData.Modewarning), (void *)&LcdData.setData.Modewarning);
    SerialScreen_ItemSetUp(LCD_PAGE_DISCHARGE_MODE, NULL, "Icon Mode Money", LCD_IconType, LCD_10sReflash, 0x6D66, pu8_type, sizeof(LcdData.setData.V2G_CurrentMode[LCD_GUN_NUM][THAISEN_V2G_MODE_LIMIT_MONEY]), (void *)&LcdData.setData.V2G_CurrentMode[LCD_GUN_NUM][THAISEN_V2G_MODE_LIMIT_MONEY]);
    SerialScreen_ItemSetUp(LCD_PAGE_DISCHARGE_MODE, NULL, "Icon Mode Elect", LCD_IconType, LCD_10sReflash, 0x6D68, pu8_type, sizeof(LcdData.setData.V2G_CurrentMode[LCD_GUN_NUM][THAISEN_V2G_MODE_LIMIT_ELECT]), (void *)&LcdData.setData.V2G_CurrentMode[LCD_GUN_NUM][THAISEN_V2G_MODE_LIMIT_ELECT]);
    SerialScreen_ItemSetUp(LCD_PAGE_DISCHARGE_MODE, NULL, "Icon Mode Auto", LCD_IconType, LCD_10sReflash, 0x6D6C, pu8_type, sizeof(LcdData.setData.V2G_CurrentMode[LCD_GUN_NUM][THAISEN_V2G_MODE_AUTO]), (void *)&LcdData.setData.V2G_CurrentMode[LCD_GUN_NUM][THAISEN_V2G_MODE_AUTO]);
    SerialScreen_ItemSetUp(LCD_PAGE_DISCHARGE_MODE, NULL, "Icon Mode Timing", LCD_IconType, LCD_10sReflash, 0x6D6A, pu8_type, sizeof(LcdData.setData.V2G_CurrentMode[LCD_GUN_NUM][THAISEN_V2G_MODE_LIMIT_TIMING]), (void *)&LcdData.setData.V2G_CurrentMode[LCD_GUN_NUM][THAISEN_V2G_MODE_LIMIT_TIMING]);
    SerialScreen_ItemSetUp(LCD_PAGE_DISCHARGE_MODE, NULL, "Icon DisChargeWay", LCD_IconType, LCD_10sReflash, 0x6D8E, pu8_type, sizeof(LcdData.setData.DisChargeWay[LCD_GUN_NUM]), (void *)&LcdData.setData.DisChargeWay[LCD_GUN_NUM]);

    SerialScreen_ItemSetUp(LCD_PAGE_DISCHARGE_MODE, NULL, "Limit Money", LCD_InputType, 0, 0x6D6E, pu32_type, sizeof(LcdData.setData.V2G_MSLimitMoney[LCD_GUN_NUM]), (void *)&LcdData.setData.V2G_MSLimitMoney[LCD_GUN_NUM]);
    SerialScreen_ItemSetUp(LCD_PAGE_DISCHARGE_MODE, NULL, "Limit Elect", LCD_InputType, 0, 0x6D72, pu32_type, sizeof(LcdData.setData.V2G_MSLimitElect[LCD_GUN_NUM]), (void *)&LcdData.setData.V2G_MSLimitElect[LCD_GUN_NUM]);
#if 0
    SerialScreen_ItemSetUp(LCD_PAGE_DISCHARGE_MODE, NULL, "Limit RHour", LCD_InputType, 0, 0x659C, pu8_type, sizeof(LcdData.setData.V2G_MSLimitReservationHour[LCD_GUN_NUM]), (void *)&LcdData.setData.V2G_MSLimitReservationHour[LCD_GUN_NUM]);
    SerialScreen_ItemSetUp(LCD_PAGE_DISCHARGE_MODE, NULL, "Limit RMin", LCD_InputType, 0, 0x659E, pu8_type, sizeof(LcdData.setData.V2G_MSLimitReservationMin[LCD_GUN_NUM]), (void *)&LcdData.setData.V2G_MSLimitReservationMin[LCD_GUN_NUM]);
#endif
    ...
    SerialScreen_ItemSetUp(LCD_PAGE_ROOT_MAIN, NULL, "factory debug", LCD_BtnType, 0x001B, 0x1000, page_type, LCD_PAGE_MENU_MONITOR, (void *)SerialScreen_EnterMonitor);
...
//    SerialScreen_ItemSetUp(LCD_PAGE_MENU_INPUT, NULL, "Fuse neg set", LCD_BtnType, 0x005B, 0x1003, page_type, LCD_PAGE_MENU_INPUT, (void *)SerialScreen_FuseNegIsSupportSet);
    ...
    SerialScreen_ItemSetUp(LCD_PAGE_MENU_MODE_SELECT, NULL, "Icon Mode Money", LCD_IconType, LCD_10sReflash, 0x65A0, pu8_type, sizeof(LcdData.setData.CurrentMode[LCD_GUN_NUM][THAISEN_CHARGE_MODE_LIMIT_MONEY]), (void *)&LcdData.setData.CurrentMode[LCD_GUN_NUM][THAISEN_CHARGE_MODE_LIMIT_MONEY]);
    SerialScreen_ItemSetUp(LCD_PAGE_MENU_MODE_SELECT, NULL, "Icon Mode Elect", LCD_IconType, LCD_10sReflash, 0x65A1, pu8_type, sizeof(LcdData.setData.CurrentMode[LCD_GUN_NUM][THAISEN_CHARGE_MODE_LIMIT_ELECT]), (void *)&LcdData.setData.CurrentMode[LCD_GUN_NUM][THAISEN_CHARGE_MODE_LIMIT_ELECT]);
    SerialScreen_ItemSetUp(LCD_PAGE_MENU_MODE_SELECT, NULL, "Icon Mode Full", LCD_IconType, LCD_10sReflash, 0x65C1, pu8_type, sizeof(LcdData.setData.CurrentMode[LCD_GUN_NUM][THAISEN_CHARGE_MODE_FULL]), (void *)&LcdData.setData.CurrentMode[LCD_GUN_NUM][THAISEN_CHARGE_MODE_FULL]);
    SerialScreen_ItemSetUp(LCD_PAGE_MENU_MODE_SELECT, NULL, "Icon Mode Timing", LCD_IconType, LCD_10sReflash, 0x65A2, pu8_type, sizeof(LcdData.setData.CurrentMode[LCD_GUN_NUM][THAISEN_CHARGE_MODE_LIMIT_TIMING]), (void *)&LcdData.setData.CurrentMode[LCD_GUN_NUM][THAISEN_CHARGE_MODE_LIMIT_TIMING]);
    SerialScreen_ItemSetUp(LCD_PAGE_MENU_MODE_SELECT, NULL, "Icon Mode Reservation", LCD_IconType, LCD_10sReflash, 0x65A3, pu8_type, sizeof(LcdData.setData.CurrentMode[LCD_GUN_NUM][THAISEN_CHARGE_MODE_LIMIT_RESERVATION]), (void *)&LcdData.setData.CurrentMode[LCD_GUN_NUM][THAISEN_CHARGE_MODE_LIMIT_RESERVATION]);
//    SerialScreen_ItemSetUp(LCD_PAGE_MENU_MODE_SELECT, NULL, "Icon warn", LCD_IconType, LCD_10sReflash, 0x65C4, pu8_type, sizeof(LcdData.setData.Modewarning), (void *)&LcdData.setData.Modewarning);
    ...
    SerialScreen_ItemSetUp(LCD_PAGE_MENU_CONFIG, NULL, "mode select set", LCD_BtnType, 0x0063, 0x1003, page_type, LCD_PAGE_MENU_CONFIG, (void *)SerialScreen_IsSupportModeSelectSet);
#ifdef SCREEN_USING_V2G
    SerialScreen_ItemSetUp(LCD_PAGE_MENU_CONFIG, NULL, "V2G set", LCD_BtnType, 0x0072, 0x1009, page_type, LCD_PAGE_MENU_CONFIG, (void *)SerialScreen_IsSupportV2GSet);
#endif /* SCREEN_USING_V2G */
    ...
    SerialScreen_ItemSetUp(LCD_PAGE_MENU_CONFIG, NULL, "mode select", LCD_IconType, LCD_10sReflash, 0x6559, pu8_type, sizeof(LcdData.setData.Icon_SupModeSelect), (void *)&LcdData.setData.Icon_SupModeSelect);
#ifdef SCREEN_USING_V2G
    SerialScreen_ItemSetUp(LCD_PAGE_MENU_CONFIG, NULL, "v2g", LCD_IconType, LCD_10sReflash, 0x6D8C, pu8_type, sizeof(LcdData.setData.Icon_SupV2G), (void *)&LcdData.setData.Icon_SupV2G);
#endif /* SCREEN_USING_V2G */
...
#ifdef SCREEN_USING_DOUBLE_GUN
    SerialScreen_ItemSetUp(LCD_PAGE_MENU_MONITOR, NULL, "liquid cool", LCD_BtnType, 0x0001, 0x1008, page_type, LCD_PAGE_LIQUID_COOLINGA, (void *)SerialScreen_EnterLiquid);
#endif /* SCREEN_USING_DOUBLE_GUN */
...
#ifdef SCREEN_USING_DOUBLE_GUN
    SerialScreen_ItemSetUp(LCD_PAGE_MENU_MONITOR_B, NULL, "liquid cool", LCD_BtnType, 0x0001, 0x1008, page_type, LCD_PAGE_LIQUID_COOLINGA, (void *)SerialScreen_EnterLiquid);
#endif /* SCREEN_USING_DOUBLE_GUN */
...
#ifdef SCREEN_USING_DOUBLE_GUN
    SerialScreen_ItemSetUp(LCD_PAGE_MENU_INOUT, NULL, "liquid cool", LCD_BtnType, 0x0001, 0x1008, page_type, LCD_PAGE_LIQUID_COOLINGA, (void *)SerialScreen_EnterLiquid);
#endif /* SCREEN_USING_DOUBLE_GUN */
    ...
    SerialScreen_ItemSetUp(LCD_PAGE_MENU_INOUT, NULL, "Lable selfCheck", LCD_IconType, LCD_10sReflash, 0x6712, pu8_type, sizeof(LcdData.setData.selfCheck_lable), (void *)&LcdData.setData.selfCheck_lable);
...
#ifdef SCREEN_USING_DOUBLE_GUN
    SerialScreen_ItemSetUp(LCD_PAGE_MENU_INOUT_B, NULL, "liquid cool", LCD_BtnType, 0x0001, 0x1008, page_type, LCD_PAGE_LIQUID_COOLINGA, (void *)SerialScreen_EnterLiquid);
#endif /* SCREEN_USING_DOUBLE_GUN */
...
#ifdef SCREEN_USING_DOUBLE_GUN
    SerialScreen_ItemSetUp(LCD_PAGE_MENU_STATE_MODULE, NULL, "liquid cool", LCD_BtnType, 0x0001, 0x1008, page_type, LCD_PAGE_LIQUID_COOLINGA, (void *)SerialScreen_EnterLiquid);
#endif /* SCREEN_USING_DOUBLE_GUN */
...
#ifdef SCREEN_USING_DOUBLE_GUN
    SerialScreen_ItemSetUp(LCD_PAGE_MENU_STATE_MODULE_B, NULL, "liquid cool", LCD_BtnType, 0x0001, 0x1008, page_type, LCD_PAGE_LIQUID_COOLINGA, (void *)SerialScreen_EnterLiquid);
#endif /* SCREEN_USING_DOUBLE_GUN */
